...
/**
 * The graph edits a canvas gesture makes, against the project we ship.
 *
...
import { agentProfiles, agentSessions, defaultAgentInvokerOptions } from "../model/agents";
import { entityLabel } from "../model/names";
import { SCHEMA_VERSION, subroutineInProject, type CanonicalProject } from "../model/project";
...
const load = (): Project => testProject() as Project;
const withNodeResources = (project: Project, subroutine: string, node: string, submitted: ResourceSelection[]) =>
  setNodeResources({
  ...
  }, subroutine, node, submitted);
const ajv = new Ajv2020({ allErrors: true, strict: true });
const validateProject = ajv.compile(projectSchema);
const validateDefinition = (name: string) => ajv.compile({
  $defs: projectSchema.$defs,
  $ref: `#/$defs/${name}`,
});

const subroutineOf = (project: Project, id = SUBROUTINE) =>
  subroutineInProject(project, id) as unknown as Record<string, unknown>;

const ids = (value: unknown) =>
  (value as Record<string, unknown>[]).map((item) => String(item.id));
...
test("the schema applies the identifier grammar to canonical names", () => {
  const validateDefinition = ajv.compile(projectSchema.$defs.canonical_identifier);
  ...
  const validateQualified = ajv.compile(projectSchema.$defs.qualified_subroutine);
  for (const value of [
    "class.tools/main",
    "space.class/main",
...
test("only in-process calls carry explicit resource arguments", () => {
  const valid = load();
  const call = (subroutineOf(valid).nodes as Record<string, unknown>[])
  ...
  assert.equal(validateProject(valid), true, ajv.errorsText(validateProject.errors));
  const missing = structuredClone(valid) as Project;
  (subroutineOf(missing).nodes as Record<string, unknown>[])
    .find((node) => node.id === "call_implement")!.operation = { target: "main__implement" };
  assert.equal(validateProject(missing), false);
  const workflow = structuredClone(valid) as Project;
  (subroutineOf(workflow).nodes as Record<string, unknown>[])
    .find((node) => node.id === "call_implement")!.kind = "workflow_call";
...
test("the schema enforces enum domains and observation values", () => {
  ...
  assert.equal(condition({ feature_id: "status", observation: "equal", value: "retry" }), true);
  assert.equal(condition({ feature_id: "status", observation: "equal" }), false);
  assert.equal(effect({ feature_id: "status", observation: "unconstrained" }), true);
  assert.equal(
    effect({ feature_id: "status", observation: "unconstrained", value: "retry" }),
    false,
  );
});
...
test("the schema makes agent resource shapes explicit", () => {
  ...
  assert.equal(node({ ...agent, operation: { model: "default", session: "decide" } }), false);
  ...
  assert.equal(profile({ ...validProfile, options: { ...validProfile.options, model: 1 } }), false);
  assert.equal(profile({ ...validProfile, options: { ...validProfile.options, model: "" } }), false);
  ...
  assert.equal(sessionParameter({ id: "decide", name: "Decide", persistent: true }), false);
});
...
test("the schema distinguishes pins, workflow definitions and qualified calls", () => {
  ...
  assert.equal(node({ ...call, operation: { ...call.operation, target: "helper_stable/" } }), false);
  ...
  assert.equal(workflow({ ...reference, external: { package: "alice.helper", workflow: "main" } }), false);
...
test("adding a python node writes an operation with nothing in it", () => {
  ...
  const { id, project } = addNode(load(), SUBROUTINE, "python", "Measure the baseline");
  assert.equal(id, "measure_the_baseline");
  const node = (subroutineOf(project).nodes as Record<string, unknown>[]).at(-1)!;
...
test("adding a feature node writes the same empty operation", () => {
  const { id, project } = addNode(load(), SUBROUTINE, "feature", "Update progress");
...
test("agent nodes select their resources", () => {
  const agent = addNode(
    load(), SUBROUTINE, "agent", "Decide", undefined,
    { profile: "default", session: "optimize" },
  ).project;
  const node = (subroutineOf(agent).nodes as Record<string, unknown>[]).at(-1)!;
...
test("connecting two nodes adds an edge with no constraints yet", () => {
  const { files, id, project } = connect(load(), SUBROUTINE, "profile", "approve");
  assert.equal(id, "profile__approve");
  const edge = (subroutineOf(project).edges as Record<string, unknown>[]).at(-1)!;
  ...
  const parallel = connect(python.project, SUBROUTINE, "profile", "prepare_optimization");
...
test("ports enforce direction while ordinary node self-loops remain valid", () => {
  ...
  const edge = (subroutineOf(loop.project).edges as Record<string, unknown>[]).at(-1)!;
  assert.equal(edge.source, "profile");
...
test("relinking preserves an edge's identity, name, and constraints", () => {
  const before = load();
  const original = (subroutineOf(before).edges as Record<string, unknown>[]).find(
    (edge) => edge.id === "iteration_consumed",
  )!;
  ...
  const edge = (subroutineOf(changed.project).edges as Record<string, unknown>[]).find(
    (item) => item.id === "iteration_consumed",
  )!;
...
test("relinking rejects a derived-looking edge id that lies about its endpoints", () => {
  const project = load();
  const edge = (subroutineOf(project).edges as Record<string, unknown>[]).find(
    (item) => item.id === "enter__profile",
  )!;
  edge.id = "other__profile";

  assert.throws(
    () => relink(project, SUBROUTINE, "other__profile", "enter", "prepare_optimization"),
    /does not match its endpoints/,
  );
});

test("relinking moves a target-owned visit with either endpoint", () => {
  const moved = relink(load(), SUBROUTINE, "enter__profile", "enter", "prepare_optimization");
  ...
  const movedEdge = (subroutineOf(moved.project).edges as Record<string, unknown>[]).find(
    (edge) => edge.id === "enter__prepare_optimization",
  );
  assert.equal(movedEdge?.name, "Enter profile", "a custom name is preserved");
  const [file] = moved.files;
  assert.equal(file?.kind, "rename");
  assert.ok(
    (moved.project.sources as Record<string, unknown>[]).some(
      ({ path }) => path === (file?.kind === "rename" ? file.to : undefined),
    ),
  );

  const sourceOnly = relink(load(), SUBROUTINE, "enter__profile", "prepare_optimization", "profile");
...
test("relinking a generated parallel edge follows its endpoints without colliding", () => {
  ...
  const parallel = connect(first.project, SUBROUTINE, "profile", "prepare_optimization");
  const changed = relink(parallel.project, SUBROUTINE, parallel.id, "optimize", "plan");
  const edges = subroutineOf(changed.project).edges as Record<string, unknown>[];
  ...
  assert.ok(edges.some((item) => item.id === first.id), "the other parallel edge is untouched");
  assert.ok(edges.some((item) => item.id === "optimize__plan"), "the existing edge is untouched");
  assert.deepEqual(changed.files, [{
...
test("relinking preserves a custom id while updating a generated name", () => {
  ...
  const edge = (subroutineOf(connected.project).edges as Record<string, unknown>[]).find(
    (item) => item.id === connected.id,
  )!;
  edge.id = "approval";

  const changed = relink(connected.project, SUBROUTINE, "approval", "profile", "exit");
  const relinked = (subroutineOf(changed.project).edges as Record<string, unknown>[]).find(
...
test("target relinking can explicitly discard the old implementation", () => {
  ...
  const oldPath = "src/demo/project/subroutines/main/nodes/profile/visit/enter__profile/impl.py";
  const newPath =
...
test("deleting a node takes its edges and its position with it", () => {
  const before = load();
  const attached = (subroutineOf(before).edges as Record<string, unknown>[]).filter(
    (edge) => edge.source === "profile" || edge.target === "profile",
  );
  assert.ok(attached.length > 0, "the fixture must have edges on this node");

  const { code, impacts, project } = remove(before, SUBROUTINE, "nodes", "profile");
  ...
  for (const edge of ids(subroutineOf(project).edges)) {
    const found = (subroutineOf(project).edges as Record<string, unknown>[]).find(
      (item) => item.id === edge,
    )!;
  ...
  assert.ok(impacts.slice(1).every((impact) =>
    impact.effect === "delete" &&
    impact.entity === "edges" &&
    impact.reasons.includes("attached")
  ));
  const layouts = (
    (project.editor as Record<string, unknown>).layouts as Record<string, unknown>
...
test("deleting a feature takes the observations that named it", () => {
  const before = load();
  const named = (subroutineOf(before).edges as Record<string, unknown>[]).filter((edge) =>
    [...(edge.conditions as Record<string, unknown>[]), ...(edge.effects as Record<string, unknown>[])]
      .some((item) => item.feature_id === "remaining_iterations"),
  );
  assert.ok(named.length > 0, "the fixture must constrain this feature somewhere");

  const { impacts, project } = remove(before, SUBROUTINE, "features", "remaining_iterations");
  assert.equal(ids(subroutineOf(project).features).includes("remaining_iterations"), false);
  for (const edge of subroutineOf(project).edges as Record<string, unknown>[]) {
    for (const collection of ["conditions", "effects"] as const) {
      for (const item of edge[collection] as Record<string, unknown>[]) {
        assert.notEqual(item.feature_id, "remaining_iterations");
      }
    }
  }
  assert.ok(impacts.some((impact) =>
...
test("constraining an edge replaces rather than duplicates", () => {
  ...
  const edge = (subroutineOf(twice).edges as Record<string, unknown>[]).find(
  ...
  const after = (subroutineOf(cleared).edges as Record<string, unknown>[]).find(
    (item) => item.id === "profile_analyze",
  )!;
  ...
  assert.throws(
    () => constrain(
      load(), SUBROUTINE, "profile_analyze", "conditions",
      "remaining_iterations", "positive",
    ),
    /not a conditions observation for integer/,
  );
  assert.throws(
    () => constrain(
      load(), SUBROUTINE, "profile_analyze", "conditions", "missing", "greater_zero",
    ),
    /no feature missing/,
  );
  assert.throws(
    () => constrain(
      load(), SUBROUTINE, "profile_analyze", "conditions",
      "remaining_iterations", "greater_zero", "1",
    ),
...
test("only an edge from a feature node can author effects", () => {
  ...
  const edge = (subroutineOf(changed).edges as Record<string, unknown>[]).find(
    (item) => item.id === connected.id,
  )!;
  assert.deepEqual(edge.effects, [
  ...
  const featureEdge = (subroutineOf(repaired).edges as Record<string, unknown>[]).find(
    (item) => item.id === "iteration_consumed",
  )!;
...
test("an enum feature constrains an edge with one declared value", () => {
  ...
  const feature = (subroutineOf(project).features as Record<string, unknown>[]).at(-1)!;
  assert.deepEqual(feature.values, ["pending", "done"]);
  const edge = (subroutineOf(project).edges as Record<string, unknown>[]).find(
    (item) => item.id === "profile_analyze",
  )!;
  assert.deepEqual((edge.conditions as Record<string, unknown>[]).filter(
    (item) => item.feature_id === "status",
  ), [
    { feature_id: "status", observation: "equal", value: "pending" },
  ]);
  const effectEdge = (subroutineOf(project).edges as Record<string, unknown>[]).find(
    (item) => item.id === outgoing.id,
  )!;
  assert.deepEqual((effectEdge.effects as Record<string, unknown>[]).filter(
    (item) => item.feature_id === "status",
  ), [
  ...
  const changed = (subroutineOf(unconstrained).edges as Record<string, unknown>[]).find(
    (item) => item.id === outgoing.id,
  )!;
  assert.deepEqual((changed.effects as Record<string, unknown>[]).filter(
    (item) => item.feature_id === "status",
  ), [
...
test("every edit satisfies the project schema", () => {
  let project = load();
  project = addNode(project, SUBROUTINE, "feature", "Measure", { x: 40, y: 80 }).project;
  project = addFeature(project, SUBROUTINE, "integer", "Attempts left", "How many remain.").project;
  ...
  project = withNodeResources(project, SUBROUTINE, "plan", [
    { resource: "profile", value: "default" }, { resource: "session", value: "optimize" },
  ]);
  ...
  assert.equal(validateProject(oldDependencies), false, "dependencies live in requirements.txt");
  ...
  const feature = (subroutineOf(invalid).features as Record<string, unknown>[]).find(
    (item) => item.id === "attempts_left",
  )!;
  ...
  const measure = (subroutineOf(wrongOperation).nodes as Record<string, unknown>[]).find(
    (item) => item.id === "measure",
  )!;
  measure.operation = { target: "somewhere" };
  assert.equal(validateProject(wrongOperation), false, "feature operations must stay empty");

  const body = addSubroutineDefinition(project, "Worker", SUBROUTINE);
  const qualifiedWorkflow = addWorkflowDefinition(body.project, SUBROUTINE, body.id);
  assert.ok(validateProject(qualifiedWorkflow.project), ajv.errorsText(validateProject.errors));
  const canonicalDeclaration = structuredClone(body.project) as Project;
  ((subroutineOf(canonicalDeclaration).subroutines as Record<string, unknown>[])[0]).id =
    "main__worker";
  assert.equal(validateProject(canonicalDeclaration), false, "declaration ids stay local leaves");
  ...
  assert.ok(validateProject(externalCall), ajv.errorsText(validateProject.errors));
  ...
  const call = (subroutineOf(invalidCall).nodes as Record<string, unknown>[]).at(-1)!;
  call.operation = { target: "pin/main_body" };
  assert.equal(validateProject(invalidCall), false, "only subroutine calls accept alias/id");
});
...
test("every edit leaves a graph the canvas can still draw", () => {
  ...
  project = remove(project, SUBROUTINE, "nodes", "prepare_optimization").project;
  ...
  assert.equal(drawn.includes("prepare_optimization"), false, "the deleted one is gone");
  ...
  assert.ok(serialize(project).startsWith(`{\n  "schema_version": ${SCHEMA_VERSION},\n`));
  assert.ok(serialize(project).endsWith("}\n"));
});

test("definitions and calls are separate edits", () => {
  const before = load();
  const { id, project } = addSubroutineDefinition(before, "Validate changes", SUBROUTINE);
  assert.equal(id, "main__validate_changes");

  const child = (subroutineOf(project).subroutines as Record<string, unknown>[]).find(
    (item) => item.id === "validate_changes",
  )!;
  assert.deepEqual(child.ports, { enter: "enter", exit: "exit", failure: "failure" });
  ...
  const called = addCall(
    project, SUBROUTINE, "subroutine_call", "Validate", id, undefined, NO_ARGUMENTS,
  ).project;
  const caller = subroutineOf(project);
  const calling = (subroutineOf(called).nodes as Record<string, unknown>[]).at(-1)!;
  assert.equal(calling.kind, "subroutine_call");
  assert.deepEqual(calling.operation, { ...NO_ARGUMENTS, target: id });

  // Untouched original.
  assert.equal((subroutineOf(load()).subroutines as unknown[]).length,
    (subroutineOf(before).subroutines as unknown[]).length);
  assert.equal((caller.nodes as unknown[]).length, (subroutineOf(before).nodes as unknown[]).length);
});
...
test("definition names are unique only among siblings", () => {
  ...
  const rightRetry = addSubroutineDefinition(leftRetry.project, "Retry", right.id);

  assert.equal(leftRetry.id, "main__left__retry");
  assert.equal(rightRetry.id, "main__right__retry");
  assert.equal(addSubroutineDefinition(rightRetry.project, "Retry", left.id).id,
    "main__left__retry_2");
});
...
test("a workflow definition wraps one existing visible subroutine", () => {
  ...
  const { id, project } = addWorkflowDefinition(body.project, SUBROUTINE, body.id);
  const envelope = (subroutineOf(project).workflows as Record<string, unknown>[]).find(
    (item) => item.subroutine === id,
  )!;
  ...
  assert.equal((subroutineOf(project).subroutines as unknown[]).length, before.length);
  assert.throws(
...
test("workflow and subroutine ids have separate namespaces", () => {
  ...
  const invoked = addCall(
    second.project, SUBROUTINE, "workflow_call", "Run check", second.id,
  );
  assert.equal(first.id, "main__check");
  assert.equal(second.id, "main__check");
  assert.equal(addSubroutineDefinition(load(), "Profile", SUBROUTINE).id, "main__profile");
  ...
  assert.equal(
    graphs[SUBROUTINE].nodes.some((node) =>
      node.id === "subroutine:check" || node.id === "workflow:check"
    ),
    false,
  );
  assert.deepEqual(graphs["workflow:main__check"].edges, []);
  assert.deepEqual(
    graphs["workflow:main__check"].nodes.map(({ id, data }) => ({ id, kind: data.kind })),
...
test("deleting a definition cascades through its callers and attached edges", () => {
  const defined = addSubroutineDefinition(load(), "Validate changes", SUBROUTINE);
  const called = addCall(
    defined.project,
    SUBROUTINE,
    "subroutine_call",
  ...
  const removal = remove(connected.project, SUBROUTINE, "subroutines", defined.id);
  assert.equal(subroutineInProject(removal.project, defined.id), undefined);
  assert.equal(ids(subroutineOf(removal.project).nodes).includes(called.id), false);
  assert.equal(ids(subroutineOf(removal.project).edges).includes(connected.id), false);
  assert.ok(removal.impacts.some((impact) =>
    impact.effect === "delete" &&
    impact.entity === "nodes" &&
    impact.id === called.id &&
...
test("deleting a workflow removes its calls but preserves its wrapped subroutine", () => {
  ...
  const session = addWorkflowSession(profile.project, workflow.id, "Conversation", true);
  const call = addCall(
    session.project,
    SUBROUTINE,
    "workflow_call",
  ...
  assert.equal(ids(subroutineOf(removal.project).nodes).includes(call.id), false);
  assert.equal(
    (subroutineOf(removal.project).workflows as Record<string, unknown>[])
  ...
  assert.equal(
    removal.impacts.some((impact) =>
      impact.entity === "subroutines" && impact.id === body.id
    ),
    false,
  );
  assert.deepEqual(
...
test("ambiguous same-kind definitions block deletion", () => {
  ...
  const definition = (root.subroutines as Record<string, unknown>[])
    .find((item) => item.id === "duplicate")!;
...
test("deleting a subroutine subtree deletes its saved layouts", () => {
  const parent = addSubroutineDefinition(load(), "Validate changes", SUBROUTINE);
  ...
  const removed = remove(positioned, SUBROUTINE, "subroutines", parent.id).project;
  const layouts = (removed.editor as Record<string, unknown>).layouts as Record<string, unknown>;
...
test("subroutine deletion includes nested definitions, outside wrappers, and their calls", () => {
  ...
  const selected = addSubroutineDefinition(parent.project, "Validate changes", parent.id);
  ...
  const insideProfile = addWorkflowProfile(inside.project, inside.id, "Runtime");
  const outside = addWorkflowDefinition(insideProfile.project, parent.id, selected.id);
  const outsideProfile = addWorkflowProfile(outside.project, outside.id, "Runtime");
  const call = addCall(
  ...
  const removal = remove(unrelated.project, parent.id, "subroutines", selected.id);
  ...
  assert.equal(
    (survivingParent.workflows as Record<string, unknown>[])
      .some((workflow) => workflow.subroutine === selected.id),
    false,
  );
  ...
  assert.deepEqual(
    removal.impacts.find((impact) =>
      impact.entity === "profiles" && impact.workflow === outside.id
    )?.reasons,
  ...
  assert.equal(removal.code.some((path) => path.startsWith(`${selectedCode}/`)), false);
  assert.ok(removal.code.includes(
...
test("a populated subtree reports every contained identity exactly once", () => {
  ...
  const nested = addSubroutineDefinition(selected.project, "Nested", selected.id);
  const node = addNode(nested.project, selected.id, "python", "Work");
  const feature = addFeature(node.project, selected.id, "boolean", "Signal", "A signal.");
  const profileParameter = addAgentProfile(feature.project, selected.id, "Input profile", true);
  const profile = addAgentProfile(profileParameter.project, selected.id, "Reviewer", false);
  ...
  const workflow = addWorkflowDefinition(session.project, selected.id, nested.id);
  const workflowProfile = addWorkflowProfile(workflow.project, workflow.id, "Runner");
  ...
  const removal = remove(workflowSession.project, SUBROUTINE, "subroutines", selected.id);
  const row = (
    scope: string,
    entity: string,
    id: string,
  ...
  const actual = removal.impacts.map((impact) =>
    row(
      impact.scope.join("/"),
      impact.entity,
      impact.id,
  ...
  const expected = [
    ...
    ...["enter", "exit", "failure", node.id]
      .map((id) => row(selectedScope, "nodes", id, "contained")),
    ...
    row(`${selectedScope}/${workflow.id}`, "profiles", workflowProfile.id, "contained", workflow.id),
    row(`${selectedScope}/${workflow.id}`, "sessions", workflowSession.id, "contained", workflow.id),
    ...["enter", "exit", "failure"]
      .map((id) => row(nestedScope, "nodes", id, "contained")),
...
test("definition navigation marks lexical call visibility without hiding the project tree", () => {
  ...
  assert.equal(snapshotDefinitions(snapshot).some(({ id }) => id === child.id), true);
});
...
test("deleting an unrelated node orphans nothing", () => {
  ...
  assert.deepEqual(remove(project, SUBROUTINE, "nodes", "profile").orphaned, []);
});

/** Every `sources` path in a project, for the manifest assertions below. */
const sourcePaths = (project: Project): string[] =>
  ((project.sources ?? []) as Record<string, unknown>[]).map((source) => String(source.path));
...
test("deleting the root subroutine replaces it with an empty main", () => {
  ...
  (before.sources as Record<string, unknown>[]).push({
    ownership: "user",
  ...
  assert.equal(sourcePaths(removal.project).includes(workflowImplementation), false);
  assert.deepEqual((removal.project.editor as Record<string, unknown>).layouts, {});
  assert.equal(validateProject(removal.project), true, JSON.stringify(validateProject.errors));

  const keys = removal.impacts.map((impact) =>
    `${impact.scope.join("/")}|${impact.entity}|${impact.id}`
  );
  ...
  assert.deepEqual(
    removal.impacts
      ...
      .map((impact) => `${impact.entity}:${impact.id}|${impact.reasons.join(",")}`),
    [
...
test("deleting a node takes its manifest entries and names its directory", () => {
  ...
  assert.ok(sourcePaths(before).some((p) => p.startsWith(prefix)), "the fixture must have them");
  ...
  assert.deepEqual(sourcePaths(project).filter((p) => p.startsWith(prefix)), []);
...
test("a sibling whose id starts with the deleted one survives", () => {
  ...
  assert.ok(sourcePaths(project).includes(`${root}/profile_2/visit/other/impl.py`));
});

test("deleting a feature takes its implementation, not the shared registry", () => {
  const { code, project } = remove(load(), SUBROUTINE, "features", "remaining_iterations");
  assert.match(code[0] ?? "", /features\/remaining_iterations$/);
  assert.deepEqual(
    sourcePaths(project).filter((p) => p.includes("/features/remaining_iterations/")),
    [],
  );
  ...
  assert.ok(sourcePaths(project).some((p) => p.endsWith("/features/__init__.py")));
});

test("deleting an edge claims its target-side visit implementation", () => {
  const edge = String(
    (subroutineOf(load()).edges as Record<string, unknown>[])[0]?.id ?? "",
  );
...
test("profiles and sessions have separate identifier namespaces", () => {
  ...
  const session = addAgentSession(
    profile.project, SUBROUTINE, "Shared resource", false, true,
  );
  const duplicateProfile = addAgentProfile(
    session.project, SUBROUTINE, "Shared resource", false,
  );
  const duplicateSession = addAgentSession(
    duplicateProfile.project, SUBROUTINE, "Shared resource", true,
  );
...
test("workflow boundaries own concrete profiles and sessions without binding them", () => {
  ...
  const session = addWorkflowSession(duplicate.project, "main", "Runtime", false);
  ...
  assert.equal(session.id, "runtime", "profile and session ids have separate namespaces");
  ...
  assert.ok(validateProject(session.project), ajv.errorsText(validateProject.errors));
  assert.throws(() => addWorkflowProfile(load(), "missing", "Runtime"), /no local workflow/);
});
...
test("unbound workflow resources delete in their workflow scope and namespace", () => {
  ...
  assert.deepEqual(rootWorkflow.sessions, [
  ...
  const nested = addWorkflowSession(wrapped.project, wrapped.id, "Temporary", false);
  const removed = remove(
    nested.project, "main", "sessions", nested.id, wrapped.id,
  );
  ...
  const nestedWorkflow = (subroutineOf(removed.project).workflows as Record<string, unknown>[])
...
test("formal resources keep every targeting workflow envelope complete", () => {
  ...
  const profile = addAgentProfile(
    wrapped.project, "main__implement", "Generator", true,
  );
  const session = addAgentSession(
    profile.project, "main__implement", "Conversation", true,
  );
  const workflow = (subroutineOf(session.project).workflows as Record<string, unknown>[])
  ...
  assert.throws(() => setWorkflowResources(session.project, "main__implement", []),
    /not every required resource/);
  const withoutProfile = remove(
    rebound, "main__implement", "profile_parameters", "generator",
  );
  const withoutSession = remove(
    withoutProfile.project, "main__implement", "session_parameters", "conversation",
  );
  const remaining = (subroutineOf(withoutSession.project).workflows as Record<string, unknown>[])
    .find((candidate) => candidate.subroutine === "main__implement")!;
  ...
  const rootSession = addAgentSession(
    rootProfile.project, SUBROUTINE, "Conversation", true,
  );
  ...
  assert.ok(validateProject(rootSession.project), ajv.errorsText(validateProject.errors));
  const rootWithoutProfile = remove(
    rootSession.project, SUBROUTINE, "profile_parameters", "generator",
  );
  const rootWithoutSession = remove(
    rootWithoutProfile.project, SUBROUTINE, "session_parameters", "conversation",
  );
...
test("a new session parameter can reuse any existing boundary session", () => {
  ...
  assert.deepEqual((added.project.workflow as Record<string, unknown>).session_arguments, {
    temporary: "conversation",
    conversation: "conversation",
  });
  assert.deepEqual((added.project.workflow as Record<string, unknown>).sessions, [
    { id: "conversation", name: "Temporary", persistent: false },
  ]);
});
...
test("an agent selects a profile and session from its own subroutine", () => {
  ...
  const operation = (subroutineOf(changed).nodes as Record<string, unknown>[])
    .find((node) => node.id === "plan")?.operation;
  assert.deepEqual(operation, { profile: "default", session: "optimize" });
  assert.throws(
    () => withNodeResources(load(), SUBROUTINE, "plan", [
      { resource: "profile", value: "missing" }, { resource: "session", value: "optimize" },
    ]),
  ...
  const withAgent = addNode(
    child.project, child.id, "agent", "Nested", undefined,
    { profile: "default", session: "optimize" },
  );
  assert.throws(
...
test("profile options and session persistence update subroutine and workflow owners", () => {
  ...
  const localProfile = setProfileConfiguration(
    load(), SUBROUTINE, "default", "claude", options,
  );
  assert.deepEqual((subroutineOf(localProfile).profiles as Record<string, unknown>[])[0], {
  ...
  const workflowProfile = setProfileConfiguration(
    boundary, SUBROUTINE, "runtime", "claude", options, "main",
  );
  const workflowSession = setSessionPersistence(
    workflowProfile, SUBROUTINE, "runtime", true, "main",
  );
  assert.deepEqual((workflowSession.workflow as Record<string, unknown>).profiles, [{
    ...testProfile("runtime", "Runtime", "claude"),
    options,
  }]);
  assert.deepEqual((workflowSession.workflow as Record<string, unknown>).sessions, [
  ...
  const rebound = setSessionPersistence(bound, SUBROUTINE, "runtime", true, "main");
  assert.equal(
    ((rebound.workflow as Record<string, unknown>).sessions as Record<string, unknown>[])[0]
      ?.persistent,
    true,
  );
});
...
test("deleting a resource cascades through its dependent nodes", () => {
  ...
  const project = load();
  subroutineOf(project, "main__implement").profile_parameters = [{ id: "agent", name: "Agent" }];
  ...
  assert.deepEqual(
    ids(subroutineOf(removed.project).nodes),
    ["enter", "profile", "prepare_optimization", "consume_iteration", "approve", "exit", "failure"],
  );
  for (const id of ["call_implement", "optimize", "plan"]) {
    assert.ok(removed.impacts.some((impact) =>
      impact.effect === "delete" &&
      impact.entity === "nodes" &&
      impact.id === id &&
  ...
  assert.ok(removed.code.includes("src/demo/project/subroutines/main/nodes/call_implement"));
});
...
test("a call can be rebound before deleting its old resource", () => {
  ...
  let project = withNodeResources(replacement.project, SUBROUTINE, "optimize", [
    { resource: "profile", value: replacement.id }, { resource: "session", value: "optimize" },
  ]);
  project = withNodeResources(project, SUBROUTINE, "plan", [
    { resource: "profile", value: replacement.id }, { resource: "session", value: "plan" },
  ]);
  subroutineOf(project, "main__implement").profile_parameters = [{ id: "agent", name: "Agent" }];
  ...
  assert.ok(ids(subroutineOf(removed.project).nodes).includes("call_implement"));
...
test("removing a formal resource drops its local callers' obsolete keys", () => {
  ...
  const removal = remove(
    bound, "main__implement", "profile_parameters", "agent",
  );
  assert.deepEqual(removal.code, [
    "src/demo/project/subroutines/main/subroutines/implement/profiles/agent",
  ]);
  const removed = removal.project;
  const call = (subroutineOf(removed).nodes as Record<string, unknown>[])
    .find((node) => node.id === "call_implement")!;
...
test("a dependent deletion supersedes an update to the same identity", () => {
  ...
  const impact = removal.impacts.find((candidate) =>
    candidate.entity === "nodes" && candidate.id === recursive.id
  );
...
test("a name is prose, and changing it touches nothing else", () => {
  ...
  const before = load();
  const after = setName(before, SUBROUTINE, "nodes", "profile", "  Profile it properly  ");
  const node = (project: Project) =>
    (subroutineOf(project).nodes as Record<string, unknown>[]).find((n) => n.id === "profile")!;
  ...
  assert.deepEqual(ids(subroutineOf(after).nodes), ids(subroutineOf(before).nodes));
  assert.deepEqual(subroutineOf(after).edges, subroutineOf(before).edges);

  // A feature's prose is `label`, not `name`.
  const labelled = setName(before, SUBROUTINE, "features", "remaining_iterations", "Left to go");
  const feature = (subroutineOf(labelled).features as Record<string, unknown>[])[0];
  assert.equal(feature.label, "Left to go");
  assert.equal(feature.id, "remaining_iterations");

  const rootNamed = setName(before, SUBROUTINE, "subroutines", SUBROUTINE, "Primary process");
  ...
  assert.deepEqual(renamedBoundary.session_arguments, { conversation: "history" });

  assert.throws(() => setName(before, SUBROUTINE, "nodes", "profile", "   "), /must not be empty/);
  assert.throws(() => setName(before, SUBROUTINE, "nodes", "nope", "x"), /no node nope/);
});
...
test("aliases orphan independently when two pins name the same package", () => {
  const project = {
    ...
    subroutine: {
      ...
      subroutines: [],
      workflows: [
        { id: "stable_tools", name: "Stable", external: { alias: "channel.stable", workflow: "main" } },
        { id: "canary_tools", name: "Canary", external: { alias: "channel.canary", workflow: "main" } },
      ],
    },
    workflow: {
      subroutine: "main_body", profiles: [], sessions: [],
  ...
  const withoutStable = remove(project, "main_body", "workflows", "main_body__stable_tools");
  assert.deepEqual(withoutStable.orphaned, ["channel.stable"]);
  assert.deepEqual(
    remove(withoutStable.project, "main_body", "workflows", "main_body__canary_tools").orphaned.sort(),
    ["channel.canary"],
  );
});
...
test("a qualified subroutine call keeps its direct pin in use", () => {
  ...
  assert.deepEqual(
    remove(called, SUBROUTINE, "nodes", "use_tools").orphaned,
    ["publisher.tools"],
  );
});
