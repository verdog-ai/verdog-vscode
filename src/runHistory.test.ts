...
function run(
  ...
  return {
    ...
    workflow: {
      id: workflow,
      definition_id: `planning.${workflow}`,
...
test('run and checkpoint envelopes are decoded only at the supported schema', () => {
  ...
  assert.equal(
    parseRunsEnvelope(JSON.stringify({...runs, schema_version: 2})),
    undefined,
  );
  assert.equal(
    parseRunsEnvelope(
  ...
  assert.equal(
    boundaryLabel(response?.checkpoints[0]?.next ?? null),
    'main/learn:main/validate #2',
  );
  const childStart = parseCheckpointsEnvelope(
    JSON.stringify({
      ...checkpointEnvelope,
      checkpoints: [
        {...checkpointEnvelope.checkpoints[0], kind: 'child-start'},
      ],
    }),
  );
  ...
  assert.equal(
    parseCheckpointsEnvelope(
      JSON.stringify({
        ...checkpointEnvelope,
        checkpoints: [
          ...checkpointEnvelope.checkpoints,
          ...checkpointEnvelope.checkpoints,
        ],
      }),
    ),
...
test('lineage is grouped by workflow and malformed cycles stay visible as roots', () => {
  ...
  assert.deepEqual(
    trees.map(({workflow, count}) => [workflow.id, count]),
    [
      ['evaluation', 1],
      ['research', 5],
    ],
  ...
  assert.deepEqual(
    research.roots
      .filter(({run: item}) => item.id.startsWith('cycle'))
      .map(({run: item}) => item.id)
      .sort(),
    ['cycle-a', 'cycle-b'],
  );
});
...
test('operation and error envelopes remain separate', () => {
  ...
  assert.deepEqual(
    parseOperationEnvelope(JSON.stringify(operation)),
    operation,
  );
  assert.deepEqual(
  ...
  assert.equal(
    parseErrorEnvelope(
      JSON.stringify({
        status: 'error',
        error: {
          code: 'run.incomplete_invocation',
          message: 'confirmation required',
        },
      }),
    )?.error.code,
    'run.incomplete_invocation',
  );
});

test('failed operations retain structured errors for explicit provider retry', () => {
  const root = run('root');
  const operation = parseOperationEnvelope(
    JSON.stringify({
      schema_version: 1,
      operation: 'resume',
      status: 'failed',
  ...
  assert.equal(
    isIncompleteInvocationError({code: 'run.incomplete', message: 'old'}),
    true,
  );
  assert.equal(
    isIncompleteInvocationError({code: 'run.operation_failed', message: 'no'}),
    false,
  );
});
