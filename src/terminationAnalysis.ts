    ...
    definitions: {
      ...
      additionalProperties: {
        ...
        properties: {
          ...
          local_status: status,
          reason: {type: 'string'},
          memory_states: {type: 'integer', minimum: 0},
          ...
          edges: {
            type: 'object',
            additionalProperties: {
              enum: ['cleared', 'remaining', 'unreachable'],
            },
          },
          regions: {
            type: 'array',
            items: {
              type: 'object',
              required: ['id', 'nodes', 'edges', 'witnesses', 'cycle'],
              properties: {
                id: {type: 'string'},
                nodes: strings,
                edges: strings,
...
export function parseTerminationReport(
  stdout: string,
): TerminationReport | undefined {
  try {
    const value: unknown = JSON.parse(stdout);
    return validReport(value) && Object.hasOwn(value.projects, '')
      ? value
      : undefined;
  } catch {
    return undefined;
...
export class TerminationAnalysis {
  ...
  private completed:
    | {key: string; state: Extract<TerminationState, {status: 'ready'}>}
    | undefined;
  ...
  private async run(
    ...
    try {
      ...
      const report =
        result.code === 0 ? parseTerminationReport(result.stdout) : undefined;
      state = controller.signal.aborted
        ? {
            status: 'unavailable',
            reason: 'Analysis exceeded its five-second time limit.',
          }
        : report !== undefined && terminationRevision(report) === key
          ...
          : {
              ...
              reason:
                result.code !== 0
                  ? result.stderr.trim() ||
                    'Analysis failed. Check the Verdog service connection, then refresh the graph.'
                  : report === undefined
                    ? 'The Verdog analysis service returned an unsupported report.'
                    : owners.some(
                          owner => !Object.hasOwn(report.projects, owner),
                        )
