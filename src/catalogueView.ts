...
type CliFailure = { code?: string; message: string };
...
async function catalogueOutcome(root: string, query: CatalogueQuery): Promise<Outcome> {
  ...
  if (query.visibility && query.visibility !== "all") {
    arguments_.push("--visibility", query.visibility);
  }
...
async function readCatalogue(host: HostState, query: CatalogueQuery = {}): Promise<CatalogueListing> {
  ...
  try {
    const { entries, login, next_cursor, rejected } = decodeCatalogueListing(JSON.parse(listed.stdout));
    for (const problem of rejected) host.output.appendLine(`catalogue ignored ${problem}`);
    return {
      entries,
    ...
    host.output.appendLine(`verdog catalogue returned unreadable JSON: ${String(error)}`);
    return {
...
async function readEntry(host: HostState, id: string): Promise<CatalogueDetail | undefined> {
  const root = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? process.cwd();
  ...
  try {
    ...
    host.output.appendLine(`catalogue entry ${id} was invalid: ${String(error)}`);
    return undefined;
  }
}
...
async function availableInterpreter(root: string, workflow: string): Promise<string | undefined> {
...
class CatalogueRecordPanel {
  ...
  private ensurePanel(title: string): vscode.WebviewPanel {
    ...
    this.panel = panel;
    panel.webview.html = webviewHtml(panel.webview, this.context.extensionUri, "catalogue");
    panel.onDidDispose(() => {
    ...
    panel.webview.onDidReceiveMessage((message: CatalogueToHost) => void this.receive(message));
    return panel;
  }

  private post(): void {
    void this.panel?.webview.postMessage({ kind: "record", record: this.record } satisfies HostToCatalogue);
  }

  private async cachedInspection(detail: CatalogueDetail): Promise<CatalogueInspection> {
    ...
    if (state.catalogue !== catalogueMetadataKey(detail)) {
      return {
        detail: "Catalogue metadata changed after this cached inspection; inspect the release again.",
        folder: cached,
        state: "incomplete",
      };
    }
    if (state.metadata === "mismatch") {
      return { detail: "Published metadata does not match the exact source.", folder: cached, state: "mismatch" };
    }
    if (
      state.dependencies === "ready" && state.metadata === "ready" &&
      state.environment === "ready" && await availableInterpreter(cached, detail.workflow_id) !== undefined
    ) return { folder: cached, state: "ready" };
    return { detail: "Inspection was interrupted or the environment is incomplete.", folder: cached, state: "incomplete" };
  }
  ...
  private async select(id: string): Promise<void> {
    ...
    if (detail === undefined) {
      this.record = {
        documentation: { detail: "Documentation was not requested.", state: "unavailable" },
        inspection: { state: "idle" },
        message: "This exact catalogue release is unavailable or no longer visible.",
        requested: id,
        state: "unavailable",
      };
      this.post();
    ...
    for (const release of detail.releases) merged.set(release.id, release);
    this.releases = [...merged.values()].sort((left, right) =>
  ...
  private async inspect(): Promise<void> {
    const detail = this.record.detail;
    if (detail === undefined || this.record.inspection.state === "running") return;
    const revision = this.revision;
    const preview = previewOf(this.host, detail);
    this.inspectionAbort?.abort();
    const controller = new AbortController();
      ...
      async (progress, token) => {
        ...
        const phase = (label: string): void => {
          ...
          if (revision === this.revision) {
            this.updateInspection({ detail: label, phase: label, state: "running" });
          }
        };
        ...
        try {
          ...
          if (checkout === undefined) {
            this.updateInspection({ detail: controller.signal.aborted
              ? "Inspection was cancelled."
              : "The exact source could not be fetched. See the Verdog output.", state: "incomplete" });
            return;
          }
          root = checkout.folder.fsPath;
          if (checkout.dependencies !== "ready") {
            await this.finishIncomplete(root, preview, "Pinned dependencies could not all be fetched.", "unchecked");
            return;
          }

          phase("Verifying published metadata");
          const described = await verdog(root, ["describe", detail.workflow_id, "--json"], {
            command: cliCommand(),
          ...
          try {
            ...
            await this.finishIncomplete(root, preview, `Source metadata was unreadable: ${String(error)}`, "unchecked");
            return;
          }
          if (differences.length > 0) {
            ...
            await configurePreviewWorkspace(this.context.globalStorageUri, root, preview, undefined);
          ...
          const receipt = synced.code === 0
            ? decodeCatalogueSyncReceipt(synced.stdout, {
                checkout: root,
                workflow: detail.workflow_id,
              })
            : undefined;
          const interpreter = receipt !== undefined && await exists(receipt.interpreter)
            ? receipt.interpreter
            : undefined;
          ...
          await configurePreviewWorkspace(this.context.globalStorageUri, root, preview, interpreter);
          await lockPreview(root, (line) => this.host.output.appendLine(line));
          if (revision !== this.revision) return;
          ...
          if (root !== undefined) await lockPreview(root, (line) => this.host.output.appendLine(line));
          if (this.inspectionAbort === controller) this.inspectionAbort = undefined;
        }
      },
    );
  }
  ...
  private async openSource(): Promise<void> {
    const detail = this.record.detail;
    const folder = "folder" in this.record.inspection ? this.record.inspection.folder : undefined;
    if (detail === undefined || folder === undefined) return;
    const preview = previewOf(this.host, detail);
    const interpreter = this.record.inspection.state === "ready"
    ...
    this.host.output.appendLine(`opening inspected release from ${workspace.fsPath}`);
    try {
      await openPreviewWorkspace(this.context, workspace, folder);
    } catch (error) {
      this.host.output.appendLine(`opening the inspected workspace failed: ${String(error)}`);
      const answer = await vscode.window.showErrorMessage(
        "Could not open the inspected source in another window.",
        "Copy path",
      );
      if (answer === "Copy path") await vscode.env.clipboard.writeText(folder);
    }
  }
  ...
  private async import(): Promise<void> {
    ...
    const suggested = detail.package ?? detail.repository.replace("/", ".").toLowerCase();
    ...
    const closure = detail.closure.length === 0
      ? "No repository dependencies"
      : detail.closure.map((pin) => `${pin.repository}@${pin.commit}`).join("\n");
    ...
    if (answer !== "Import exact release") return;
    ...
    if (imported === undefined) {
      if (root === this.host.root && this.host.preview === undefined) await refresh(this.host as OpenHost);
    ...
    if (root === this.host.root && this.host.preview === undefined) await refresh(this.host as OpenHost);
    const generationFailed = result.code !== 0 || imported.generated_status !== 0;
    this.record = {
      ...this.record,
    ...
    this.post();
    const actions = root === this.host.root
    ...
    if (next === "Reveal binding") {
      const document = await vscode.workspace.openTextDocument(vscode.Uri.file(path.join(root, "project.json")));
...
async function existingAlias(root: string, alias: string): Promise<boolean> {
  try {
    const raw = await vscode.workspace.fs.readFile(vscode.Uri.file(path.join(root, "project.json")));
    const project = JSON.parse(Buffer.from(raw).toString("utf8")) as Record<string, unknown>;
    return Array.isArray(project.externals) && project.externals.some((item) =>
...
async function writableProjectAt(root: string): Promise<boolean> {
  try {
    const required = [root, path.join(root, "project.json"), path.join(root, ".git")];
    const modules = path.join(root, ".gitmodules");
    if (await exists(modules)) required.push(modules);
    await Promise.all(required.map((candidate) => accessFile(candidate, fsConstants.W_OK)));
    return true;
  } catch {
    return false;
  }
...
async function chooseTargetProject(host: HostState): Promise<string | undefined> {
  ...
  return (await vscode.window.showQuickPick(
    roots.map((root) => ({ description: root, label: path.basename(root), root })),
...
async function syncTarget(root: string, host: HostState): Promise<void> {
  ...
  if (result.code !== 0) {
    void vscode.window.showWarningMessage(`Verdog sync failed: ${cliFailure(result).message}`);
  }
}
...
class CatalogueView implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.html = webviewHtml(view.webview, this.context.extensionUri, "catalogue");
    view.webview.onDidReceiveMessage((message: CatalogueToHost) => void this.receive(message));
  }
  ...
  private async load(query: CatalogueQuery = this.query, append = false): Promise<void> {
    ...
    if (append && listing.state !== "unavailable" && listing.state !== "unauthenticated") {
      ...
      for (const entry of listing.entries) merged.set(entry.id, entry);
      this.entries = [...merged.values()];
      this.post({ ...listing, entries: this.entries, state: this.entries.length === 0 ? "empty" : "listed" });
      return;
    }
  ...
  private async receive(message: CatalogueToHost): Promise<void> {
    switch (message.kind) {
      ...
      case "refresh":
        ...
        return;
      case "search":
        await this.load({ query: message.query.trim(), visibility: message.visibility });
        return;
      case "load-more":
        if (message.cursor) await this.load({ ...this.query, cursor: message.cursor }, true);
        return;
      case "open-record": {
        const summary = this.entries.find((entry) => entry.id === message.entry) ??
          this.entries.find((entry) => entry.releases.some((release) => release.id === message.entry));
        if (summary !== undefined) await this.record.open(summary);
        return;
      }
      case "copy-reference":
      case "import":
...
async function folderSize(uri: vscode.Uri): Promise<number> {
  try {
    ...
    if (stat.type & vscode.FileType.File) return stat.size;
    const children = await vscode.workspace.fs.readDirectory(uri);
    return (await Promise.all(children.map(([name]) => folderSize(vscode.Uri.joinPath(uri, name)))))
...
async function manageCache(context: vscode.ExtensionContext): Promise<void> {
  ...
  const choices: { description: string; label: string; target?: vscode.Uri; workspace?: vscode.Uri }[] = [];
  try {
    for (const [name, type] of await vscode.workspace.fs.readDirectory(previewRoot)) {
      ...
      choices.push({
        ...
        target,
        workspace: vscode.Uri.file(previewWorkspace(
  ...
  const total = (await Promise.all(roots.map(folderSize))).reduce((sum, size) => sum + size, 0);
  choices.unshift({ description: `${formatBytes(total)} total`, label: "Remove all catalogue caches" });
  const selected = await vscode.window.showQuickPick(choices, {
    placeHolder: choices.length === 1 ? "The catalogue cache is empty" : undefined,
    title: `Verdog catalogue cache · ${formatBytes(total)}`,
  });
  if (selected === undefined) return;
  const all = selected.target === undefined;
  const answer = await vscode.window.showWarningMessage(
    all ? "Remove all Verdog catalogue caches?" : `Remove cached inspection of ${selected.label}?`,
    { modal: true },
    "Remove",
  );
  if (answer !== "Remove") return;
  if (all) {
    for (const root of roots) await deleteCache(root);
  } else {
    await deleteCache(selected.target!);
    if (selected.workspace !== undefined) await deleteCache(selected.workspace);
  }
}
...
export function registerCatalogue(
  ...
  return [
    ...
    vscode.commands.registerCommand("verdog.catalogue", () =>
      vscode.commands.executeCommand("workbench.view.extension.verdogCatalogue")
    ),
    vscode.commands.registerCommand("verdog.manageCatalogueCache", () => manageCache(context)),
...
async function runPreviewed(host: OpenHost): Promise<void> {
  ...
  if (answer === "Run anyway") {
    await runVerb(host, host.root, "run", host.preview?.workflow ? [host.preview.workflow] : []);
  }
}
