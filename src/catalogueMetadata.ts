...
/**
 * Stage-one catalogue documentation.
 *
...
import type { CatalogueDocumentation, CatalogueSummary } from "../model/catalogue";
...
export function readmeCandidates(packageName: string | null): string[] {
  return [
    "README.md",
    ...(packageName === null ? [] : [`src/${packageDirectory(packageName)}/README.md`]),
  ];
}
...
async function runGit(
  ...
  return verdog(root, arguments_, { command: ["git"], signal });
}
...
async function ensureBare(
  ...
  const bare = await runGit(root, ["rev-parse", "--is-bare-repository"], signal);
  if (bare.code === 0 && bare.stdout.trim() !== "true") {
    log("catalogue metadata cache is unexpectedly a worktree; remove it before retrying");
    return false;
  }
  if (bare.code !== 0) {
    const initialized = await runGit(root, ["init", "--bare", "--quiet"], signal);
    if (initialized.code !== 0) {
      log(`catalogue metadata cache could not be initialized: ${initialized.combined.trim()}`);
      return false;
    }
  }
  ...
  if (remote.code === 0) {
    ...
    log(`catalogue metadata cache has an unexpected origin; remove it before retrying`);
    return false;
  }
  ...
  if (added.code !== 0) {
    log(`catalogue metadata remote could not be configured: ${added.combined.trim()}`);
    return false;
  }
  return true;
}
...
async function fetchFromGit(
  ...
  if (!(await ensureBare(root, entry.repository, signal, log))) {
    return {
      detail: "The local metadata cache could not be initialized. See the Verdog output.",
      state: "unavailable",
    };
  }
  const fetchResult = await runGitHubGit(
    (arguments_) => runGit(root, arguments_, signal),
    ["fetch", "--quiet", "--depth", "1", "--filter=blob:none", "origin", entry.commit],
    signal,
  );
  const fetched = fetchResult.outcome;
  if (fetched.code !== 0) {
    const detail = fetched.code === 130
  ...
  for (const candidate of readmeCandidates(entry.package)) {
    ...
    const shown = await runGit(
      root,
      ["show", "--no-ext-diff", object],
      signal,
    );
    if (shown.code === 0 && Buffer.byteLength(shown.stdout, "utf8") <= MAX_README_BYTES) {
