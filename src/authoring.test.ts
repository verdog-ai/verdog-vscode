...
/**
 * @fileoverview The two things authoring completion has to get right.
 *
...
function after(text: string, needle: string, nth = 1): number {
  let index = -1;
...
test('subroutine targets include every subroutine in a direct pin', () => {
  ...
  assert.deepEqual(
    offered
      .filter(({externalAlias}) => externalAlias !== undefined)
      .map(({id}) => id),
    ['publisher.tools/main', 'publisher.tools/main__implement'],
  );
  assert.equal(
    documentDefinitions(PROJECT, 'main', 'workflow_call', external).some(
      ({id}) => id.includes('/'),
    ),
    false,
  );
...
test('an incomplete document keeps feature order, labels and subroutine scope', () => {
  ...
  assert.deepEqual(
    documentFeatures(text, 'first').map(feature => feature.id),
    ['wrong'],
  );
  const context = contextAt(text, text.length);
...
test('the observation vocabulary agrees with the project schema', () => {
  ...
  assert.deepEqual(
    [...new Set(Object.values(CONDITIONS).flat())].sort(),
    vocabulary('condition'),
  );
  assert.deepEqual(
    [...new Set(Object.values(EFFECTS).flat())].sort(),
    vocabulary('effect'),
  );
});
...
test('a skeleton asks only for what its author chooses', () => {
  ...
  const featureNode = snippetsFor('nodes').find(
    item => item.label === 'feature node',
  )!;
  ...
  assert.deepEqual(
    snippetsFor('features').map(
      item => /"kind": "([a-z]+)"/.exec(item.body)?.[1],
    ),
    [...FEATURE_KINDS],
  );
  const enumFeature = snippetsFor('features').find(
    item => item.label === 'enum feature',
  )!;
