...
/**
 * A preview is a clone you may not write to.
 *
...
import { MARKER, type Preview, checkoutRoot, interpreterIn, settings } from "./preview";
...
test("two releases of one workflow check out to different places", () => {
  ...
  const first = checkoutRoot("/storage", entry.repository, entry.commit, entry.workflow);
  const second = checkoutRoot("/storage", entry.repository, "b".repeat(40), entry.workflow);
  ...
  const hostile = checkoutRoot("/storage", "../outside", entry.commit, "../../workflow");
...
test("the settings make the checkout read-only and silent, and never speak for it", () => {
  const value = JSON.parse(settings(entry, "/home/dev/my-project/.venv/bin/python")) as Record<
    string,
    unknown
  >;
  ...
  assert.equal(value["python.defaultInterpreterPath"], "/home/dev/my-project/.venv/bin/python");
...
test("with no interpreter to borrow, the key is absent rather than empty", () => {
  ...
  const value = JSON.parse(settings(entry, undefined)) as Record<string, unknown>;
...
test("the extension remains available for safe inspection in Restricted Mode", async () => {
  ...
  assert.match(capability?.description ?? "", /read-only catalogue inspection/i);
});
