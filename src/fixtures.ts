...
/** @fileoverview Small in-memory projects shared by model tests. */
...
function port(id: string, kind: 'enter' | 'exit' | 'failure') {
  return {
    id,
    kind,
...
function emptySubroutine(id: string, name: string) {
  return {
    edges: [
      {
        conditions: [],
...
const PROJECT = {
  ...
  subroutine: {
    edges: [
      ...
      {
        conditions: [
          {feature_id: 'remaining_iterations', observation: 'equal_zero'},
        ],
        effects: [],
      ...
      },
      {
        ...
        effects: [],
        id: 'prepare_optimize',
        name: 'Prepare optimize',
        source: 'prepare_optimization',
        target: 'optimize',
      },
      {
        conditions: [],
      ...
      },
      {
        ...
        source: 'plan',
        target: 'call_implement',
      },
      {
        conditions: [],
        effects: [],
        id: 'implement_consume',
        name: 'Consume iteration',
        source: 'call_implement',
        target: 'consume_iteration',
      },
      {
        conditions: [
          {feature_id: 'remaining_iterations', observation: 'greater_zero'},
...
export function testProject(): Record<string, unknown> {
  return structuredClone(PROJECT);
}
export function testProjectText(): string {
