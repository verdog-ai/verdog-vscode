...
const emptyDocuments = (): ProjectSnapshot["entity_documents"] => ({
  edges: {},
  features: {},
  nodes: {},
  profile_parameters: {},
...
test("external workflow aliases keep local identity without exposing target bindings", () => {
  ...
  pinned.subroutine.profile_parameters = [{ id: agentProfileId("agent"), name: "Agent" }];
  pinned.workflow.profile_arguments = { [agentProfileId("agent")]: agentProfileId("builder") };
  ...
  assert.deepEqual(page?.fields, [{ label: "external", value: "publisher.tool / main" }]);
...
test("entity property pages use semantic titles", () => {
  ...
  const pages = [
    entityPropertyPage(snapshot, subroutine, {
      entity: "subroutines", id: graphId("main"), subroutine: "main",
    }, "Subroutine main"),
    entityPropertyPage(snapshot, workflow, {
      entity: "workflows", id: graphId("main"), subroutine: "main",
    }, "Workflow main"),
    entityPropertyPage(snapshot, subroutine, {
      entity: "edges", id: edgeId("profile_analyze"), subroutine: "main",
    }, "Subroutine main"),
    entityPropertyPage(snapshot, subroutine, {
      entity: "nodes", id: nodeId("optimize"), subroutine: "main",
    }, "Subroutine main"),
    entityPropertyPage(snapshot, subroutine, {
      entity: "features", id: featureId("remaining_iterations"), subroutine: "main",
    }, "Subroutine main"),
    entityPropertyPage(snapshot, subroutine, {
      entity: "profiles", id: agentProfileId("default"), subroutine: "main",
    }, "Subroutine main"),
    entityPropertyPage(snapshot, subroutine, {
      entity: "sessions", id: agentSessionId("optimize"), subroutine: "main",
    }, "Subroutine main"),
    entityPropertyPage(snapshot, subroutine, {
      entity: "nodes", id: nodeId("call_implement"), subroutine: "main",
    }, "Subroutine main"),
    entityPropertyPage(snapshot, subroutine, {
      entity: "nodes", id: nodeId("call_workflow"), subroutine: "main",
    }, "Subroutine main"),
  ];
  ...
  assert.equal(pages[0]?.entries.some(({ entity }) => entity === "nodes"), true);
  assert.deepEqual(pages[1]?.entries.map(({ entity, id }) => ({ entity, id })), [
    { entity: "nodes", id: "subroutine" },
  ]);
  assert.deepEqual(pages.slice(0, 4).map((page) => ({
  ...
  const readOnly = entityPropertyPage(snapshot, subroutine, {
    entity: "nodes", id: nodeId("optimize"), subroutine: "main",
  }, "Subroutine main");
  assert.deepEqual(
  ...
  const readOnlyEdge = entityPropertyPage(snapshot, subroutine, {
    entity: "edges", id: edgeId("profile_analyze"), subroutine: "main",
  }, "Subroutine main");
  assert.equal(readOnlyEdge?.constraintsWritable, false);
  assert.equal(pages.slice(2, 7).every((page) => page?.entries.length === 0), true);
  assert.deepEqual(pages.slice(7).map((page) =>
    ...
    [{
      ...
      removal: undefined,
      target: {
        inspection: { entity: "subroutines", id: "main__implement", subroutine: "main" },
...
test("entity property pages preserve node documents and workflow-owned settings", () => {
  ...
  const workflow = value.workflow as LocalWorkflowDefinition;
  ...
  workflow.sessions = [{ id: agentSessionId("history"), name: "History", persistent: false }];
  ...
  const scopes = snapshotCanvasGraphs(snapshot);
  const node = entityPropertyPage(snapshot, scopes.main!, {
    entity: "nodes", id: nodeId("call_implement"), subroutine: "main",
  }, "Subroutine main")!;
  ...
  const wrapper = entityPropertyPage(snapshot, scopes["workflow:main"]!, {
    entity: "workflows", id: graphId("main"), subroutine: "main", workflow: graphId("main"),
  }, "Workflow main")!;
  assert.deepEqual(wrapper.resources.map(({ label, value }) => ({ label, value })), [
    { label: "profile agent", value: "builder" },
    { label: "session conversation", value: "history" },
  ]);
  assert.equal(wrapper.resourcesWritable, true);
  const wrapperCall = entityPropertyPage(snapshot, scopes["workflow:main"]!, {
    entity: "workflows", id: graphId("main"), subroutine: "main", workflow: graphId("main"),
  }, "Workflow main", { entity: "nodes", id: nodeId("subroutine") })!;
  ...
  assert.deepEqual(wrapperCall.entries.map(({ entity, id, meta, removal, target }) => ({
    entity,
    id,
    meta,
    removal,
  ...
  const profile = entityPropertyPage(snapshot, scopes["workflow:main"]!, {
    entity: "profiles",
  ...
  assert.deepEqual(profile.documents, [
    { label: "profile declaration", path: "workflows/main/profiles/builder/__init__.py" },
    { label: "profile implementation", path: "workflows/main/profiles/builder/impl.py" },
  ]);
  const session = entityPropertyPage(snapshot, scopes["workflow:main"]!, {
    entity: "sessions",
...
test("enum properties show the selected value", () => {
  ...
  const graph = subroutineInProject(enumProject, "main") as unknown as
  ...
  graph.edges = [{
    conditions: [{ feature_id: "status", observation: "equal", value: "pending" }],
  ...
  assert.deepEqual(
    entityDetails(enumProject, "main", "edges", "profile_analyze")
      ?.constraints,
    [
      { collection: "conditions", expression: "status=pending", feature: "status" },
...
test("edge constraints use the language's feature-id notation", () => {
  assert.deepEqual([
    ...
    constraintNotation("conditions", "num_iterations", "integer", "equal_zero"),
    constraintNotation("conditions", "num_iterations", "integer", "greater_zero"),
    ...
    constraintNotation("effects", "num_iterations", "integer", "unconstrained"),
...
test("agent and call resources become compatible property choices", () => {
  ...
  if (call?.kind !== "subroutine_call") return;
...
test("resource updates are normalized against the same complete contract", () => {
  ...
  assert.deepEqual(normalizeNodeResources(snapshot, "main", "optimize", [
    { resource: "profile", value: "default" },
    { resource: "session", value: "plan" },
  ]), { kind: "agent", profile: "default", session: "plan" });

  assert.deepEqual(normalizeNodeResources(snapshot, "main", "call_implement", [
  ...
  assert.throws(() => normalizeNodeResources(snapshot, "main", "optimize", [
    { resource: "profile", value: "default" },
    { resource: "profile", value: "default" },
    { resource: "session", value: "plan" },
  ]), /more than once/);
  ...
  assert.throws(() => normalizeNodeResources(snapshot, "main", "call_implement", [
    { parameter: "worker", resource: "profile", value: "default" },
...
test("workflow boundaries use the same call fields with explicit resources", () => {
  ...
  assert.deepEqual(normalizeWorkflowResources(target, workflow, [
    ...
    session_arguments: { draft: "archive", review: "history", scratch: "temporary" },
  });
  assert.throws(() => normalizeWorkflowResources(target, workflow, [
...
test("an entity's properties come from the graph, and name is not id", () => {
  assert.deepEqual(entityDetails(project, "main", "features", "remaining_iterations"), {
    fields: [
  ...
  assert.equal(
    entityDetails(parameters, "main", "session_parameters", "conversation")?.persistent,
    undefined,
  );
  ...
  assert.notEqual(edge.name, "profile_analyze", "the label is prose, not the identifier");
  ...
  assert.deepEqual(
    entityDetails(project, "main", "edges", "enter__profile")
      ?.constraints,
    [],
  );
  ...
  const unchangedEdge = (unchangedGraph.edges as Record<string, unknown>[]).find(
  ...
  const boundCall = boundGraph.nodes.find((node) => node.id === "call_implement")!;
  boundCall.operation = {
    profile_arguments: { agent: "default" },
    session_arguments: { conversation: "plan" },
    target: "main__implement",
  };
  assert.deepEqual(entityDetails(bound, "main", "nodes", "call_implement")?.fields, [
    { label: "calls", value: "main__implement" },
  ]);
  ...
  assert.equal(
    entityDetails(external, "main_body", "workflows", "main_body__helper")?.idEditable,
    true,
  );
  assert.equal(
    entityDetails(external, "main_body", "workflows", "main_body__helper")?.nameEditable,
    true,
  );
  ...
  assert.deepEqual(wrapper.fields, [{ label: "subroutine", value: "main__worker" }]);
  assert.equal(
...
test("parameter pages resolve to the concrete resources bound through the call paths", () => {
  ...
  main.session_parameters = [{ id: agentSessionId("conversation"), name: "Conversation" }];
  ...
  implement.profile_parameters = [{ id: agentProfileId("worker"), name: "Worker" }];
  ...
  const workflow = value.workflow as LocalWorkflowDefinition;
  workflow.profile_arguments = { [agentProfileId("agent")]: agentProfileId("builder") };
  workflow.session_arguments = { [agentSessionId("conversation")]: agentSessionId("history") };
  workflow.profiles = [{
    ...testProfile("builder", "Builder", "claude"),
    options: { model: "claude-fable-5-1", reasoning_effort: "high", extra_args: [], web_search: true },
  }];
  workflow.sessions = [{ id: agentSessionId("history"), name: "History", persistent: true }];
  ...
  const chained = resolveParameter(value, graphId("main__implement"), "profile", "worker");
  assert.deepEqual(chained.map(({ owner, id }) => [owner.kind, owner.id, id]), [["workflow", "main", "builder"]]);
  assert.deepEqual(
    resolveParameter(value, graphId("main"), "session", "conversation").map(bindingSummary),
    ["workflow main → history (persistent)"],
  );
  // Bound by nobody inside the project.
  implement.session_parameters = [{ id: agentSessionId("unbound"), name: "Unbound" }];
  assert.deepEqual(resolveParameter(value, graphId("main__implement"), "session", "unbound"), []);
  ...
  const scopes = snapshotCanvasGraphs(snapshot);
  const page = entityPropertyPage(snapshot, scopes["main/implement"] ?? scopes.main!, {
    entity: "profile_parameters", id: agentProfileId("worker"), subroutine: "main__implement",
  }, "Subroutine implement")!;
  assert.deepEqual(page.fields, [
    { label: "resolves to", value: "workflow main → builder (claude · claude-fable-5-1 · high · web search)" },
  ]);
  assert.equal(page.entries.length, 1);
  const [entry] = page.entries;
  assert.equal(entry.entity, "profiles");
  assert.equal(entry.id, "builder");
  assert.deepEqual(entry.meta, ["resolves to", "claude · claude-fable-5-1 · high · web search"]);
  assert.equal(entry.declaration, "workflows/main/profiles/builder/__init__.py");
  assert.deepEqual(entry.target, {
    inspection: { entity: "profiles", id: "builder", subroutine: "main", workflow: "main" },
    scope: "workflow:main",
    selection: { entity: "nodes", id: "subroutine" },
  });

  const unbound = entityPropertyPage(snapshot, scopes.main!, {
    entity: "session_parameters", id: agentSessionId("unbound"), subroutine: "main__implement",
  }, "Subroutine implement")!;
...
test("a profile page carries the web_search option", () => {
  const value = testProject() as unknown as CanonicalProject;
  const main = subroutineInProject(value, graphId("main"))!;
  main.profiles = [{ ...testProfile("default", "Default"), options: { model: null, reasoning_effort: null, extra_args: [], web_search: true } }];
  const details = entityDetails(value, "main", "profiles", "default")!;
  assert.equal(details.profile?.options.web_search, true);
  assert.equal(entityDetails(testProject(), "main", "profiles", "default")!.profile?.options.web_search, undefined);
});
