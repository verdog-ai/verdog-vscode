...
const graph = (id: string): SubroutineDefinition => ({
  edges: [{
    conditions: [],
    effects: [],
    id: "enter__exit",
...
const project = (): CanonicalProject => {
  ...
  root.workflows.push(
    {
      subroutine: "main__worker", profiles: [], sessions: [],
...
test("the central index separates workflow envelopes from subroutine graphs", () => {
  ...
  assert.deepEqual([...index.subroutines.keys()], [
    "main",
    "main__worker",
  ...
  assert.equal(definitionIn(project(), "workflow", "main__worker")?.target, "main__worker");
  assert.equal(definitionIn(project(), "subroutine", "main__worker")?.target, "main__worker");
  assert.equal(definitionIn(project(), "workflow", "main__reviewer")?.external?.alias, "reviewer.pin");
  assert.deepEqual([...index.duplicates], []);
  assert.deepEqual(subroutinesIn(project()).map(({ id }) => id), [
    "main",
    "main__worker",
    "main__worker__nested",
    "main__worker__normalize",
...
test("the index records ambiguity without replacing the first declaration", () => {
  ...
  assert.equal(index.definitions.get("subroutine:main__normalize")?.subroutine, first);
});

test("lexical visibility includes both kind-specific namespaces", () => {
  const visible = visibleDefinitions(project(), "main__worker__nested")
    .map(({ id, kind }) => `${kind}:${id}`);
  ...
  assert.equal(
    resolveCall(project(), "main__worker", {
      ...
      operation: { profile_arguments: {}, session_arguments: {}, target: "main__worker" },
    })?.id,
    "main__worker",
...
test("source paths follow the lexical graph tree", () => {
  const value = project();
  assert.deepEqual(definitionPath(value, "workflow", "main"), ["workflows", "main"]);
  assert.deepEqual(definitionPath(value, "subroutine", "main"), ["subroutines", "main"]);
  ...
  assert.deepEqual(definitionPath(value, "subroutine", "main__worker__nested"), [
    "subroutines",
    "main",
    "subroutines",
    "worker",
