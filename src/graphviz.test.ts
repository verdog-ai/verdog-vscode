...
const edge = (id: string, source: string, target: string): SubroutineEdge => ({
  id,
  source,
  target,
});
...
test("DOT quotes identifiers and labels in one place", () => {
  ...
  assert.ok(dot.includes(String.raw`label="PYTHON\nsay \"hi\"\\then\t\u0001next  ↗"`));
...
test("DOT owns node placement and spline routing", () => {
  ...
  assert.ok(automatic.dot.includes(
    'bgcolor="transparent", rankdir="TB", ranksep=0.888889, nodesep=1.875, splines="spline", outputorder="edgesfirst"',
  ));
  assert.ok(automatic.dot.includes("fixedsize=false, width=3, height=1.1111111111111112"));
...
test("non-strict DOT retains parallel edges and self-loops without edge labels", () => {
  ...
  assert.ok(edgeLines.some((line) => line.includes(`id="${edgeDomId("first")}"`)));
  assert.ok(edgeLines.some((line) => line.includes(`id="${edgeDomId("second")}"`)));
