...
export interface CatalogueImportReceipt {
  alias: string;
  binding: string;
  commit: string;
...
export interface ReviewedCatalogueImport {
  alias: string;
  commit: string;
  into: string;
  package: string;
...
export function decodeCatalogueImportReceipt(
  stdout: string,
): CatalogueImportReceipt | undefined {
  try {
...
export function catalogueImportReceiptMatches(
  ...
  return (
    receipt.repository.toLowerCase() === reviewed.repository.toLowerCase() &&
