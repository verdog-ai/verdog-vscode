...
function world(): ProjectGraphs {
  return { entity_documents: {
    ...
    project: testProject() } as unknown as ProjectGraphs;
}
...
function edited(
  ...
  return { project: change(project as unknown as mutate.Project, id), root: at.root };
}

const nodes = (project: mutate.Project, id = SUBROUTINE) =>
  ((subroutineInProject(project, id) as unknown as Record<string, unknown>).nodes as Record<string, unknown>[]);
...
test("a pinned workflow is edited in its own project, under its own id", () => {
  ...
  assert.equal(subroutineFile("/w/mine", "main", snapshot.pinned ?? {}).root, "/w/mine");

  // Handing `mutate` the composite id is the bug, and it fails loudly rather than quietly.
  assert.throws(
    () => mutate.addNode(projectIn(snapshot, KEY).project as unknown as mutate.Project, KEY, "python", "X"),
...
test("every gesture lands in the pinned project and leaves this one alone", () => {
  ...
  const added = edited(snapshot, KEY, (project, workflow) =>
  ...
  const removed = edited(snapshot, KEY, (project, workflow) =>
    mutate.remove(project, workflow, "nodes", "prepare_optimization").project);
  assert.ok(!nodes(removed.project).some((node) => node.id === "prepare_optimization"));
  ...
  assert.equal(nodes(named.project).find((n) => n.id === "profile")!.name, "Profile it");

  const constrained = edited(snapshot, KEY, (project, workflow) =>
    mutate.constrain(project, workflow, "profile_analyze", "conditions", "remaining_iterations", "greater_zero"));
  ...
  const assigned = edited(snapshot, KEY, (project, workflow) =>
    mutate.setNodeResources({ ...snapshot, project: project as ProjectGraphs["project"] }, workflow, "plan", [
...
test("generation runs in the clone that owns the workflow, not the one that pins it", () => {
  ...
  assert.equal(
    subroutineFile("/w/mine", "drexlerd.pyverdog_lgp/main", { "drexlerd.pyverdog_lgp": {} }).root,
