...
import {
  parseTerminationReport,
  TerminationAnalysis,
} from './terminationAnalysis';
import type {TerminationReport, TerminationState} from '../model/termination';

function report(
  projects: Record<string, string> = {'': 'root'},
): TerminationReport {
  return {
    projects,
    definitions: {
      main: {
        ...
        rules: 1,
        dependencies: [],
        edges: {enter__exit: 'cleared'},
        regions: [],
      },
    },
  };
}
function outcome(body: TerminationReport): Outcome {
  return {
    code: 0,
...
test('analysis validates complete native reports once', () => {
  ...
  assert.equal(
    parseTerminationReport(
      JSON.stringify({...report(), definitions: {main: legacy}}),
    ),
    undefined,
  );
  ...
  assert.equal(
    parseTerminationReport(
      JSON.stringify({...report(), definitions: {main: withoutEdges}}),
    ),
    undefined,
  );
...
test('edge statuses and ordered cycle rows are decoded once at the native boundary', () => {
  ...
  const definition = {
    ...
    edges: {start: 'cleared', again: 'remaining', disconnected: 'unreachable'},
    regions: [region],
  };
  const decode = (main: unknown) =>
    parseTerminationReport(JSON.stringify({...report(), definitions: {main}}));
  ...
  for (const cycle of [
    ...
    assert.equal(
      decode({...definition, regions: [{...region, cycle}]}),
      undefined,
    );
  }
...
test('superseding a worker aborts it and serializes the replacement; old results cannot publish', async () => {
  ...
  const finishes: Array<(result: Outcome) => void> = [];
  let active = 0;
  let maximum = 0;
  const analysis = new TerminationAnalysis(
    async signal => {
  ...
  try {
    ...
    assert.equal(
      starts.length,
      1,
      'replacement waits for the killed process to close',
    );
...
test('refresh suspends immediately and only unchanged saved hashes restore a completed report', async () => {
  ...
  try {
    ...
    analysis.suspend();
    assert.equal(
      published.at(-1)?.status,
      'checking',
      'old highlights disappear before saved files are read',
    );
    ...
    assert.equal(
      calls,
      1,
      'metadata-only saves reuse their completed analysis',
    );
    analysis.suspend();
  ...
  const pending = new TerminationAnalysis(
    () => result,
    state => published.push(state),
    1,
  );
  try {
    ...
    assert.equal(
      ...
      'checking',
      'a finishing worker cannot publish during the next snapshot read',
    );
  } finally {
    pending.dispose();
  }
...
test('missing analyzed owners explain rejected pins without accepting a partial certificate', async () => {
  for (const [projects, reason] of [
    [
      {'': 'root'},
      /Some pinned graphs could not be analyzed.*schema, package identity, and dependency chain/,
    ],
    [{'': 'root', pin: 'different'}, /saved graphs changed/],
  ] as const) {
    const analysis = new TerminationAnalysis(
      async () => outcome(report(projects)),
      () => {},
      1,
    );
    try {
      ...
      if (analysis.state?.status === 'unavailable') {
        assert.match(analysis.state.reason, reason);
      }
    } finally {
      analysis.dispose();
    }
...
test('mismatched hashes, invalid output, and CLI or service failures are unavailable, not certificates', async () => {
  for (const result of [
    ...
    ...[
      'not installed',
      'http://127.0.0.1:8765 could not be reached',
      'not signed in; run `verdog login` first',
    ...
    try {
      ...
      if (result.code !== 0 && analysis.state?.status === 'unavailable') {
        assert.equal(
          ...
          result.stderr,
          "show the CLI's service or authentication failure",
        );
      }
    } finally {
      analysis.dispose();
...
test('anonymous analysis failure asks for a service connection, not sign-in', async () => {
  const analysis = new TerminationAnalysis(
    async () => ({code: 1, stdout: '', stderr: '', combined: ''}),
    () => {},
    1,
  );
  try {
...
test('an unavailable worker retries unchanged graphs after repair and caches only the successful report', async () => {
  ...
  try {
    ...
    assert.equal(
      calls,
      2,
      'successful unchanged graphs still reuse their completed report',
    );
  } finally {
    analysis.dispose();
  }
    ...
    current => {
      ...
      return new Promise(resolve =>
        current.addEventListener('abort', () =>
          resolve({code: 130, stdout: '', stderr: '', combined: ''}),
        ),
      );
    },
    state => published.push(state),
