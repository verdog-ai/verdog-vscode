  ...
  try {
    ...
    if (current()) {
      ...
      void vscode.window.showWarningMessage(
        `Could not open ${relative}: ${String(error)}`,
      );
    }
    return false;
  }
...
export async function deliverPendingReveal(host: HostState): Promise<void> {
  ...
  if (pending === undefined || view === undefined || !host.canvasReady) {
    return;
  }
  ...
  if (delivered && host.view === view) {
    return;
  }

  host.pendingReveal = pending;
  if (host.view !== view && host.canvasReady) {
...
export async function followNavigation(
  ...
  const graph =
    target === undefined || host.snapshot === undefined
      ? undefined
      : snapshotCanvasGraphs(host.snapshot)[target.scope];
  const selection = target?.selection;
  const body =
    graph?.scope.kind === 'subroutine' && host.snapshot !== undefined
  ...
  if (openDeclaration && entry.declaration !== undefined) {
    ...
    if (host.navigationRevision !== revision) {
      return;
    }
  }
...
export async function syncActiveEditorReadonly(host: OpenHost): Promise<void> {
  ...
  if (
    readonly === undefined ||
    (!readonly && !host.protectedDocuments.has(key))
  ) {
    return;
  }
  ...
  try {
    ...
    if (readonly) {
      host.protectedDocuments.add(key);
    } else {
      host.protectedDocuments.delete(key);
    }
  } catch (error) {
    host.output.appendLine(
      `${command} failed for ${uri.fsPath}: ${String(error)}`,
    );
  }
}
...
export async function selectWorkflowEnvironment(
  ...
  const select = async () => {
    ...
    try {
      const extension =
        vscode.extensions.getExtension<PythonExtension>('ms-python.python');
  ...
  host.workflowEnvironmentSelection = host.workflowEnvironmentSelection.then(
    select,
    select,
  );
...
export async function editSubroutine(
  ...
  try {
    ...
    void vscode.window.showWarningMessage(
      `Verdog: ${String(error).replace(/^Error: /, '')}`,
    );
    return false;
  }
  ...
  const authoredUri = (relative: string): vscode.Uri => {
    ...
    if (resolved === undefined) {
      throw new Error(`authored path was not preflighted: ${relative}`);
    }
  ...
  const dirtyWithin = (relative: string): vscode.TextDocument | undefined => {
    ...
    return vscode.workspace.textDocuments.find(candidate => {
      if (!candidate.isDirty || candidate.uri.scheme !== 'file') {
        return false;
      }
  ...
  for (const edit of next.files ?? []) {
    if (edit.kind === 'delete' && stopIfDirty(edit.path)) {
      return false;
    }
  }
  ...
  for (const edit of next.files ?? []) {
    ...
    try {
      await vscode.workspace.fs.delete(authoredUri(edit.path), {
        recursive: true,
        useTrash: true,
      });
  ...
  for (const edit of next.files ?? []) {
    if (edit.kind === 'rename') {
      edited.renameFile(authoredUri(edit.from), authoredUri(edit.to), {
        overwrite: false,
      });
    }
  }
  ...
  if (graphHash(before) !== graphHash(next.project)) {
    await generate(host, at.root);
  }
  await refresh(host);
  for (const edit of next.files ?? []) {
    ...
    if (edit.kind === 'rename') {
      host.output.appendLine(`moved ${edit.from} → ${edit.to}`);
    }
  }
  host.output.appendLine(
    `${describe}${next.report ? ` (${next.report})` : ''}`,
  );
  return true;
}
...
export async function readAccess(host: OpenHost): Promise<void> {
  ...
  if (result.code !== 0) {
    host.output.appendLine(
      `Catalogue permissions could not be read: ${result.combined.trim()}`,
    );
    return;
  }
  try {
    ...
    if (
      typeof answer.accessible !== 'boolean' ||
      typeof answer.repository !== 'string'
    ) {
    ...
    host.output.appendLine(
      'verdog access returned an unsupported catalogue permission response.',
    );
  }
}
...
export async function chooseSubroutine(
  host: HostState,
): Promise<GraphId | undefined> {
...
export async function chooseSubroutineIn(
  root: string,
): Promise<GraphId | undefined> {
  let found: GraphId[];
  try {
    ...
    void vscode.window.showWarningMessage(
      `${root} could not be read: ${String(error)}`,
    );
    return undefined;
  }
  if (found.length <= 1) {
  ...
  return vscode.window.showQuickPick(found, {
    title: `Which subroutine in ${path.basename(root)}?`,
  });
}
...
export function projectOf(
  ...
  return projectIn(host.snapshot, subroutineId).project;
}
...
export function entities(
  ...
  const found =
    host.snapshot === undefined
      ? undefined
      : subroutineIn(host.snapshot, subroutine)?.[kind];
...
export async function refresh(
  ...
  host.termination ??= new TerminationAnalysis(
    signal =>
      runVerdogCommand(host.root, ['analyze', '--json'], {
        announce: false,
        output: host.output,
        signal,
        streamOutput: false,
  ...
  if (host.preview !== undefined && host.preview.workflow) {
    const definition = definitionIn(
      host.snapshot.project,
      'workflow',
...
function publish(
  ...
  for (const [file, entries] of byFile) {
    host.problems.set(vscode.Uri.file(file), entries);
  }
}
...
export async function runVerb(
  ...
  const result = await runVerdogCommand(
    root,
    structured ? [verb, '--json'] : [verb, ...args],
    {
      output: host.output,
      progress: {
