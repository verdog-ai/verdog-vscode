...
test("file protection follows manifest ownership", () => {
  ...
  assert.equal(projectFileReadonly(root, inside("src", "demo", "__init__.py"), snapshot), true);
  assert.equal(projectFileReadonly(root, inside("src", "demo", "authored.py"), snapshot), false);
  assert.equal(projectFileReadonly(root, inside("project.json"), snapshot), false);
  assert.equal(projectFileReadonly(root, inside("src", "notes.py"), snapshot), false);
  assert.equal(projectFileReadonly(root, inside("external", "child", "tools", "project.json"), snapshot), false);
  assert.equal(
    projectFileReadonly(root, inside("external", "child", "tools", "src", "child", "__init__.py"), snapshot),
    true,
  );
  assert.equal(
    projectFileReadonly(root, inside("external", "child", "tools", "src", "child", "authored.py"), snapshot),
    false,
  );
  assert.equal(
    projectFileReadonly(
  ...
  assert.equal(projectFileReadonly(root, inside("external_tools", "notes.py"), snapshot), false);
  assert.equal(
    projectFileReadonly(root, path.resolve(root, "..", "other", "__init__.py"), snapshot),
    undefined,
  );
});

test("an old clone reports its schema as unsupported", async (context) => {
  const root = await fs.mkdtemp(path.join(tmpdir(), "verdog-vscode-old-schema-"));
  context.after(() => fs.rm(root, { force: true, recursive: true }));
  await fs.writeFile(path.join(root, "project.json"), JSON.stringify({ schema_version: 12 }));
...
test("partial graph edits still expose the available identities", async (context) => {
  ...
  for (const subroutine of [
    ...
    await fs.writeFile(path.join(root, "project.json"), JSON.stringify({
      schema_version: SCHEMA_VERSION, package: "demo", subroutine,
    }));
    ...
    if (subroutine !== undefined) {
      ...
      assert.ok(entityPropertyPage(snapshot, graph, target.inspection, "Subroutine main"));
      assert.deepEqual(nodeResourceFields(snapshot, "main", "unfinished"), []);
      for (const category of ["subroutines", "workflows", "profiles", "sessions", "features", "status"] as const) {
...
test("entity documents follow their lexical source paths", async (context) => {
  const root = await fs.mkdtemp(path.join(tmpdir(), "verdog-vscode-documents-"));
  context.after(() => fs.rm(root, { force: true, recursive: true }));
  ...
  subroutine.edges.push({
    conditions: [], effects: [], id: "enter_update", name: "Enter update",
    source: "enter", target: "update_status",
  });
  subroutine.edges.push({
    conditions: [], effects: [], id: "update_exit", name: "Update exit",
    source: "update_status", target: "exit",
  });
  ...
  subroutine.features.push({
    description: "Current status", id: "status", kind: "boolean", label: "Status",
  });
  subroutine.profile_parameters.push({ id: "agent", name: "Agent" });
  subroutine.profiles.push(testProfile("local", "Local"));
  subroutine.session_parameters.push({ id: "conversation", name: "Conversation" });
...
test("a verdict the CLI never produced is absent rather than empty", () => {
  ...
  assert.equal(parseVerdict("verdog could not be started: ENOENT", "/clone"), undefined);
});
...
test("a pinned dependency's workflows are drawable, namespaced, and owner-qualified", () => {
  ...
  const subroutine = (id: string) => ({
    edges: [{ conditions: [], effects: [], id: "e", name: "e", source: "a", target: "b" }],
    features: [],
    profile_parameters: [],
    profiles: [],
    session_parameters: [],
...
test("an edit to a pinned workflow is aimed at the submodule, not at your own graph", () => {
  ...
  const pinned = { "publisher.level_one": {}, "publisher.level_one/publisher.demo": {} };
  assert.deepEqual(subroutineFile("/w/level2", "publisher.level_one/main", pinned), {
  ...
  assert.deepEqual(
    subroutineFile("/w/level2", "publisher.level_one/publisher.demo/main", pinned).root,
    path.join(
      "/w/level2", "external", "publisher", "level_one", "external", "publisher", "demo",
    ),
  );
});
...
test("a clone follows each owner's direct aliases without flattening packages", async (context) => {
  ...
  const subroutine = (nodes: Record<string, unknown>[] = []) => ({
    edges: [],
    features: [],
    profile_parameters: [],
    profiles: [],
  ...
  const project = (
    ...
    rootSubroutine.workflows = binding === undefined
      ? []
      : [{
          id: binding.id,
          name: binding.id,
  ...
  const childDeclaration = "src/publisher/demo/subroutines/main_body/nodes/run/__init__.py";
  await fs.mkdir(demoOne, { recursive: true });
  await Promise.all([
    fs.writeFile(
      path.join(root, "project.json"),
      JSON.stringify(project("owner.root", [
        { alias: "publisher.level_one", package: "publisher.level_one" },
      ], { alias: "publisher.level_one", id: "level_one" }, [call("main_body__level_one", "call_level_one")], [
        { path: "src/owner/root/workflows/main_body/requirements.txt" },
      ])),
    ),
    fs.writeFile(
      path.join(levelOne, "project.json"),
      JSON.stringify(project("publisher.level_one", [
  ...
  assert.equal(
    snapshot.entity_documents.nodes["publisher.level_one/publisher.demo/main_body"].run.declaration,
  ...
  const graphs = snapshotSubroutines(snapshot as unknown as Parameters<typeof snapshotSubroutines>[0]);
  ...
  assert.deepEqual(
    graphs["publisher.level_one/main_body"].nodes.find((node) => node.id === "call_demo_one")?.data.definition,
    {
  ...
  assert.deepEqual(
    ...
    { alias: "publisher.demo", graph: "main_body", ownerPath: "publisher.level_one" },
  );
  assert.equal(
    graphs["publisher.level_one/main_body"].nodes.find(
      (node) => node.id === "call_demo_subroutine",
  ...
  assert.equal(subroutineCallTarget(snapshot, "main_body", "main_body")?.id, "main_body");
  assert.equal(
  ...
  assert.equal(
    subroutineCallTarget(snapshot, "publisher.level_one/main_body", "publisher.demo/nested/main"),
    undefined,
  );
  const definitions = snapshotDefinitions(snapshot);
  const rootVisibility = visibleDefinitionKeys(snapshot, "main_body");
  assert.equal(rootVisibility.has("publisher.level_one/subroutine:main_body"), true);
  assert.equal(
    rootVisibility.has("publisher.level_one/publisher.demo/subroutine:main_body"),
    false,
  );
  assert.equal(
    visibleDefinitionKeys(snapshot, "publisher.level_one/main_body")
      .has("publisher.level_one/publisher.demo/subroutine:main_body"),
    true,
  );
  assert.deepEqual(
    definitions.find(({ key }) => key === "workflow:main_body")?.documents,
    [{ label: "requirements", path: "src/owner/root/workflows/main_body/requirements.txt" }],
  );
  assert.deepEqual(
    definitions.find(({ key }) => key === "publisher.level_one/workflow:main_body")?.documents,
    [{
      label: "requirements",
      path: "external/publisher/level_one/src/publisher/level_one/workflows/main_body/requirements.txt",
    }],
  );
  assert.deepEqual(definitions.find(({ key }) => key === "workflow:main_body__level_one"),
    {
  ...
  assert.deepEqual(definitions.find(({ key }) => key === "publisher.level_one/workflow:main_body__demo_one"),
    {
  ...
  assert.deepEqual(
    withoutNestedPin.find(({ key }) => key === "publisher.level_one/workflow:main_body__demo_one"),
    {
      declaredIn: "publisher.level_one/main_body",
      documents: [],
      id: "main_body__demo_one",
  ...
  assert.equal(
    subroutineFile(root, "publisher.level_one/publisher.demo/main_body", snapshot.pinned).root,
    demoOne,
  );
});
...
test("a clone does not follow an invalid alias outside its root", async (context) => {
  ...
  const outside = await fs.mkdtemp(path.join(tmpdir(), "verdog-vscode-outside-"));
  context.after(() => fs.rm(root, { force: true, recursive: true }));
  context.after(() => fs.rm(outside, { force: true, recursive: true }));

  const project = (externals: Record<string, unknown>[]) => ({
    externals,
    generated_from: "0".repeat(64),
    package: "owner.root",
    schema_version: SCHEMA_VERSION,
    sources: [],
    subroutine: {
      edges: [], features: [], id: "main_body", name: "Main body", nodes: [],
      profile_parameters: [], profiles: [], session_parameters: [], sessions: [],
      ports: { enter: "enter", exit: "exit", failure: "failure" }, subroutines: [], workflows: [],
    },
    workflow: {
      subroutine: "main_body", profiles: [], sessions: [],
      profile_arguments: {}, session_arguments: {},
  ...
  await Promise.all([
    ...
    fs.writeFile(path.join(outside, "project.json"), JSON.stringify(project([]))),
  ]);
...
test("a clone does not follow a symlinked external checkout", async (context) => {
  ...
  try {
    await fs.symlink(outside, path.join(root, "external", "owner", "borrowed"), "dir");
  } catch {
  ...
  await Promise.all([
    fs.writeFile(
      path.join(root, "project.json"),
      JSON.stringify(project([{ alias: "owner.borrowed", package: "owner.outside" }])),
    ),
    fs.writeFile(path.join(outside, "project.json"), JSON.stringify(project([]))),
  ]);
...
test("entity documents have readable stable labels", () => {
  ...
  assert.deepEqual(
    documentsOf({ declaration: "profiles/agent/__init__.py" }, "profile_parameters"),
    [{ label: "profile parameter declaration", path: "profiles/agent/__init__.py" }],
  );
});
  ...
  const snapshot = {
    ...
    pinned: {
      "local.tools": {
        package: "publisher.tools",
        subroutine: {
          id: "main_body", name: "Theirs", nodes: [], edges: [], workflows: [], subroutines: [],
        },
        workflow: {
          subroutine: "main_body", profiles: [], sessions: [],
          profile_arguments: {}, session_arguments: {},
    ...
    project: {
      subroutine: {
        id: "main_body", name: "Ours", nodes: [], edges: [], workflows: [], subroutines: [],
      },
      workflow: {
        subroutine: "main_body", profiles: [], sessions: [],
        profile_arguments: {}, session_arguments: {},
