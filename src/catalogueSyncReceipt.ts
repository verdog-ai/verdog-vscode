...
export function catalogueSyncPaths(
  ...
  const environment = paths.resolve(checkout, ".verdog", "environments", workflow);
  return {
    environment,
...
export function decodeCatalogueSyncReceipt(
  ...
  try {
    ...
    const paths = catalogueSyncPaths(expected.checkout, expected.workflow, platform);
    if (
