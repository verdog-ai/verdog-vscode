...
/**
 * @fileoverview Completion for `project.json`, so the graph can be written without memorising it.
 *
...
function observationItems(
  ...
  return observationsFor(collection, feature?.kind).map(value => {
    ...
    item.detail =
      feature === undefined ? 'observation' : `${feature.kind} feature`;
...
function definitionItems(
  ...
  return documentDefinitions(text, subroutine, callKind, external).map(
    definition => {
      const item = new vscode.CompletionItem(
        definition.id,
    ...
    {language: 'json', pattern: '**/project.json', scheme: 'file'},
    {
      async provideCompletionItems(document, position) {
        ...
        if (context.property === 'observation') {
          if (
            context.collection !== 'conditions' &&
            context.collection !== 'effects'
          ) {
            return undefined;
          }
        ...
        if (context.property === 'target') {
          ...
          if (
            context.callKind === 'subroutine_call' &&
            context.subroutine !== undefined
          ) {
            try {
              const snapshot = await readClone(
                path.dirname(document.uri.fsPath),
              );
              external = externalSubroutineTargets(
                snapshot,
                context.subroutine,
              );
            } catch {
