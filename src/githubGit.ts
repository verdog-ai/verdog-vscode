...
/**
 * @fileoverview Run one GitHub network operation with the user's configured Git transport first.
 *
...
export interface GitHubGitAttempt {
  outcome: Outcome;
  transport: 'configured' | 'github-ssh';
}

export interface GitHubGitResult {
  attempts: readonly GitHubGitAttempt[];
  outcome: Outcome;
  usedSshFallback: boolean;
}

export type GitRunner = (args: string[]) => Promise<Outcome>;
...
/** Run `args`, retrying an ordinary Git failure with a command-local GitHub SSH rewrite. */
export async function runGitHubGit(
  ...
  const ssh = await run(['-c', GITHUB_SSH_URL_REWRITE, ...args]);
  return result(
    ssh,
...
export function formatGitHubGitFailure(gitResult: GitHubGitResult): string {
  if (gitResult.outcome.code === 0) {
    return '';
  }
  return gitResult.attempts.map(attemptFailure).join('\n\n');
}
