...
test("an import receipt agrees with the exact reviewed release and its derived paths", () => {
  ...
  const collision = decodeCatalogueImportReceipt(envelope({ binding: "main__review__main_27" }));
...
test("partial or non-matching import receipts never authorize a success claim", () => {
  assert.equal(decodeCatalogueImportReceipt(envelope({ binding: undefined })), undefined);
  assert.equal(decodeCatalogueImportReceipt(envelope({ generated_status: -1 })), undefined);
