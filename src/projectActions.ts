...
import { definitionComponentEntries, definitionTarget, removalNavigationEntry } from "../webview/navigation";
...
const DEFINITION_KIND_CHOICES = DEFINITION_KINDS.map((definitionKind) => ({
  definitionKind,
  description: definitionKind === "workflow"
...
const parseEnumValues = (value: string): string[] =>
  value.split(",").map((member) => member.trim()).filter(Boolean);

const sessionDescription = (session: AgentSession): string =>
  session.origin === "parameter"
    ? "parameter"
...
async function createEntity(
  ...
  describe: string,
  create: (project: mutate.Project, at: GraphId) => {
    files?: mutate.AuthoredFileEdit[]; id: string; project: mutate.Project;
  },
  navigationRevision: number,
  workflow?: GraphId,
): Promise<void> {
  let id: string | undefined;
  const saved = await editSubroutine(host, subroutine, describe, (project, at) => {
    const result = create(project, at);
    id = result.id;
    return { ...result, report: result.id };
  });
  if (!saved || id === undefined || host.navigationRevision !== navigationRevision || host.snapshot === undefined) return;
  const snapshot = host.snapshot;
  const { ownerPath } = subroutineAddress(snapshot.pinned, subroutine);
  const scope = workflow === undefined
    ? subroutine : qualifiedKey(ownerPath, definitionKey("workflow", workflow));
  ...
  const target = definitionComponentEntries(snapshot, current, inspection)
    .find((entry) => entry.entity === entity && entry.id === id)?.target;
  if (target !== undefined) await followNavigationTarget(host, target);
}

function enumValuesProblem(value: string): string | undefined {
  const members = parseEnumValues(value);
  if (members.length === 0) return "Enter at least one value.";
  const problem = members.map(identifierProblem).find((item) => item !== undefined);
  if (problem !== undefined) return problem;
  return new Set(members).size === members.length ? undefined : "Each value must be unique.";
}
...
async function addNodeHere(
  ...
  if (subroutine === undefined) return;
  const kind =
    given?.kind ??
    (await vscode.window.showQuickPick(NODE_KIND_CHOICES, { title: "What kind of node?" }))?.label;
  if (!isExecutableNodeKind(kind)) return;
  const name =
    given?.name ??
  ...
  const detail = kind === "agent" ? await agentAssignment(host, subroutine, given) : undefined;
  if (kind === "agent" && detail === undefined) return;
  await createEntity(host, subroutine, "nodes", `Added ${kind} node "${name.trim()}"`,
    (project, at) => mutate.addNode(project, at, kind, name, undefined, detail), navigationRevision);
}
...
async function agentAssignment(
  ...
  const graph = host.snapshot === undefined ? undefined : subroutineIn(host.snapshot, subroutine);
  ...
  const selectedProfile = given?.profile === undefined
  ...
  if (selectedProfile === undefined || !profiles.some(({ id }) => id === selectedProfile)) {
    return undefined;
  }
  const selectedSession = given?.session === undefined
    ? (await vscode.window.showQuickPick(
  ...
  return selectedSession !== undefined && sessions.some(({ id }) => id === selectedSession)
...
async function callArguments(
  ...
  const snapshot = host.snapshot;
  const graph = snapshot === undefined ? undefined : subroutineIn(snapshot, caller);
  const child = snapshot === undefined ? undefined : subroutineCallTarget(snapshot, caller, target);
  if (graph === undefined || child === undefined) {
    void vscode.window.showInformationMessage(`Cannot inspect subroutine ${target}.`);
    return undefined;
  }
  ...
  return { profile_arguments: profileArguments, session_arguments: sessionArguments };
}
...
async function addDefinitionHere(
  ...
  const kind = given?.kind ?? (await vscode.window.showQuickPick(
    DEFINITION_KIND_CHOICES,
    { title: "What should be added?" },
  ))?.definitionKind;
  if (kind !== "workflow" && kind !== "subroutine") return;
  if (kind === "workflow") {
    const at = locate(host, parent);
    const project = projectOf(host, parent);
    const available = visibleDefinitions(project, at.subroutine, "subroutine")
      .filter((definition) =>
        !definitionIndex(project).definitions.has(
          definitionKey("workflow", qualifyGraph(at.subroutine, graphLeaf(definition.id))),
        )
      );
    ...
    const target = given?.target ?? (await vscode.window.showQuickPick(
    ...
    if (target === undefined || !available.some((definition) => definition.id === target)) return;
    await createEntity(host, parent, "workflows", `Added workflow definition for "${target}"`,
      (next, owner) => mutate.addWorkflowDefinition(next, owner, target), navigationRevision);
    return;
  }
  ...
  if (name === undefined || !name.trim()) return;
  await createEntity(host, parent, "subroutines", `Added subroutine definition "${name.trim()}"`,
    (project, at) => mutate.addSubroutineDefinition(project, name, at), navigationRevision);
}
...
async function addCallHere(
  ...
  const target = given?.target ?? (await vscode.window.showQuickPick(
    targets.map((definition) => ({
      description: definition.externalAlias !== undefined
  ...
  if (name === undefined || !name.trim()) return;
  const arguments_ = kind === "subroutine_call"
    ? await callArguments(host, subroutine, target)
    : undefined;
  if (kind === "subroutine_call" && arguments_ === undefined) return;
  await createEntity(host, subroutine, "nodes", `Added ${kind} "${name.trim()}"`,
    (project, local) => mutate.addCall(project, local, kind, name, target, undefined, arguments_), navigationRevision);
}

async function addFeatureHere(
  host: OpenHost,
  subroutineId?: string,
  given?: { description?: string; kind?: string; label?: string; values?: string[] },
): Promise<void> {
  const navigationRevision = host.navigationRevision;
  const subroutine = subroutineId ?? (await chooseSubroutine(host));
  if (subroutine === undefined) return;
  const kind =
    given?.kind ??
    (await vscode.window.showQuickPick(FEATURE_KIND_CHOICES, { title: "What kind of feature?" }))
      ?.label;
  if (!isFeatureKind(kind)) return;
  const label =
    given?.label ??
    (await vscode.window.showInputBox({
  ...
  const description =
    given?.description ??
    (await vscode.window.showInputBox({
      prompt: "What does it mean? An agent reads this, and the compiler requires it.",
      title: label.trim(),
    }));
  ...
  await createEntity(host, subroutine, "features", `Added ${kind} feature "${label.trim()}"`,
    (project, at) => mutate.addFeature(project, at, kind, label, description, values), navigationRevision);
}

async function resourceParameter(given?: boolean): Promise<boolean | undefined> {
  if (given !== undefined) return given;
  return (await vscode.window.showQuickPick(
    [
      { description: "owned by this subroutine", label: "Local definition", parameter: false },
      { description: "supplied by the caller", label: "Parameter", parameter: true },
    ],
...
async function addProfileHere(
  ...
  const subroutine = subroutineId ?? (await chooseSubroutine(host));
  if (subroutine === undefined) return;
  const parameter = workflow === undefined ? await resourceParameter(given?.parameter) : false;
  if (parameter === undefined) return;
  const name = given?.name ?? await vscode.window.showInputBox({
    prompt: "What is this profile called?",
    title: workflow !== undefined
      ? "New workflow profile"
      : parameter ? "New profile parameter" : "New local profile",
  });
  if (name === undefined || !name.trim()) return;
  await createEntity(host, subroutine, parameter ? "profile_parameters" : "profiles", `Added profile ${name.trim()}`,
    (project, at) => workflow === undefined
      ? mutate.addAgentProfile(project, at, name, parameter)
      : mutate.addWorkflowProfile(project, workflow, name), navigationRevision, workflow);
}
...
async function addSessionHere(
  ...
  const name = given?.name ?? await vscode.window.showInputBox({
    ...
    title: workflow !== undefined
      ...
      : parameter ? "New session parameter" : "New local session",
  });
  if (name === undefined || !name.trim()) return;
  const persistent = parameter
    ? undefined
    : given?.persistent ?? (await vscode.window.showQuickPick(
        [
          { description: "kept across invocations", label: "Persistent", value: true },
          { description: "starts fresh for each invocation", label: "Fresh", value: false },
        ],
        { title: "How long does this session live?" },
      ))?.value;
  if (!parameter && persistent === undefined) return;
  if (workflow !== undefined && persistent === undefined) return;
  await createEntity(host, subroutine, parameter ? "session_parameters" : "sessions", `Added session ${name.trim()}`,
    (project, at) => workflow === undefined
      ? mutate.addAgentSession(project, at, name, parameter, persistent)
      : mutate.addWorkflowSession(project, workflow, name, persistent!), navigationRevision, workflow);
}
...
async function chooseEnds(
  ...
  const sources = nodes.filter(({ kind }) => kind !== "exit" && kind !== "failure");
  if (sources.length === 0) {
    void vscode.window.showInformationMessage(
  ...
  const sourceId = given?.source ?? (await vscode.window.showQuickPick(
    sources.map(({ id }) => id),
...
async function chooseEdge(host: OpenHost, subroutine: string): Promise<string | undefined> {
  const items = entities(host, subroutine, "edges").map((edge) => ({
    description: `${String(edge.source)} → ${String(edge.target)}`,
    label: String(edge.id),
  }));
  if (items.length === 0) {
    void vscode.window.showInformationMessage("This subroutine has no edges to constrain.");
    return undefined;
  }
  return (await vscode.window.showQuickPick(items, { title: "Which edge?" }))?.label;
}
...
async function connectNodes(
  ...
  await createEntity(host, pair.subroutine, "edges", `Connected ${pair.source} to ${pair.target}`,
    (project, at) => mutate.connect(project, at, pair.source, pair.target), navigationRevision);
}
...
async function chooseRelinkEndpoint(
  ...
  const fixed = nodes.find((node) => node.id === edge[endpoint === "source" ? "target" : "source"]);
  if (fixed === undefined) return;
  const choices = nodes.filter((node) => edgeDirectionProblem(
  ...
  const chosen = await vscode.window.showQuickPick(choices, { title: `Change ${endpoint} of ${id}` });
  if (chosen === undefined || chosen.label === edge[endpoint]) return;
  await relinkEdge(host, subroutine, id,
...
function subroutineFeatures(host: OpenHost, subroutineId: string): Feature[] {
  return featuresIn(projectOf(host, subroutineId), locate(host, subroutineId).subroutine);
}
...
async function clearConstraint(
  ...
  return editSubroutine(host, subroutineId, `Cleared ${featureId} on ${edgeId}`, (project, at) => ({
...
async function constrainEdge(
  ...
  const edge = entities(host, subroutineId, "edges").find((item) => item.id === edgeId);
  const source = entities(host, subroutineId, "nodes").find((item) => item.id === edge?.source);
  let collection: "conditions" | "effects" = "conditions";
  if (source?.kind === "feature") {
    const side = await vscode.window.showQuickPick(
      [
        { description: "what must hold for this edge to be taken", label: "conditions" },
        { description: "what must hold between pre-node and candidate states", label: "effects" },
      ] as const,
...
async function renameEntity(
  ...
  const kind = entity === "subroutines" || entity === "workflows"
  ...
  const outcome = await runVerdogCommand(at.root, arguments_, {
    output: host.output,
    progress: {
...
async function confirmRemoval(
  ...
  const snapshot = host.snapshot;
  const links = snapshot === undefined
    ? []
  ...
  return await new Promise<boolean>((resolve) => {
    ...
    const settle = (approved: boolean, dispose: boolean): void => {
      if (settled) return;
      settled = true;
      if (activeRemovalReview === panel) activeRemovalReview = undefined;
    ...
    panel.webview.onDidReceiveMessage((message: RemovalReviewToHost) => {
      const kind = typeof message === "object" && message !== null ? message.kind : undefined;
      if (kind === "ready") {
        ...
        if (entry !== undefined) void followNavigation(host, entry, kind === "open");
      }
    });
    panel.webview.html = webviewHtml(panel.webview, host.extension, "removalReview");
  });
}
      ...
      if (
        ...
        if (dry.newImplementation === undefined) {
          const answer = await vscode.window.showWarningMessage(
            ...
            {
              detail: `${dry.oldImplementation} no longer belongs to the edge and will be deleted. ` +
          ...
          if (answer !== "Delete implementation and relink") return undefined;
          implementation = "fresh";
        } else {
          const answer = await vscode.window.showWarningMessage(
            `Relink ${edge} to ${target}?`,
            {
              detail: `Its implementation moves from ${dry.oldImplementation} to ` +
          ...
          if (answer === "Move implementation and relink") implementation = "move";
          else if (answer === "Start fresh and delete old") implementation = "fresh";
          else return undefined;
        }
      }
      const result = mutate.relink(project, at, edge, source, target, implementation);
      return { files: result.files, project: result.project };
    },
  );
}

async function undoImports(host: OpenHost, root: string, aliases: string[]): Promise<void> {
...
async function removeEntity(
  ...
  if (removalInProgress) {
    ...
    void vscode.window.showInformationMessage("Finish the current deletion first.");
    return;
  }
  ...
  try {
    ...
    const edited = await editSubroutine(
      ...
      async (project, at) => {
        ...
        const deleted = result.impacts.filter(({ effect }) => effect === "delete").length;
        return {
...
async function deleteViaPalette(host: OpenHost): Promise<void> {
  ...
  if (subroutine === undefined) return;
  const graph = host.snapshot === undefined
    ? undefined
    : subroutineIn(host.snapshot, subroutine) as unknown as Record<string, unknown> | undefined;
  ...
  const choices: { description: string; entity: mutate.Entity; label: string }[] = [];
  for (const entity of [
    "nodes",
    "workflows",
    "subroutines",
    ...
    for (const raw of Array.isArray(graph[entity]) ? (graph[entity] as unknown[]) : []) {
      const item = raw as Record<string, unknown>;
      const localId = entity === "workflows"
        ? workflowId(item)
        : typeof item.id === "string"
          ? item.id
          : undefined;
      const definition =
        localId !== undefined && (entity === "workflows" || entity === "subroutines")
      ...
      if (entity === "nodes" && ports.has(id)) continue;
      choices.push({
        description:
          entity === "edges"
...
export async function handleCanvasAction(
  ...
  switch (message.kind) {
    ...
    case "add-feature":
      ...
      return;
    case "add-profile":
      await addProfileHere(host, message.subroutine, undefined, message.workflow);
      return;
    case "add-session":
      await addSessionHere(host, message.subroutine, undefined, message.workflow);
      return;
    case "connect":
    ...
    case "relink":
      await relinkEdge(host, message.subroutine, message.edge, message.source, message.target);
      return;
    case "relink-endpoint":
      await chooseRelinkEndpoint(host, message.subroutine, message.edge, message.endpoint);
      return;
    ...
    case "set-node-resources": {
      ...
      if (snapshot === undefined) {
        void vscode.window.showWarningMessage("Verdog: the project is not ready.");
        return;
      }
      return editSubroutine(
        host,
        ...
        (project) => {
          const { ownerPath } = subroutineAddress(snapshot.pinned, message.subroutine);
          ...
          return {
            project: mutate.setNodeResources(fresh, message.subroutine, message.node, message.resources),
          };
        },
      );
    }
    case "set-workflow-resources": {
      return editSubroutine(
        ...
        (project) => ({
          project: mutate.setWorkflowResources(project, message.workflow, message.resources),
        }),
      );
    }
    case "name":
    ...
    case "rename":
      return renameEntity(host, message.subroutine, message.entity, message.id, message.to);
    case "remove": {
      await removeEntity(
        host,
        message.subroutine,
...
export function registerProjectCommands(host: OpenHost): vscode.Disposable[] {
  return [
    ...
    vscode.commands.registerCommand("verdog.check", () =>
      runVerb(host, host.showingSubroutine === undefined ? host.root : locate(host, host.showingSubroutine).root, "check"),
    ),
    vscode.commands.registerCommand("verdog.run", () =>
      runVerb(host, host.showingSubroutine === undefined ? host.root : locate(host, host.showingSubroutine).root, "run"),
    ),
    ...
    vscode.commands.registerCommand(
      ...
      (given?: { kind?: string; name?: string; subroutine?: string; target?: string }) =>
        addDefinitionHere(host, given?.subroutine, given),
    ),
    vscode.commands.registerCommand(
      "verdog.addCall",
      (given?: { kind?: string; name?: string; target?: string; subroutine?: string }) =>
    ...
    vscode.commands.registerCommand(
      "verdog.deleteEntity",
      async (given?: { entity?: mutate.Entity; id?: string; subroutine?: string }) => {
    ...
    vscode.commands.registerCommand(
      "verdog.connect",
      (given?: { source?: string; target?: string; subroutine?: string }) => connectNodes(host, given),
    ),
    vscode.commands.registerCommand(
      "verdog.constrain",
      async (given?: {
        ...
        if (edge !== undefined) await constrainEdge(host, subroutine, edge);
      },
    ),
