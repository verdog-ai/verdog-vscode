...
async function graphActions() {
  ...
  const picks: unknown[] = [];
  const inputs: string[] = [];
  const prompts: Array<{choices: unknown[]; title: string}> = [];
  ...
  let edits = 0;
  const {handleCanvasAction} = await load<typeof import('./projectActions')>(
    'projectActions.ts',
    {
      vscode: {
        window: {
      ...
      './projectHost': {
        chooseSubroutine: () => 'main',
        cliCommand: () => ['verdog'],
        editSubroutine: async (
          _: OpenHost,
          subroutine: string,
          _describe: string,
          ...
          const result = await change(at.project, at.id);
          ...
          const project =
            result.project as unknown as ProjectSnapshot['project'];
          state.snapshot =
            ownerPath === undefined
              ? {...state.snapshot!, project}
              : {
                  ...state.snapshot!,
                  pinned: {...state.snapshot!.pinned, [ownerPath]: project},
                };
        ...
        followNavigation() {},
        followNavigationTarget: (_: OpenHost, target: NavigationTarget) =>
          reveals.push(target),
        projectOf: (_: OpenHost, subroutine: string) =>
          projectIn(state.snapshot!, subroutine).project,
        isEditable: () => state.snapshot!.editable,
        locate: (_: OpenHost, subroutine: string) => ({
          root: state.root,
          subroutine: projectIn(state.snapshot!, subroutine).id,
        }),
        readAccess() {},
        refresh() {},
        runVerb() {},
  ...
  return {
    ...
    reveals,
    warnings,
    run: (action: CanvasAction, subroutine = 'main') =>
      handleCanvasAction(state, {...action, subroutine}),
    failSave: () => {
      saved = false;
    },
    navigateDuringSave: () => {
      navigateDuringSave = true;
    },
...
test('selected-node connection asks only for a valid target and reveals the resulting edge', async () => {
  ...
  assert.deepEqual(
    run.prompts.map(({title}) => title),
    ['From profile to which node?'],
  );
...
test('endpoint pickers filter port directions and preserve relink implementation confirmation', async () => {
  ...
  await run.run({
    ...
    edge: edgeId('enter__profile'),
    endpoint: 'source',
  });
  const labels = run.prompts[0].choices.map(
  ...
  assert.equal(
    run.editCount(),
    0,
    'cancelling implementation move must leave the edge untouched',
  );
  run.picks.push({label: 'prepare_optimization'});
  ...
  assert.equal(run.editCount(), 1);
  assert.ok(
    subroutineIn(run.state.snapshot!, 'main')!.edges.some(
      edge =>
        edge.source === 'prepare_optimization' && edge.target === 'profile',
    ),
  );
  ...
  assert.ok(
    !target.prompts[0].choices.some(
      item => (item as {label: string}).label === 'enter',
    ),
  );
});
...
test('creation reveals nodes, definitions, features, and workflow or parameter resources', async () => {
  const cases: Array<{
    ...
    {
      action: {kind: 'add-definition'},
      picks: [{definitionKind: 'subroutine'}],
      inputs: ['Created'],
      entity: 'subroutines',
      scope: 'main__created',
    },
    {
      action: {kind: 'add-call'},
      picks: [{label: 'subroutine_call'}, {label: 'main__implement'}],
      inputs: ['Created'],
      entity: 'nodes',
      scope: 'main',
    },
    {
      action: {kind: 'add-feature'},
      picks: [{label: 'boolean'}],
      inputs: ['Created', 'An explicit flag'],
      entity: 'features',
      scope: 'main',
    },
    {
      action: {kind: 'add-profile'},
      picks: [{parameter: true}],
      inputs: ['Created'],
      entity: 'profile_parameters',
      scope: 'main',
    },
    {
      action: {kind: 'add-session'},
      picks: [{parameter: true}],
      inputs: ['Created'],
    ...
    {
      action: {kind: 'add-profile', workflow: graphId('main')},
      picks: [],
      inputs: ['Created'],
      entity: 'profiles',
      scope: 'workflow:main',
    ...
    {
      action: {kind: 'add-session', workflow: graphId('main')},
      picks: [{value: true}],
      inputs: ['Created'],
      entity: 'sessions',
      scope: 'workflow:main',
  ...
  for (const item of cases) {
    ...
    assert.equal(
      run.reveals[0]?.inspection?.entity,
      item.entity,
...
test('only the latest refresh publishes a snapshot or read failure', async () => {
  ...
  const {refresh} = await load<typeof import('./projectHost')>(
    'projectHost.ts',
    {
      vscode: {
        commands: {},
  ...
  state.view = {
    webview: {postMessage: (message: HostToCanvas) => posted.push(message)},
...
test('automatic service analysis waits for Workspace Trust', async () => {
  ...
  const {refresh} = await load<typeof import('./projectHost')>(
    'projectHost.ts',
    {
      ...
      './verdogCommand': {
        cliCommand: () => ['verdog'],
        runVerdogCommand: async (_root: string, args: string[]) => {
          assert.deepEqual(args, ['analyze', '--json']);
          ++calls;
          return {
            code: 0,
            combined: '',
  ...
  try {
    ...
    if (state.snapshot?.termination?.status === 'unavailable') {
      assert.match(
        state.snapshot.termination.reason,
        /Trust this workspace.*send project manifests/,
      );
    }
    await delay(300);
...
test('workspace trust centrally gates editing and executable workflow verbs', async () => {
  ...
  assert.equal(
    isEditable(state),
    true,
    'catalogue permissions cannot restrict local editing',
  );
  state.access = {...state.access, accessible: true, read: true};
  assert.equal(
    isEditable(state),
    true,
    'read-only catalogue access cannot restrict local editing',
  );
  trusted = false;
  for (const verb of ['check', 'run', 'sync'] as const) {
    await runVerb(state, state.root, verb);
  }
  assert.equal(calls, 0);
  assert.equal(warnings.length, 3);
  assert.ok(
    warnings.every(warning => warning.includes('Trust this workspace')),
  );
  trusted = true;
...
test('explicit access lookup accepts API 14 catalogue permissions without changing editing', async () => {
  ...
  assert.equal(
    state.access,
    undefined,
    'legacy installation flags do not grant catalogue permissions',
  );
  assert.equal(isEditable(state), true);
});
...
test('property action handlers return the actual edit or rename result, including cancellation', async () => {
  ...
  const {handleCanvasAction} = await load<typeof import('./projectActions')>(
    'projectActions.ts',
    {
      ...
      './projectHost': {
        chooseSubroutine() {},
        cliCommand: () => ['verdog'],
        editSubroutine: async () => saved,
        entities: () => [],
        followNavigation() {},
        followNavigationTarget() {},
        projectOf: testProject,
        isEditable: () => true,
        locate: () => ({root: '/test/project', subroutine: 'main'}),
        readAccess() {},
        refresh() {},
        runVerb() {},
        subroutineIds: () => [],
  ...
  const actions: CanvasAction[] = [
    {kind: 'name', entity: 'nodes', id: nodeId('profile'), name: 'renamed'},
    {
      ...
      feature: featureId('remaining_iterations'),
    },
    {
  ...
  for (const message of actions) {
    saved = false;
    assert.equal(
      await handleCanvasAction(state, {...message, subroutine: 'main'}),
      false,
      message.kind,
    );
    saved = true;
  ...
  assert.equal(
    await handleCanvasAction(state, {
      kind: 'constrain',
      edge: edgeId('enter__profile'),
      subroutine: 'main',
    }),
...
test('a failed document save is not saved; a later generation warning does not undo a saved graph', async () => {
  ...
  const {editSubroutine} = await load<typeof import('./projectHost')>(
    'projectHost.ts',
    {
      vscode: {
      ...
      './cli': {
        verdog: async () => {
          ++generations;
          return {code: 1, combined: 'generation failed'};
        },
        diagnosticRange() {},
        parseVerdict() {},
        readCheckVerdict: async () => undefined,
...
test('host navigation shares history dispatch, prunes branches, and acknowledges every property result', async () => {
  ...
  const panel = {
    webview: {
      onDidReceiveMessage: (receive: typeof browserReceive) => {
        browserReceive = receive;
      },
      postMessage: (message: HostToNavigationBrowser) =>
  ...
  const {registerCanvas} = await load<typeof import('./canvasView')>(
    'canvasView.ts',
    {
      vscode: {
        Uri: {joinPath: () => ({})},
        ...
        window: {
          ...
          registerWebviewViewProvider: (
            _id: string,
            value: vscode.WebviewViewProvider,
          ) => {
            provider = value;
          },
      ...
      './projectHost': {
        ...
        followNavigation: (_host: OpenHost, entry: NavigationEntry) =>
          followedEntries.push(entry),
        followNavigationTarget: (_host: OpenHost, target: NavigationTarget) =>
  ...
  const view = {
    webview: {
      onDidReceiveMessage: (receive: typeof canvasReceive) => {
        canvasReceive = receive;
      },
      postMessage: (message: HostToCanvas) => canvasMessages.push(message),
    },
    onDidDispose() {},
  } as unknown as vscode.WebviewView;
  ...
  const browse = async (category: 'profiles' | 'sessions' | 'features') => {
    await canvasReceive({
      kind: 'browse',
      page: navigationPage(state.snapshot!, 'main', category),
      target: {scope: 'main'},
      panel: category,
    });
  };
  ...
  await canvasReceive({
    ...
    target: {scope: 'main'},
    panel: 'profiles',
    restoration: back.route,
  });
  await browse('features');
  const internals = provider as unknown as {
    browserVisits: Map<string, unknown>;
    browserTraversal: {entries: string[]};
  };
  assert.equal(internals.browserVisits.size, 2);
  assert.deepEqual(
    new Set(internals.browserVisits.keys()),
    new Set(internals.browserTraversal.entries),
  );
  ...
  await browse('profiles');
  assert.equal(
    internals.browserVisits.size,
    2,
    'a repeated route shares its retained page',
  );
  ...
  const page = entityPropertyPage(
    state.snapshot!,
    snapshotCanvasGraphs(state.snapshot!).main,
    inspection,
    'main',
  );
  assert.ok(page);
  ...
  assert.equal(calls, 1);
  assert.deepEqual(browserMessages.at(-1), {
  ...
  const report: TerminationReport = {
    ...
    definitions: {
      main: {
        ...
        reason: 'residual cycle',
        memory_states: 2,
        rules: 2,
        dependencies: [],
        edges: {enter__profile: 'remaining'},
        regions: [
          {
            id: 'region-1',
            nodes: ['profile'],
            edges: ['enter__profile'],
            witnesses: [],
  ...
  for (const invalid of [
    {...highlight, route: 'stale'},
    {...highlight, revision: 'stale'},
    {...highlight, region: 'missing'},
  ]) {
    await browserReceive(invalid);
  }
  assert.equal(
    canvasMessages.length,
    beforeHighlight,
    'stale routes, revisions, and unknown regions never reach the canvas',
  );
  ...
  assert.equal(
    canvasMessages.length,
    checkingMessages,
    'a refresh immediately disables prior report highlights',
  );
  state.snapshot!.termination = {status: 'ready', report};
  ...
  const edge = statusPage.termination?.entries.find(
    ({entity}) => entity === 'edges',
  );
  assert.ok(edge);
  await browserReceive({
    kind: 'reveal',
    route: statusRoute.route,
    key: edge.key,
  });
  assert.equal(
    followedEntries.at(-1)?.id,
    edge.id,
    'termination references use ordinary entity navigation',
  );
  panel.dispose();
