...
test("an installed extension opens the generated workspace in a new window", async () => {
  ...
  const { openPreviewWorkspace } = await load<typeof import("./checkout")>(
    ...
    {
      "./cli": {
        verdog: async (_root: string, arguments_: string[]) => {
      ...
      vscode: {
        ...
        commands: {
          executeCommand: async (...arguments_: unknown[]) => {
...
test("development opens an isolated host with the local Verdog extension", async () => {
  ...
  const workspace = uri(`/cache/preview-workspaces/${hash}.code-workspace`) as unknown as vscode.Uri;
  const context = {
    extensionMode: 2,
  ...
  assert.equal(
    ...
    inherited ? `${executableDirectory}${path.delimiter}${inherited}` : executableDirectory,
  );
...
test("README fetch retries the exact release through command-scoped GitHub SSH", async () => {
  ...
  const calls: string[][] = [];
  const run = async (_root: string, arguments_: string[]): Promise<Outcome> => {
    calls.push([...arguments_]);
    if (same(arguments_, ["rev-parse", "--is-bare-repository"])) return outcome(0, "true\n");
    if (same(arguments_, ["remote", "get-url", "origin"])) {
      return outcome(0, `https://github.com/${repository}.git\n`);
    }
    if (same(arguments_, fetch)) return outcome(128, "", "fatal: HTTPS credentials unavailable\n");
    if (same(arguments_, ["-c", GITHUB_SSH_URL_REWRITE, ...fetch])) return outcome(0);
    if (same(arguments_, ["cat-file", "-s", `${commit}:README.md`])) {
      return outcome(0, `${Buffer.byteLength(readme)}\n`);
    }
    if (same(arguments_, ["show", "--no-ext-diff", `${commit}:README.md`])) {
      return outcome(0, readme);
    }
    return outcome(1, "", `unexpected Git command: ${arguments_.join(" ")}\n`);
  };
  const { fetchCatalogueReadme } = await load<typeof import("./catalogueMetadata")>(
  ...
  assert.deepEqual(result, { markdown: readme, path: "README.md", state: "ready" });
  assert.deepEqual(calls.filter((arguments_) => arguments_.includes("fetch")), [
    fetch,
    ["-c", GITHUB_SSH_URL_REWRITE, ...fetch],
  ]);
  assert.equal(calls.some((arguments_) => arguments_[0] === "config"), false);
});
...
test("inspection retries source and recursive dependencies through command-scoped GitHub SSH", async () => {
  ...
  const run = async (_root: string, arguments_: string[]): Promise<Outcome> => {
    calls.push([...arguments_]);
    if (same(arguments_, ["rev-parse", "--git-dir"])) return outcome(0, ".git\n");
    if (same(arguments_, ["remote", "get-url", "origin"])) {
      return outcome(0, `https://github.com/${repository}.git\n`);
    }
    if (same(arguments_, sourceFetch)) return outcome(128, "", "fatal: HTTPS source denied\n");
    if (same(arguments_, ["-c", GITHUB_SSH_URL_REWRITE, ...sourceFetch])) return outcome(0);
    if (same(arguments_, ["checkout", "--detach", "--force", "--quiet", commit])) {
      checkedOut = true;
      return outcome(0);
    }
    if (same(arguments_, submoduleSync)) return outcome(0);
    if (same(arguments_, submoduleUpdate)) {
      return outcome(128, "", "fatal: HTTPS dependency denied\n");
    }
    if (same(arguments_, ["-c", GITHUB_SSH_URL_REWRITE, ...submoduleUpdate])) return outcome(0);
    return outcome(1, "", `unexpected Git command: ${arguments_.join(" ")}\n`);
  };
  const fs = {
    ...
    stat: async (location: MockUri) => {
      ...
      if (checkedOut && (name === "project.json" || name === ".gitmodules")) return {};
  ...
  const { materialise } = await load<typeof import("./checkout")>("checkout.ts", {
    vscode: {
      Uri: {
        file: uri,
        joinPath: (base: MockUri, ...parts: string[]) => uri(path.join(base.fsPath, ...parts)),
      },
      workspace: { fs },
    },
    "./cli": { verdog: run },
  ...
  assert.deepEqual(calls.filter((arguments_) => arguments_.includes("fetch")), [
    sourceFetch,
    ["-c", GITHUB_SSH_URL_REWRITE, ...sourceFetch],
  ]);
  assert.deepEqual(calls.filter((arguments_) => arguments_.includes("submodule")), [
    submoduleSync,
    submoduleUpdate,
...
test("preview locking leaves generated environments writable while locking publisher sources", async () => {
  const calls: Array<{ arguments_: string[]; command: string[] | undefined; root: string }> = [];
  const { lockPreview } = await load<typeof import("./checkout")>("checkout.ts", {
    ...
    "./cli": { verdog: async (
      ...
      calls.push({ arguments_: [...arguments_], command: options.command, root });
      return outcome(0);
    } },
  });
  ...
  assert.deepEqual(calls, [{
    arguments_: ["-R", "a-w", "--", ".vscode", "project.json", "src"],
    command: ["chmod"],
...
test("legacy preview environment repair makes only an untracked real .venv writable", async () => {
  ...
  const calls: Array<{ arguments_: string[]; command: string[] | undefined; root: string; signal?: AbortSignal }> = [];
  const { preparePreviewEnvironment } = await load<typeof import("./checkout")>("checkout.ts", {
    "node:fs/promises": { lstat: async (location: string) => {
    ...
    "./cli": { verdog: async (
      ...
      calls.push({ arguments_: [...arguments_], command: options.command, root: calledRoot, signal: options.signal });
      return outcome(0);
    } },
  });

  assert.equal(await preparePreviewEnvironment(root, () => undefined, signal), true);
  assert.deepEqual(calls, [
    { arguments_: ["ls-files", "--", ".venv"], command: ["git"], root, signal },
    { arguments_: ["-R", "u+w", "--", ".venv"], command: ["chmod"], root, signal },
  ]);
});

test("preview environment repair refuses symlinked and tracked .venv state", async () => {
  for (const scenario of [
    { directory: false, label: "symlink", tracked: "", warning: /not a real directory/ },
    { directory: true, label: "tracked", tracked: ".venv/pyvenv.cfg\n", warning: /source tracks \.venv/ },
  ]) {
    const calls: string[][] = [];
    const logs: string[] = [];
    const { preparePreviewEnvironment } = await load<typeof import("./checkout")>("checkout.ts", {
      "node:fs/promises": { lstat: async () => ({
        isDirectory: () => scenario.directory,
        isSymbolicLink: () => !scenario.directory,
      }) },
      vscode: { Uri: { file: uri }, workspace: { fs: {} } },
      "./cli": { verdog: async (_root: string, arguments_: string[]): Promise<Outcome> => {
        calls.push([...arguments_]);
    ...
    assert.equal(
      await preparePreviewEnvironment("/catalogue/exact-release", (line) => logs.push(line)),
      false,
      scenario.label,
    );
    assert.deepEqual(
