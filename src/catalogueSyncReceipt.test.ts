...
const receipt = (overrides: Record<string, unknown> = {}): string => JSON.stringify({
  environment: "/cache/exact-release/.verdog/environments/main__review",
  interpreter: "/cache/exact-release/.verdog/environments/main__review/bin/python",
  only_binary: true,
...
test("a wheel-only sync receipt is bound to the selected POSIX checkout and workflow", () => {
  ...
  assert.deepEqual(decodeCatalogueSyncReceipt(receipt(), { checkout, platform: "linux", workflow }), {
    environment: "/cache/exact-release/.verdog/environments/main__review",
    interpreter: "/cache/exact-release/.verdog/environments/main__review/bin/python",
    only_binary: true,
    requirements: ["alpha==1"],
...
test("Windows inspection accepts only its selected Scripts interpreter", () => {
  ...
  assert.deepEqual(paths, {
    environment: "C:\\cache\\exact-release\\.verdog\\environments\\main__review",
    interpreter: "C:\\cache\\exact-release\\.verdog\\environments\\main__review\\Scripts\\python.exe",
  });
  assert.ok(decodeCatalogueSyncReceipt(JSON.stringify({
    ...paths,
    only_binary: true,
    requirements: [],
    status: "ready",
    workflow_id: workflow,
  }), { checkout: windowsCheckout, platform: "win32", workflow }) !== undefined);
});
...
test("mismatched, borrowed, non-wheel-only and partial sync receipts stay incomplete", () => {
  for (const overrides of [
    ...
    { interpreter: "/cache/exact-release/.verdog/environments/main__review/Scripts/python.exe" },
    { interpreter: "./.verdog/environments/main__review/bin/python" },
    { requirements: undefined },
  ]) {
    assert.equal(
      decodeCatalogueSyncReceipt(receipt(overrides), { checkout, platform: "linux", workflow }),
      undefined,
    );
  }
});
