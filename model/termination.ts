...
export type TerminationCycleStep = { node: string; values: string[]; edge: string };
...
export type DefinitionTermination = {
  status: TerminationStatus;
  local_status: TerminationStatus;
  reason: string;
  memory_states: number;
  rules: number;
  dependencies: { scope: string; node: string }[];
...
export type DefinitionTerminationState = Exclude<TerminationState, { status: "ready" }> |
  { status: "ready"; definition: DefinitionTermination; revision: string };

export const terminationRevision = (report: TerminationReport): string =>
  JSON.stringify(Object.entries(report.projects).sort(([left], [right]) => left.localeCompare(right)));

export function terminationFor(
  state: TerminationState | undefined,
  scope: string,
): DefinitionTerminationState {
  if (state === undefined) return { status: "unavailable", reason: "Analysis has not run yet." };
  if (state.status !== "ready") return state;
  const definition = state.report.definitions[scope];
  return definition === undefined
    ? { status: "unavailable", reason: "Analysis is unavailable for this definition." }
    : { status: "ready", definition, revision: terminationRevision(state.report) };
}
...
export const terminationStatus = (state: DefinitionTerminationState) =>
  state.status === "ready" ? state.definition.status : state.status;

export type EdgeTerminationDisplayStatus = TerminationEdgeStatus | "checking" | "unavailable";
...
export function edgeTerminationStatus(
  ...
  if (state?.status !== "ready" || state.definition.local_status === "unavailable") return "unavailable";
  return state.definition.edges[edge] ?? "unavailable";
}

export type TerminationHighlight = { scope: string; region: string | null; revision: string };
...
export function highlightedTerminationRegion(
  ...
  if (state?.status !== "ready" || highlight?.scope !== scope ||
    highlight.revision !== terminationRevision(state.report)) return undefined;
  return state.report.definitions[scope]?.regions.find(({ id }) => id === highlight.region);
}
