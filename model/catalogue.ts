...
export interface CataloguePreviewNode {
  id: string;
  kind: string;
  name: string;
}
export interface CataloguePreviewEdge {
  id: string;
  name: string;
  source: string;
  target: string;
}
export interface CataloguePreviewFeature {
  id: string;
  kind?: string;
  name: string;
}
...
export interface CatalogueEnvironment {
  python?: string;
  requirements: string[];
  runtime?: string;
...
export interface DecodedCatalogueListing {
  entries: CatalogueSummary[];
  login?: string;
  next_cursor?: string;
  rejected: string[];
...
function commit(value: unknown, name: string): string {
  ...
  if (!COMMIT.test(found)) {
    throw new Error(`${name} must be a 40-digit commit hash`);
  }
  return found.toLowerCase();
}

function repository(value: unknown, name: string): string {
  const found = string(value, name);
...
function publishedAt(value: unknown, name: string): string {
  const found = string(value, name);
...
function visibility(
  ...
  if (raw.visibility === 'public' || raw.visibility === 'restricted') {
    return raw.visibility;
  }
  if (raw.scope === 'public') {
    return 'public';
  }
  if (raw.scope === 'private') {
    return 'restricted';
  }
...
function legacyScope(
  ...
  if (raw.scope === 'private' || raw.scope === 'public') {
    return raw.scope;
  }
...
function release(
  ...
  return {
    ...
    visibility:
      raw.visibility === undefined && raw.scope === undefined
        ? inheritedVisibility
        : visibility(raw, name),
  };
}
...
function preview(
        ...
        return [
          {
            ...
            name:
              typeof node.name === 'string' && node.name ? node.name : node.id,
          },
        ];
      })
    : [];
        ...
        return [
          {
            id: edge.id,
            name:
              typeof edge.name === 'string' && edge.name ? edge.name : edge.id,
            source: edge.source,
            target: edge.target,
          },
        ];
  ...
  const features = Array.isArray(raw.features)
    ? raw.features.flatMap(value => {
        const feature = object(value);
        if (feature === undefined || typeof feature.id !== 'string') {
          return [];
        }
        return [
          {
  ...
  const ports = Object.fromEntries(
    Object.entries(rawPorts).filter(
...
function environment(value: unknown): CatalogueEnvironment | undefined {
  ...
  if (
    raw.requirements.some(item => typeof item !== 'string' || item.length === 0)
  ) {
    throw new Error(
      'entry.environment.requirements must contain non-empty strings',
    );
  }
  const requirements = raw.requirements as string[];
  if (new Set(requirements).size !== requirements.length) {
    throw new Error(
      'entry.environment.requirements must not contain duplicates',
    );
  }
  if (
    raw.python !== undefined &&
    (typeof raw.python !== 'string' || raw.python.length === 0)
  ) {
  ...
  if (
    ...
    throw new Error(
      'entry.environment.schema_version must be a positive integer',
    );
  }
  return {
    ...(typeof raw.python === 'string' ? {python: raw.python} : {}),
    requirements: [...new Set(requirements)].sort((left, right) =>
      left.localeCompare(right),
    ),
    ...(typeof raw.runtime === 'string' ? {runtime: raw.runtime} : {}),
    ...(typeof schema === 'number' ? {schema_version: schema} : {}),
  };
}
...
function decode(value: unknown): CatalogueDetail {
  ...
  const closure = (
    raw.closure === undefined
      ? []
      : !Array.isArray(raw.closure)
        ? (() => {
            throw new Error('entry.closure must be an array');
          })()
        : raw.closure.map((value, index) =>
            pin(value, `entry.closure[${index}]`),
          )
  ).sort((left, right) =>
    `${left.repository}@${left.commit}`.localeCompare(
      `${right.repository}@${right.commit}`,
    ),
  );
  ...
  return {
    ...
    dependency_count: integer(
      raw.repository_dependency_count ?? raw.dependency_count,
      closure.length,
    ),
...
export function decodeCatalogueDescriptor(value: unknown): CatalogueDescriptor {
  ...
  if (!Array.isArray(raw.closure)) {
    throw new Error('descriptor.closure must be an array');
  }
  const decodedPreview = preview(raw.preview, workflowId);
  if (decodedPreview === undefined) {
    throw new Error('descriptor.preview must be an object');
  }
  const decodedEnvironment = environment(raw.environment);
  const schema = raw.schema_version;
  return {
...
export function catalogueMetadataDifferences(
  ...
  if (detail.workflow_id !== descriptor.workflow_id) {
    differences.push('workflow identity');
  }
  if (detail.package !== descriptor.package) {
    differences.push('package');
  }
  if (detail.display_name !== descriptor.display_name) {
    differences.push('display name');
  }
  if (
    detail.preview === undefined ||
    !equivalent(detail.preview.canonical, descriptor.preview.canonical)
  ) {
    differences.push('workflow structure');
  }
  if (!equivalent(detail.closure, descriptor.closure)) {
    differences.push('repository dependencies');
  }
  if (
    detail.environment === undefined ||
    !equivalent(detail.environment, descriptor.environment)
  ) {
...
export function decodeCatalogueListing(
  value: unknown,
): DecodedCatalogueListing {
  const raw = record(value, 'catalogue');
  if (!Array.isArray(raw.entries)) {
    throw new Error('catalogue.entries must be an array');
  }
  ...
  const login =
    raw.login === undefined ? undefined : string(raw.login, 'catalogue.login');
  const next =
