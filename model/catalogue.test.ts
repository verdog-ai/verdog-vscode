...
function id(digit: string): string {
  return `${digit.repeat(8)}-${digit.repeat(4)}-4${digit.repeat(3)}-a${digit.repeat(3)}-${digit.repeat(12)}`;
}
...
test('new records retain visibility, ports, environment and exact release metadata', () => {
  const decoded = decodeCatalogueDetail(
    entry({
      display_name: 'Review evidence',
      environment: {
        python: '>=3.12',
        requirements: ['z>=1', 'a==2'],
        schema_version: 32,
      },
      preview: {
        edges: [
  ...
  assert.deepEqual(decoded.preview?.ports, {
    enter: 'enter',
    exit: 'exit',
    failure: 'failure',
  });
...
test('inspection compares the exact source descriptor including workflow ports', () => {
  const preview = {
    edges: [
      {id: 'enter__exit', name: 'Continue', source: 'enter', target: 'exit'},
    ],
    features: [],
    name: 'Main',
    nodes: [
  ...
  const detail = decodeCatalogueDetail(
    entry({
      display_name: 'Main',
  ...
  assert.deepEqual(catalogueMetadataDifferences(detail, changed), [
    'workflow structure',
  ]);
});
...
test('legacy records remain readable but cannot pass inspection without published gate metadata', () => {
  const detail = decodeCatalogueDetail(
    entry({closure: undefined, environment: undefined, preview: undefined}),
  );
  const descriptor = decodeCatalogueDescriptor({
    closure: [],
