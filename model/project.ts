...
export const DEFINITION_KIND = {
  ...
  workflows: "workflow",
} as const satisfies Record<(typeof DEFINITION_COLLECTION)[DefinitionKind], DefinitionKind>;
...
export const isExecutableNodeKind = (value: unknown): value is ExecutableNodeKind =>
  EXECUTABLE_NODE_KINDS.some((kind) => kind === value);

export const isCallNodeKind = (value: unknown): value is CallNodeKind =>
  CALL_NODE_KINDS.some((kind) => kind === value);

export const isAddableNodeKind = (value: unknown): value is AddableNodeKind =>
  isExecutableNodeKind(value) || isCallNodeKind(value);

/** Why an edge cannot use these endpoint kinds, or nothing when it can. */
export function edgeDirectionProblem(source: NodeKind, target: NodeKind): string | undefined {
  if (target === "enter") return "the enter port cannot have incoming edges";
  if (source === "exit") return "the exit port cannot have outgoing edges";
  if (source === "failure") return "the failure port cannot have outgoing edges";
  return undefined;
}
...
export type SubroutineCallArguments = {
...
export type ProjectNode = ProjectNodeBase & (
  ...
  | {
      ...
      operation: NodeOperation & SubroutineCallArguments & {
        target: GraphId | QualifiedSubroutineTarget;
      };
    }
  | {
...
export type LocalWorkflowDefinition = CanonicalObject & SubroutineCallArguments & {
  profiles: AgentProfile[];
  sessions: AgentSession[];
  subroutine: GraphId;
};
...
export type WorkflowDefinition = LocalWorkflowDefinition | ExternalWorkflowDefinition;
...
const graph = (value: unknown): SubroutineDefinition | undefined => {
...
const localWorkflow = (value: unknown): LocalWorkflowDefinition | undefined => {
  const candidate = object(value);
...
export const definitionKey = (kind: DefinitionKind, id: GraphId): string => `${kind}:${id}`;

const GRAPH_SEPARATOR = "__";

/** The declaration-local leaf of a canonical graph id. */
export const graphLeaf = (id: GraphId): GraphId => {
  const separator = id.lastIndexOf(GRAPH_SEPARATOR);
  return separator < 0 ? id : graphId(id.slice(separator + GRAPH_SEPARATOR.length));
};

/** A declaration's canonical identity within its lexical owner. */
export const qualifyGraph = (owner: GraphId | undefined, localId: GraphId): GraphId =>
  owner === undefined ? localId : graphId(`${owner}${GRAPH_SEPARATOR}${localId}`);
...
export function workflowId(value: unknown): GraphId | undefined {
  ...
  return local === undefined
    ? (typeof candidate.id === "string" ? candidate.id as GraphId : undefined)
...
export function definitionIndex(project: unknown): DefinitionIndex {
  ...
  const localDefinitions: { definition: Definition; workflow: LocalWorkflowDefinition }[] = [];
  ...
  const visitWorkflow = (
    ...
    if (workflow !== undefined) {
      ...
      if (add(definition)) localDefinitions.push({ definition, workflow });
      return;
    }
  ...
  const visitGraph = (
    ...
    for (const raw of entries(body.workflows)) {
      ...
      if (localId !== undefined) visitWorkflow(raw, id, [...path, "workflows", localId]);
    }
  ...
  if (rootWorkflow !== undefined) {
    visitWorkflow(rootWorkflow, undefined, ["workflows", graphLeaf(rootWorkflow.subroutine)]);
  }
  ...
  for (const { definition, workflow } of localDefinitions) {
    ...
    if (subroutine !== undefined) {
      definition.name = typeof subroutine.name === "string" ? subroutine.name : definition.localId;
  ...
  return {
    ...
    ...(rootWorkflow === undefined
      ? {}
      : { rootWorkflow: qualifyGraph(undefined, graphLeaf(rootWorkflow.subroutine)) }),
  };
}
...
): Definition | undefined => index.definitions.get(definitionKey(kind, id));

export const subroutinesIn = (
  project: unknown,
  index = definitionIndex(project),
): SubroutineDefinition[] =>
  [...index.subroutines].map(([id, body]) =>
    id === body.id ? body : { ...body, id }
  );

export const subroutineInProject = (
  project: unknown,
  id: GraphId,
): SubroutineDefinition | undefined => definitionIndex(project).subroutines.get(id);
...
export function visibleDefinitions(
  ...
  const visible = scopes.flatMap((scope) =>
    [...index.definitions.values()].filter(
      (definition) =>
        definition.declaredIn === scope && (kind === undefined || definition.kind === kind),
    ),
  );
  const roots = ([
    ["workflow", index.rootWorkflow],
    ["subroutine", index.rootSubroutine],
  ] as const).flatMap(([rootKind, id]) => {
    const definition = id === undefined
      ? undefined
      : index.definitions.get(definitionKey(rootKind, id));
    return definition === undefined || (kind !== undefined && definition.kind !== kind)
      ? []
      : [definition];
  });
  return [
    ...visible,
    ...roots.filter(
      (root) => !visible.some(({ id, kind: visibleKind }) => id === root.id && visibleKind === root.kind),
    ),
  ];
}
...
export const definitionPath = (
  ...
  id: GraphId,
  index = definitionIndex(project),
): string[] | undefined => definitionIn(project, kind, id, index)?.path;
...
export function resolveCall(
  ...
  return visibleDefinitions(project, graphId, expected, index).find(
    (definition) => definition.id === node.operation?.target &&
