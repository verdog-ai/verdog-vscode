...
/** @fileoverview Fault-tolerant reads of the project document while an author is typing it. */
...
export function contextAt(text: string, offset: number): CompletionContext {
  ...
  const bodyPaths: JSONPath[] =
    path[0] === 'subroutine' ? [['subroutine']] : [];
  ...
  const kind =
    itemPath === undefined ? undefined : stringAt(root, [...itemPath, 'kind']);
  ...
  return {
    ...
    property,
    subroutine:
      scope.length > 0 && scope.every((id): id is string => id !== undefined)
        ? scope.join('__')
        : undefined,
  };
}
...
export function featuresIn(
  ...
  const found: Feature[] = [];
  const bodies =
    subroutine === undefined ? [] : [subroutineInProject(project, subroutine)];
  for (const body of bodies) {
    if (body === undefined) {
      continue;
    }
    for (const feature of entries(body.features)) {
      if (typeof feature.id !== 'string' || !isFeatureKind(feature.kind)) {
        continue;
      }
      const common = {
        id: feature.id,
      ...
      if (feature.kind === 'enum') {
        found.push({
          ...common,
          kind: 'enum',
          values: array(feature.values).filter(
...
export function documentFeatures(
  ...
  return featuresIn(parse(text) as unknown, subroutine);
}
...
export function documentDefinitions(
  ...
  if (subroutine === undefined || callKind === undefined) {
    return [];
  }
  return callTargets(
