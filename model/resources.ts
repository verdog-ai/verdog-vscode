...
/** @fileoverview Resource contracts shared by entity property pages and the mutation boundary. */
...
export type ResourceSelection = Pick<
  ResourceField,
...
function resourceKey(
  field: Pick<ResourceField, 'parameter' | 'resource'>,
): string {
  return `${field.resource}:${field.parameter ?? ''}`;
}

function callResourceFields(
  ...
  if (target === undefined) {
    return [
      ...Object.entries(object(args.profile_arguments))
        .sort()
        .map(([parameter, value]) => ({
      ...
      ...Object.entries(object(args.session_arguments))
        .sort()
        .map(([parameter, value]) => ({
  ...
  return [
    ...entries(target.profile_parameters)
      .filter(parameter => text(parameter.id))
      .map(parameter => ({
        label: `profile ${parameter.id}`,
        options: [...profiles],
        parameter: text(parameter.id),
        resource: 'profile' as const,
        value: text(object(args.profile_arguments)[text(parameter.id)]),
      })),
    ...entries(target.session_parameters)
      .filter(parameter => text(parameter.id))
      .map(parameter => ({
        ...
        parameter: text(parameter.id),
        resource: 'session' as const,
        value: text(object(args.session_arguments)[text(parameter.id)]),
      })),
  ];
}

/** Resources explicitly owned by a workflow boundary. */
export function workflowBoundaryResources(workflow: LocalWorkflowDefinition): {
  profiles: string[];
...
export function nodeResourceFields(
  ...
  const target = subroutineCallTarget(
    snapshot,
    subroutineId,
  ...
  return callResourceFields(
    target,
    node.operation ?? {profile_arguments: {}, session_arguments: {}},
    profiles,
    sessions,
  );
}
...
function normalizedSelections(
  ...
  if (expected.size !== fields.length) {
    throw new Error('this node has duplicate resource fields');
  }
  ...
  for (const resource of submitted) {
    ...
    if (selected.has(key)) {
      throw new Error(`resource ${key} was submitted more than once`);
    }
    const field = expected.get(key);
    if (field === undefined) {
      throw new Error(`resource ${key} is not required by this node`);
    }
  ...
  if (selected.size !== expected.size) {
    throw new Error('not every required resource was submitted');
  }
  return selected;
}

function callArguments(
  selected: ReadonlyMap<string, ResourceSelection>,
): SubroutineCallArguments {
  const args: SubroutineCallArguments = {
    profile_arguments: {},
    session_arguments: {},
  };
  for (const resource of selected.values()) {
    if (resource.parameter === undefined) {
      throw new Error('a call binding needs a parameter');
    }
    if (resource.resource === 'profile') {
      args.profile_arguments[agentProfileId(resource.parameter)] =
        agentProfileId(resource.value);
    } else {
      args.session_arguments[agentSessionId(resource.parameter)] =
...
export function normalizeWorkflowResources(
  ...
  return callArguments(
    normalizedSelections(workflowResourceFields(target, workflow), submitted),
  );
}
