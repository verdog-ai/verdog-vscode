...
if (/["']\.\/impl\/(?:edit|format|parser|scanner)["']/.test(extension)) {
  throw new Error(
    'dist/extension.js: jsonc-parser left an unresolved internal import',
  );
}
...
const duplicates = Object.keys(webviews.metafile.inputs).filter(
  file =>
...
if (duplicates.length > 0) {
  throw new Error(
    `a second copy of a shared library was bundled:\n  ${duplicates.join('\n  ')}`,
  );
}
for (const name of [
  'dist/webview.js',
...
const packageRoots = new Set(
  [
    ...
    ...Object.keys(webviews.metafile.inputs),
  ]
...
for (const root of [...packageRoots].sort()) {
  ...
  const licenses = (await readdir(root))
    .filter(name => /^(licen[cs]e|notice)([.-]|$)/i.test(name))
    .sort();
  if (licenses.length === 0) {
    throw new Error(`Missing bundled dependency license: ${root}`);
  }
  notices.push(`${metadata.name}@${metadata.version} (${metadata.license})`);
  for (const license of licenses) {
    notices.push(await readFile(`${root}/${license}`, 'utf8'));
  }
}
