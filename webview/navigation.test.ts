...
const documents = (): ProjectSnapshot["entity_documents"] => ({
  edges: {},
  features: {},
  nodes: {},
  profile_parameters: {},
...
const project = (): CanonicalProject => testProject() as unknown as CanonicalProject;
...
test("status navigation preserves root and pinned verification context", () => {
  ...
  assert.deepEqual(navigationPage(current, "main", "status"), {
    ...
    title: "Status",
    termination: { state: { status: "unavailable", reason: "Analysis has not run yet." }, entries: [] },
  });
...
test("termination links reuse scoped entity navigation without deletion actions", () => {
  ...
  const definition: DefinitionTermination = {
    status: "not_certified", local_status: "not_certified", reason: "Residual cycles remain.",
    memory_states: 4, rules: 5, dependencies: [{ scope: "main__implement", node: "call_implement" }],
    edges: { iteration_consumed: "remaining" },
    regions: [{
      id: "0", nodes: ["profile", "consume_iteration"], edges: ["iteration_consumed"],
      witnesses: [{ feature: "remaining_iterations", expression: "remaining_iterations↓", edges: ["iteration_consumed"], opposing_edges: [] }],
      cycle: [{ node: "consume_iteration", values: ["remaining_iterations>0"], edge: "iteration_consumed" }],
    }],
  };
  current.termination = {
    status: "ready", report: {
      ...
      definitions: {
        ...
        main__implement: { ...definition, status: "certified", local_status: "certified", dependencies: [], regions: [] },
      },
    },
  };
  ...
  assert.equal(analysis.entries.find(({ entity }) => entity === "features")?.target?.inspection?.id, "remaining_iterations");
  const dependency = analysis.entries.find(({ key }) => key.startsWith("termination/dependency:"));
  assert.equal(dependency?.target?.scope, "main__implement");
  assert.deepEqual(dependency?.meta, ["Certified", "called by call_implement"]);
  assert.ok(analysis.entries.every(({ removal, children }) => removal === undefined && children.length === 0));
  ...
  const page = entityPropertyPage(current, graph, target.inspection!, "Subroutine main");
  assert.deepEqual(page?.termination, navigationPage(current, "main", "status").termination);
});
...
test("definition navigation keeps root and pin duplicate identities distinct", () => {
  ...
  const localNested = definitions.find(({ key }) => key === "subroutine:main__implement");
  const external = definitions.find(({ key }) => key === "publisher.tool/subroutine:main");
  ...
  assert.equal(local.declaration, "src/demo/project/subroutines/main/__init__.py");
  assert.equal(
  ...
  assert.deepEqual(
    pinnedComponents.find(({ entity, id }) => entity === "nodes" && id === "profile")?.removal,
    { entity: "nodes", id: "profile", subroutine: "publisher.tool/main" },
  );
  assert.deepEqual(
    pinnedComponents.find(({ entity, id }) =>
      entity === "edges" && id === "profile_analyze"
    )?.removal,
    { entity: "edges", id: "profile_analyze", subroutine: "publisher.tool/main" },
  );
  assert.deepEqual(
    pinnedComponents.find(({ entity, id }) =>
      entity === "profiles" && id === "default"
    )?.removal,
...
test("same-id subroutine and workflow definitions list only their matching callers", () => {
  ...
  root.sources = [...root.sources as unknown[], { path: rootWorkflowDeclaration }];
  root.subroutine.workflows.push({
    subroutine: graphId("main__implement"),
    profiles: [],
    sessions: [],
  ...
  const subroutine = navigationPage(current, "main", "subroutines").entries.find(
    ({ key }) => key === "subroutine:main__implement",
  );
  const rootSubroutine = navigationPage(current, "main", "subroutines").entries.find(
    ({ key }) => key === "subroutine:main",
  );
  const workflow = navigationPage(current, "main", "workflows").entries.find(
    ({ key }) => key === "workflow:main__implement",
  );
  const rootWorkflow = navigationPage(current, "main", "workflows").entries.find(
  ...
  assert.deepEqual(subroutine.children.map((entry) => ({
  ...
  assert.deepEqual(rootCaller, {
    ...
    target: {
      inspection: { entity: "workflows", id: "main", subroutine: "main", workflow: "main" },
      scope: "workflow:main",
      selection: { entity: "nodes", id: "subroutine" },
    },
  });
  assert.equal(rootCaller.removal, undefined);
  assert.deepEqual(workflow.children.map((entry) => ({
    declaration: entry.declaration,
    entity: entry.entity,
    id: entry.id,
    meta: entry.meta,
  ...
  assert.ok(caller);
  assert.ok(call);
  assert.deepEqual(calledDefinitionTarget(current, caller, call), subroutine.target);
  assert.deepEqual(definitionTarget(current, graphs.main__implement!), subroutine.target);
  assert.deepEqual(definitionTarget(current, graphs["workflow:main__implement"]!), workflow.target);
});
...
test("qualified subroutine calls and external workflow calls keep their definition owner", () => {
  ...
  current.entity_documents.nodes.main = {
    call_pinned: {
      declaration: "src/demo/project/subroutines/main/nodes/call_pinned/__init__.py",
    },
    run_borrowed: {
      declaration: "src/demo/project/subroutines/main/nodes/run_borrowed/__init__.py",
    },
  };

  const subroutines = navigationPage(current, "main", "subroutines").entries;
  const local = subroutines.find(({ key }) => key === "subroutine:main__implement");
  const external = subroutines.find(
    ({ key }) => key === "publisher.tool/subroutine:main__implement",
  );
  assert.ok(local);
  assert.ok(external);
  assert.equal(local.children.some(({ id }) => id === "call_pinned"), false);
  assert.deepEqual(
    external.children.find(({ id, target }) =>
      id === "call_pinned" && target?.scope === "main"
    )?.target,
    {
  ...
  assert.deepEqual(external.children.find(({ id }) => id === "call_pinned")?.removal, {
    entity: "nodes",
    id: "call_pinned",
    subroutine: "main",
  });
  ...
  const pinnedWorkflow = workflows.find(({ key }) => key === "publisher.tool/workflow:main");
  assert.ok(binding);
  assert.ok(pinnedWorkflow);
  assert.deepEqual(binding.children.map(({ id }) => id), ["run_borrowed"]);
  ...
  assert.ok(caller);
  assert.ok(call);
  assert.deepEqual(calledDefinitionTarget(current, caller, call), binding.target);
  ...
  assert.equal(pinnedWorkflow.children.some(({ id }) => id === "run_borrowed"), false);
});
...
test("external owner folders distinguish a colliding alias and preserve nested scope", () => {
  ...
  const definitions = navigationPage(current, "main", "subroutines").entries;
  const local = definitions.find(({ key }) => key === "subroutine:main");
  const external = definitions.find(({ key }) => key === "demo.project/subroutine:main");
...
test("a feature and each canvas referrer link to their own declarations", () => {
  ...
  assert.deepEqual(feature.target, {
    inspection: { entity: "features", id: "remaining_iterations", subroutine: "main" },
    scope: "main",
  });
...
test("read-only navigation offers no deletion actions", () => {
  ...
  const entries = (["subroutines", "workflows", "features", "profiles", "sessions"] as const)
...
test("partially authored references degrade to empty instead of throwing", () => {
  const current = snapshot(project());
  const graph = current.project.subroutine as unknown as Record<string, unknown>;
  graph.edges = [{ conditions: "being typed", effects: undefined, id: "partial" }];
  graph.nodes = [{ id: "partial", kind: "agent", operation: undefined }];

  assert.equal(navigationPage(current, "main", "features").entries.length, 1);
  assert.deepEqual(navigationPage(current, "main", "profiles").entries[0]?.children, []);
...
test("workflow navigation keeps its wrapper context and explicit boundary resources", () => {
  ...
  const current = snapshot(root);
  const declaration = "src/demo/project/workflows/main/profiles/builder/__init__.py";
  ...
  assert.deepEqual(profiles.entries.map(({ id }) => id), ["builder"]);
  ...
  assert.deepEqual(profiles.entries[0]?.scope, ["demo/project", "Workflows", "main"]);
  ...
  assert.deepEqual(sessions.entries.map(({ id, meta }) => ({ id, meta })), [
    { id: "history", meta: ["persistent"] },
    { id: "temporary", meta: ["fresh"] },
  ]);
  assert.deepEqual(sessions.entries.map(({ removal }) => removal), [undefined, undefined]);
  assert.deepEqual(sessions.entries.map(({ removalBlocked }) => removalBlocked), [
  ...
  assert.deepEqual(navigationPage(current, "workflow:main", "features").entries, []);
  assert.equal(
    navigationPage(current, "workflow:main", "subroutines").entries
      .find(({ key }) => key === "subroutine:main")?.target?.scope,
    "main",
  );
  assert.equal(
    navigationPage(current, "workflow:main", "workflows").entries
      .find(({ key }) => key === "workflow:main")?.target?.scope,
    "workflow:main",
  );
  assert.equal(navigationPage(current, "workflow:main", "status").context, "Workflow main");
  ...
  assert.deepEqual(components.map(({ entity, id }) => ({ entity, id })), [
    { entity: "nodes", id: "subroutine" },
    { entity: "profiles", id: "builder" },
    { entity: "sessions", id: "history" },
    { entity: "sessions", id: "temporary" },
  ]);
  assert.equal(components.every(({ scope }) => scope.length === 0), true);
  ...
  assert.equal(components[1]?.removalBlocked, "Rebind this profile before deleting it.");
...
test("a subroutine definition lists every directly owned entity", () => {
  ...
  root.subroutine.session_parameters = [{ id: "conversation", name: "Conversation" }];
  root.subroutine.workflows.push({
    profile_arguments: {},
    profiles: [],
    session_arguments: {},
  ...
  for (const port of ["enter", "exit", "failure"]) {
    assert.equal(
      components.find(({ entity, id }) => entity === "nodes" && id === port)?.removal,
      undefined,
    );
  }
  assert.deepEqual(
    components.find(({ entity, id }) => entity === "nodes" && id === "profile")?.target,
    {
  ...
  assert.deepEqual(
    components.find(({ entity, id }) => entity === "edges" && id === "profile_analyze")
      ?.removal,
...
test("a profile referrer focuses its agent node and opens that node's declaration", () => {
  ...
  assert.deepEqual(profile.children.find(({ id }) => id === "call_implement")?.target, {
    selection: { entity: "nodes", id: "call_implement" },
    scope: "main",
  });
});
...
test("resource parameters are labeled as parameters", () => {
  ...
  const parameter = navigationPage(snapshot(root), "main", "profiles").entries.find(
    ({ id }) => id === "agent",
  );
...
test("a parameter links to every workflow and subroutine call that binds it", () => {
  ...
  assert.deepEqual(parameter.children.find(({ entity }) => entity === "nodes")?.target, {
    selection: { entity: "nodes", id: "call_implement" },
    scope: "main",
  });
  assert.deepEqual(parameter.children.find(({ entity }) => entity === "workflows")?.target, {
    inspection: {
      entity: "workflows",
      id: "main__implement",
      subroutine: "main",
...
test("a removed workflow resource opens its workflow declaration and properties", () => {
  ...
  assert.equal(entry.declaration, declaration);
  assert.deepEqual(entry.scope, ["demo/project", "main", "Workflows", "main__implement"]);
  assert.deepEqual(entry.target, {
    inspection: {
      entity: "profiles",
      id: "builder",
