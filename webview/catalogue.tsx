...
/** @fileoverview Discovery sidebar and the full-editor scholarly record for a published workflow. */
...
interface Host {
  postMessage: (message: CatalogueToHost) => void;
}
...
function exactReference(entry: CatalogueSummary): string {
  return `${entry.repository}@${entry.commit} ${entry.workflow_id}`;
}
...
function locallyVisible(
  ...
  if (scope === 'public' || scope === 'restricted') {
    return entry.visibility === scope;
  }
  // `published_by_me` is authoritative. The owner fallback keeps one preceding CLI usable.
  return (
    entry.published_by_me ||
    (login !== undefined && entry.repository.startsWith(`${login}/`))
  );
}
  ...
  return (
    <li className="catalogue-row">
      ...
      >
        ...
        <span
          className={`row-abstract${entry.description === null ? ' absent' : ''}`}
        >
        ...
        <span className="row-citation">
          ...
          {entry.dependency_count > 0
            ? ` · ${entry.dependency_count} dependencies`
            : ''}
        </span>
      </button>
    </li>
...
function CatalogueSidebar({initial}: {initial: CatalogueListing}) {
  ...
  return (
    <aside
      ...
      aria-label="Published workflow catalogue"
    >
      <header className="sidebar-header">
        ...
        <div className="filter-line">
          <label className="visually-hidden" htmlFor="catalogue-scope">
            Visibility
          </label>
          <select
            id="catalogue-scope"
      ...
      <div aria-live="polite" className="sidebar-status">
        ...
        {listing.state === 'unavailable'
          ? (listing.detail ?? 'The catalogue is unavailable.')
          : undefined}
      </div>
      ...
      shown.length === 0 ? (
        ...
        <ol className="catalogue-index">
          {shown.map(entry => (
            <CatalogueRow entry={entry} key={entry.id} />
          ))}
        </ol>
      )}
      {listing.next_cursor !== undefined ? (
        <button
          className="load-more"
          onClick={() =>
            host.postMessage({cursor: listing.next_cursor!, kind: 'load-more'})
          }
          type="button"
        >
...
function previewDot(preview: CataloguePreview): string {
  ...
  for (const node of preview.nodes) {
    lines.push(
      `${quote(node.id)} [label=${quote(`${node.kind.replaceAll('_', ' ')}\n${node.name}`)}];`,
    );
  }
  for (const edge of preview.edges) {
    lines.push(
      `${quote(edge.source)} -> ${quote(edge.target)} [label=${quote(edge.name)}];`,
    );
  }
  lines.push('}');
  return lines.join('\n');
...
function WorkflowFigure({preview}: {preview: CataloguePreview}) {
  ...
  useEffect(() => {
    ...
    if (container === null || preview.nodes.length === 0) {
      return;
    }
  ...
  if (preview.nodes.length === 0) {
    return (
      <p className="section-note">No nodes were published for this figure.</p>
    );
  }
  return (
    <>
      <figure className="workflow-figure">
        ...
        <figcaption>
          <strong>Figure 1.</strong> Published workflow structure for this exact
          release. This is the publisher&apos;s catalogue description; source
          comparison occurs during inspection.
        </figcaption>
      </figure>
      <details className="structure-outline">
        <summary>Textual structure</summary>
        <p>
          {preview.nodes.length} nodes and {preview.edges.length} transitions.
        </p>
        <ul>
          {preview.nodes.map(node => (
            <li key={node.id}>
              <code>{node.id}</code> — {node.kind.replaceAll('_', ' ')}:{' '}
              {node.name}
            </li>
          ))}
        </ul>
...
function renderDocumentation(
  ...
  markdown.renderer.rules.link_open = (
    ...
    const resolved = documentationHref(
      href,
      detail.repository,
      detail.commit,
      documentation.path,
    ...
    if (resolved === undefined) {
      tokens[index].attrSet('href', '#unsafe-link');
    } else {
      tokens[index].attrSet(
        'href',
        resolved.startsWith('#') ? resolved : '#open-link',
      );
      if (!resolved.startsWith('#')) {
        tokens[index].attrSet('data-verdog-href', resolved);
      }
    }
  ...
  const html = useMemo(
    () =>
      documentation.state === 'ready'
        ? renderDocumentation(documentation, detail)
        : undefined,
    [detail, documentation],
  );
  if (documentation.state === 'loading') {
    return (
      <p className="section-note">
        Retrieving the exact README without checking out a worktree…
      </p>
    );
  }
  if (documentation.state !== 'ready') {
    return (
      <div className={`documentation-state ${documentation.state}`}>
        ...
        <button
          onClick={() => host.postMessage({kind: 'retry-readme'})}
          type="button"
        >
          Retry documentation
        </button>
...
function StageRail({record}: {record: CatalogueRecord}) {
  ...
  return (
    <ol aria-label="Catalogue workflow stages" className="stage-rail">
      ...
      <li className={inspection === 'ready' ? 'complete' : inspection}>
        <span>2</span>
        <strong>Inspect</strong>
        <small>{inspectState}</small>
      </li>
      <li className={importState}>
        <span>3</span>
        <strong>Import</strong>
        <small>{importState}</small>
      </li>
    </ol>
  );
}
  ...
  return (
    <dl className="metadata">
      ...
      <div>
        <dt>Workflow</dt>
        <dd>
          <code>{detail.workflow_id}</code>
        </dd>
      </div>
      <div>
        <dt>Revision</dt>
        <dd>
          <code title={detail.commit}>{detail.commit}</code>
        </dd>
      </div>
      <div>
        <dt>Package</dt>
        <dd>
          <code>{detail.package ?? 'unavailable'}</code>
        </dd>
      </div>
      <div>
        <dt>Visibility</dt>
        <dd>{detail.visibility === 'public' ? 'Public' : 'Restricted'}</dd>
      </div>
      <div>
        <dt>Published</dt>
        <dd>{date(detail.published_at)}</dd>
      </div>
      {detail.updated_at ? (
        <div>
...
function RecordActions({record}: {record: CatalogueRecord}) {
  ...
  return (
    <div className="record-actions">
      <button
        onClick={() => host.postMessage({kind: 'copy-reference'})}
        type="button"
      >
        Copy reference
      </button>
      {inspection.state === 'incomplete' || inspection.state === 'mismatch' ? (
        <>
          <button
            onClick={() => host.postMessage({kind: 'show-output'})}
            type="button"
          >
            Show output
          </button>
          {inspection.folder ? (
            <button
              onClick={() => host.postMessage({kind: 'open-source'})}
              type="button"
            >
              Open source anyway
            </button>
      ...
      {inspection.state === 'ready' ? (
        <>
          <button
            ...
            type="button"
          >
            Open inspected source
          </button>
          <button
            className="primary"
            onClick={() => host.postMessage({kind: 'import'})}
            type="button"
          >
            Import into project…
          </button>
...
function CatalogueRecordPage({initial}: {initial: CatalogueRecord}) {
  ...
  if (record.state === 'loading') {
    return (
      <main className="record-loading">
        Retrieving the exact catalogue record…
      </main>
    );
  }
  if (record.detail === undefined) {
    return (
      <main className="record-loading error">
        {record.message ?? 'This record is unavailable.'}
      </main>
    );
  }
  ...
  return (
    <div className="record-page">
      <header className="record-header">
        <div className="record-heading">
          ...
          <p className="record-address">
            <code>{detail.workflow_id}</code> · {detail.repository}
          </p>
        </div>
      ...
      <main className="record-main">
        ...
        {record.message ? (
          <p aria-live="polite" className="record-message">
            {record.message}
          </p>
        ) : undefined}
        {record.inspection.state !== 'idle' &&
        record.inspection.state !== 'ready' ? (
          <p
            aria-live="polite"
            className={`inspection-note ${record.inspection.state}`}
          >
          ...
          <article>
            ...
            >
              ...
              {detail.preview === undefined ? (
                <p className="section-note">
                  A graph preview is unavailable for this legacy record.
                </p>
              ) : (
            ...
            >
              ...
              <table>
                <tbody>
                  <tr>
                    <th scope="row">Workflow identifier</th>
                    <td>
                      <code>{detail.workflow_id}</code>
                    </td>
                  </tr>
                  <tr>
                    <th scope="row">Python package</th>
                    <td>
                      <code>{detail.package ?? 'unavailable'}</code>
                    </td>
                  </tr>
                  <tr>
                    <th scope="row">Python constraint</th>
                    <td>{environment?.python ?? 'Not recorded'}</td>
                  </tr>
                  <tr>
                    <th scope="row">Schema version</th>
                    <td>{environment?.schema_version ?? 'Not recorded'}</td>
                  </tr>
                  {detail.preview === undefined
                    ? undefined
                    : Object.entries(detail.preview.ports).map(
                        ([port, node]) => (
                          <tr key={port}>
              ...
              {detail.preview && detail.preview.features.length > 0 ? (
                <div className="feature-list">
                  ...
                  <ul>
                    {detail.preview.features.map(feature => (
                      <li key={feature.id}>
                        <code>{feature.id}</code> — {feature.name}
              ...
              {environment === undefined ? (
                <p className="section-note">
                  Environment metadata is unavailable for this legacy record.
                </p>
              ) : environment.requirements.length === 0 ? (
                <p className="section-note">
                  No additional Python requirements are declared.
                </p>
              ) : (
            ...
            >
              ...
              {detail.closure.length === 0 ? (
                <p className="section-note">
                  No repository dependencies are recorded.
                </p>
              ) : (
                <div className="table-scroll">
                  <table>
                    ...
                    <tbody>
                      {detail.closure.map(pin => (
                        <tr key={`${pin.repository}@${pin.commit}`}>
                          <td>{pin.repository}</td>
                          <td>
                            <code title={pin.commit}>
              ...
              >
                {detail.releases.map((release, index) => (
                  <option key={release.id} value={release.id}>
                    {release.commit.slice(0, 12)} · {date(release.published_at)}
                    {index === 0 ? ' · latest visible' : ''}
                  </option>
                ))}
              </select>
            </section>
...
function Catalogue() {
  ...
  useEffect(() => {
    const listen = (event: MessageEvent) =>
      setMessage(event.data as HostToCatalogue);
    window.addEventListener('message', listen);
    host.postMessage({kind: 'ready'});
    return () => window.removeEventListener('message', listen);
  }, []);
  if (message === undefined) {
    return <p className="booting">Opening catalogue…</p>;
  }
