...
const entry: NavigationEntry = {
  children: [{ children: [], entity: "nodes", id: "propose", key: "node:propose", meta: [], scope: [] }],
  entity: "profiles", id: "generator", key: "profile:generator", meta: [], scope: ["main"],
};
const render = (page: NavigationPage): string => renderToStaticMarkup(createElement(Browser, {
  canGoBack: true, canGoForward: false, edit() {}, host: { postMessage() {} },
  overview: { scope: "main" }, page, pending: false, retainedRoutes: ["current"], route: "current",
}));

test("navigation exposes a scope breadcrumb and tree controls while references start collapsed", () => {
  const html = render({ category: "profiles", context: "Subroutine main", entries: [entry], title: "Profiles" });
...
test("entity IDs lead property-page headings, with the semantic kind kept separate", () => {
  const html = render({
    category: "entity", title: "Profile", kind: "profile", id: "generator", name: "Generator",
    context: "Workflow main", entity: "profiles", entries: [], fields: [], documents: [],
    resources: [], idWritable: true, nameWritable: true, constraintsWritable: false,
    resourcesWritable: false, settingsWritable: true,
  });
...
test("shared entity grouping remains expanded for deletion reviews", () => {
  const html = renderToStaticMarkup(createElement(EntityTree<NavigationEntry>, {
    items: [entry], renderItem: (item: NavigationEntry) => createElement("li", { key: item.key }, item.id),
  }));
