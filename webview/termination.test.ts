...
const analysis: TerminationNavigation = {
  ...
  state: {
    ...
    definition: {
      status: "not_certified", local_status: "certified", reason: "A called definition is not certified.",
      memory_states: 4, rules: 5, dependencies: [{ scope: "missing/main", node: "child" }],
      edges: { forward: "remaining", back: "remaining" },
      regions: [{
        id: "0", nodes: ["first", "second"], edges: ["forward", "back"],
        witnesses: [{ feature: "iterations", expression: "iterations↓", edges: ["forward"], opposing_edges: ["back"] }],
        cycle: [
          { node: "first", values: ["iterations>0", "phase=ready"], edge: "forward" },
...
test("termination presentation distinguishes local and aggregate certification and preserves advisory meaning", () => {
  ...
  assert.doesNotMatch(html, /<details[^>]*data-disclosure="termination\/region:0"[^>]*open/);
});
...
test("pending analysis exposes no old regions or actionable highlights", () => {
  ...
  assert.match(badge, /<button aria-label="Structural termination: Checking. Show analysis."/);
...
test("cycle rows preserve native order, observations, edge identities, and closing step", () => {
  ...
  ].map((entry) => ({ ...entry, key: `${entry.entity}:${entry.id}`, children: [], scope: [], meta: [] }));
  const html = renderToStaticMarkup(createElement(TerminationSection, {
    analysis: { ...analysis, entries }, onHighlight() {},
    renderEntry: (entry) => createElement("li", { key: entry.key }, createElement("a", { href: `#${entry.key}` }, entry.id)),
  }));
  assert.match(html, /<caption>One residual cycle<\/caption>/);
  const cycle = html.slice(html.indexOf("<tbody>"), html.indexOf("<\/tbody>"));
  assert.ok(cycle.indexOf('href="#nodes:first"') < cycle.indexOf('href="#nodes:second"'));
  assert.ok(cycle.indexOf('href="#edges:forward"') < cycle.indexOf('href="#edges:back"'));
