...
export class PropertyRequests {
  ...
  navigate(route: string): void {
    if (this.route !== route) ++this.visit;
    this.route = route;
  }

  submit(edit: NavigationPropertyEdit): PropertyRequest | undefined {
    if (this.pending !== undefined || this.route === undefined) return undefined;
  ...
  settle(result: PropertyResult): { request: PropertyRequest; current: boolean } | undefined {
