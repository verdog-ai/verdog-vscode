...
type Host = { postMessage(message: NavigationBrowserToHost): void };

declare function acquireVsCodeApi(): Host;

function EntryLink({ entry, host, label, route }: {
  ...
  const cancelClick = () => {
    if (clickTimer.current === undefined) return;
    window.clearTimeout(clickTimer.current);
    clickTimer.current = undefined;
  };
  const reveal = () => host.postMessage({ key: entry.key, kind: "reveal", route });
  useEffect(() => cancelClick, []);
  return (
    <li className="navigation-entry">
      <a
        aria-keyshortcuts={entry.declaration === undefined ? undefined : "Shift+Enter"}
        ...
        title={entry.declaration === undefined
      ...
      {entry.meta.map((item) => <span className="reason" key={item}>{item}</span>)}
      {(entry.removal !== undefined || entry.removalBlocked !== undefined) && (
        <button
          ...
          onClick={() => {
            if (entry.removal === undefined) return;
            cancelClick();
            host.postMessage({ key: entry.key, kind: "remove", route });
          }}
          title={entry.removalBlocked ?? `Delete ${entityLabel(entry.entity)} ${entry.id}`}
          type="button"
        >
      ...
      {entry.children.length > 0 && (
        <details className="references" data-disclosure={JSON.stringify(["references", entry.key])}>
          <summary>
          ...
          <ul className="tree">
            {entry.children.map((child) => (
              <EntryLink entry={child} host={host} key={child.key} route={route} />
            ))}
          </ul>
        </details>
      )}
...
function NavigationEntries({ entries, host, route }: {
  ...
  const expand = (open: boolean) => {
    for (const details of tree.current?.querySelectorAll("details") ?? []) details.open = open;
  };
  return (
    <div className="navigation-tree" ref={tree}>
      <div aria-label="Tree controls" className="tree-actions">
        <button onClick={() => expand(true)} type="button">Expand all</button>
        <button onClick={() => expand(false)} type="button">Collapse all</button>
      </div>
      <EntityTree
        items={entries}
        renderItem={(entry) => (
...
function PropertyPage({
  ...
  const resolved = page.entity === "profile_parameters" || page.entity === "session_parameters";
  return (
    <>
      <Properties
        ...
        onConstrain={page.entity === "edges" ? () => edit({ kind: "constrain" }) : undefined}
        ...
        onSessionPersistence={(persistent) => edit({ kind: "session-persistence", persistent })}
      ...
      {page.entries.length > 0 && (
        <details className="section" data-disclosure="components" open>
          <summary>
            <span>{call ? "Target" : resolved ? "Resolves to" : "Components"}</span>
...
export function Browser({
  ...
  useLayoutEffect(() => {
    ...
    const rememberDisclosure = (event: Event) => {
      ...
      if (key !== undefined) views.at(route).disclosures.set(key, details.open);
    };
    element.addEventListener("toggle", rememberDisclosure, true);
    views.restore(route, element);
    return () => element.removeEventListener("toggle", rememberDisclosure, true);
  }, [route, views]);
  useLayoutEffect(() => {
    if (main.current !== null) views.restoreDisclosures(route, main.current);
  }, [page, route, views]);
  useEffect(() => views.retain(retainedRoutes), [retainedRoutes, views]);
  const summary = page.category === "status"
    ...
    : page.category === "entity"
      ? page.entries.length === 0
        ...
        : page.kind === "subroutine_call" || page.kind === "workflow_call"
          ? "Properties · Target"
          : page.entity === "profile_parameters" || page.entity === "session_parameters"
  ...
  const footer = page.category === "status"
    ...
    <div className="identity-page">
      <header className="page-header">
        <div className="page-heading">
          {page.category === "entity" && <p className="page-kind">{page.title}</p>}
          <h1 className={page.category === "entity" ? "page-title-id" : undefined}>
            {page.category === "entity" ? page.id : page.title}
          </h1>
          {page.context !== undefined && (
            <nav aria-label="Current scope" className="scope-breadcrumb">
              {overview === undefined ? page.context : (
                <a
        ...
        <nav aria-label="Navigation history" className="history-actions">
          <button
            aria-label="Back"
            disabled={!canGoBack}
            onClick={() => host.postMessage({ direction: "back", kind: "navigate" })}
            title="Back"
            type="button"
          >
            ←
          ...
          <button
            ...
            onClick={() => host.postMessage({ direction: "forward", kind: "navigate" })}
            title="Forward"
            type="button"
          >
            →
      ...
      <main
        ...
        onFocusCapture={(event) => {
          const key = (event.target as HTMLElement).dataset.navigationFocus;
          if (key !== undefined) views.at(route).focus = key;
        }}
        onScroll={(event) => { views.at(route).scrollTop = event.currentTarget.scrollTop; }}
        ref={main}
        tabIndex={-1}
      >
        {(page.category === "status" || page.category === "entity") && page.termination !== undefined && (
          <TerminationSection
            analysis={page.termination}
            onHighlight={(region, revision) => host.postMessage({ kind: "termination-highlight", region, revision, route })}
            renderEntry={(entry, label) => <EntryLink entry={entry} host={host} key={entry.key} label={label} route={route} />}
          />
        )}
        {page.category === "entity" ? (
          <PropertyPage
            edit={edit}
            failedRequest={failedRequest?.route === route ? failedRequest : undefined}
            host={host}
            key={route}
            page={page}
            pending={pending}
          ...
          <>
            <dl className="status-list">
              <div>
                <dt>Graph</dt>
                <dd title={page.status.graphHash}><code>{page.status.graphHash.slice(0, 12)}</code></dd>
              </div>
              <div>
                <dt>Generated files</dt>
                <dd className={page.status.stale ? "status-warning" : "status-current"}>
                  {page.status.stale ? "stale" : "current"}
                </dd>
              </div>
              <div>
                <dt>Check</dt>
                <dd className={page.status.checked ? "status-current" : "status-warning"}>
                  {page.status.checked
              ...
              <div>
                <dt>Location</dt>
                <dd><code>{page.status.location}</code></dd>
              </div>
            </dl>
            {(!page.status.checked || page.status.stale) && (
              <p className="status-note">Run Verdog: Check to refresh this status.</p>
            )}
            {page.status.pinned && (
              <p className="status-note">
                Edits land in that submodule; check and commit there, then bump the parent pin.
              </p>
            )}
          </>
        ) : (
      ...
      <footer>
        ...
        <div className="actions">
          <button onClick={() => host.postMessage({ kind: "close" })} type="button">Close</button>
        </div>
      </footer>
    </div>
  );
...
function App({ host }: { host: Host }) {
  ...
  useEffect(() => {
    const listen = (event: MessageEvent) => {
      ...
      if (message.kind === "page") {
        ...
        setPending(false);
        if (!message.saved && completed.current) setFailedRequest(completed.request);
      }
    };
    const navigateWithKeys = (event: KeyboardEvent) => {
      if (
        ...
        event.preventDefault();
        host.postMessage({ direction: event.altKey ? "back" : "forward", kind: "navigate" });
        return;
      }
      if (
        event.key !== "Escape" ||
  ...
  return message === undefined
    ? <p className="loading" role="status">Preparing navigation…</p>
    : <Browser
