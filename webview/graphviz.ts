...
const PALETTE: Record<NodeKind, { fill: string; gradient: string; stroke: string }> = {
...
export const nodeDomId = (id: string): string => `verdog-node-${encodeURIComponent(id)}`;
export const edgeDomId = (id: string): string => `verdog-edge-${encodeURIComponent(id)}`;

const nodeLabel = ({ data }: SubroutineNode): string =>
...
export function graphvizSource(
  ...
  for (const node of graph.nodes) {
    ...
    const compact = node.data.kind === "feature"
      ? ', width=1.75, height=0.625, fontsize=14, margin="0.14,0.08"'
      : "";
    lines.push(
      `  ${quote(node.id)} [id=${quote(nodeDomId(node.id))}, label=${
        quote(nodeLabel(node))
