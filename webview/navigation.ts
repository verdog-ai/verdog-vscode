...
function initPath(path: string | undefined): string | undefined {
  return path?.endsWith('/__init__.py') ? path : undefined;
}
...
function declarationOf(documents: readonly DocumentLink[]): string | undefined {
  return documents
...
function callDefinitionKey(
  ...
  const external =
    kind === 'subroutine' ? parseQualifiedSubroutineTarget(target) : undefined;
  if (external === undefined) {
    return undefined;
  }
  const key = qualifiedKey(
    qualifiedKey(ownerPath, external.alias),
    definitionKey(kind, external.subroutine),
  );
  return available.has(key) ? key : undefined;
}
...
function definitionCallReferences(
  ...
  const owners: Array<[string | undefined, ProjectSnapshot['project']]> = [
  ...
  for (const [ownerPath, project] of owners) {
    ...
    for (const graph of subroutinesIn(project, index)) {
      ...
      for (const node of nodes) {
        if (!isCallNodeKind(node?.kind)) {
          continue;
        }
        const parentKey = callDefinitionKey(
          snapshot,
          caller,
          node,
          visible,
        ...
        if (siblings === undefined) {
          references.set(parentKey, [entry]);
        } else {
          siblings.push(entry);
        }
      }
  ...
  for (const definition of definitions) {
    ...
    const {ownerPath, subroutine} = subroutineAddress(
      snapshot.pinned,
      definition.target,
    );
...
function definitionNavigationTarget(
  item: DefinitionItem,
): NavigationTarget | undefined {
  const targetScope =
    item.ambiguous === true
      ? undefined
      : (item.scope ?? item.target ?? item.declaredIn);
  return targetScope === undefined
    ...
    : {
        inspection: inspection(
          ...
          item.id,
          item.ownerGraph,
          item.kind === 'workflow' && item.workflow !== undefined
            ? item.id
            : undefined,
        ),
        scope: targetScope,
...
function definitionEntry(
  ...
  const declaring =
    unavailable && item.declaredIn !== undefined
      ? definitions.find(
          ({kind, target}) =>
            kind === 'subroutine' && target === item.declaredIn,
        )
      : undefined;
  const declaration =
    declarationOf(item.documents) ?? declarationOf(declaring?.documents ?? []);
...
function parameterBindingReferences(
  ...
  return (
    definitionCallReferences(snapshot, indexes, definitions).get(key) ?? []
  );
}

type LocalIdentity = Exclude<DocumentedEntity, 'edges' | 'nodes'>;

function workflowCanvas(snapshot: ProjectSnapshot, current: string) {
  const definition = snapshotDefinitions(snapshot).find(
    ({key}) => key === current,
  );
  if (
    definition?.kind !== 'workflow' ||
...
function localIdentityEntries(
  ...
  if (canvas !== undefined) {
    ...
    const resources =
      category === 'profiles'
        ? agentProfiles(canvas.workflow).map(({id, provider}) => ({
            id,
    ...
    return entries(graph.features).flatMap(feature => {
      ...
      return [
        localIdentityEntry(
          ...
          edges
            .filter(edge =>
              [...entries(edge.conditions), ...entries(edge.effects)].some(
  ...
  const resources =
    category === 'profiles' ? agentProfiles(graph) : agentSessions(graph);
  const nodes = entries(graph.nodes);
  const bindings = parameterBindingReferences(snapshot, current);
  return resources.map(resource => {
    const entity: LocalIdentity =
      resource.origin === 'parameter'
        ? category === 'profiles'
          ? 'profile_parameters'
          : 'session_parameters'
        : category;
      ...
      const used =
        node.kind === 'agent'
          ...
          : node.kind === 'subroutine_call' &&
            Object.values(object(operation[`${field}_arguments`])).some(
              id => id === resource.id,
            );
...
function directComponent(entry: NavigationEntry): NavigationEntry {
  return {
    ...entry,
    scope: [],
  };
...
function canvasComponentEntry(
  ...
  const selection: CanvasSelection =
    entity === 'edges' ? {entity, id: edgeId(id)} : {entity, id: nodeId(id)};
  const declaration = entityDeclaration(snapshot, current, entity, id);
  return {
    children: [],
    ...(declaration === undefined ? {} : {declaration}),
...
export function definitionComponentEntries(
  ...
  if (inspected.entity !== 'subroutines' && inspected.entity !== 'workflows') {
    return [];
  }
  const indexes = snapshotDefinitionIndexes(snapshot);
  const definitions = snapshotDefinitions(snapshot, indexes);
  ...
  if (definition === undefined || definition.ambiguous === true) {
    return [];
  }
  ...
  if (current.scope.kind !== 'subroutine' || definition.target !== current.id) {
    return [];
  }
  const graph = object(subroutineIn(snapshot, current.id));
  const ports = new Set(
    Object.values(object(graph.ports)).filter(
  ...
  const nested = definitions
    ...
    .map(item =>
      definitionEntry(snapshot, visible, definitions, callers, item),
    );
  return [
    ...nodes,
    ...edges,
...
export function resolvedBindingEntries(
  ...
  if (
    inspected.entity !== 'profile_parameters' &&
    inspected.entity !== 'session_parameters'
  ) {
    return [];
  }
  const resource =
    inspected.entity === 'profile_parameters' ? 'profile' : 'session';
  const category = resource === 'profile' ? 'profiles' : 'sessions';
  const {ownerPath, subroutine} = subroutineAddress(
    snapshot.pinned,
    inspected.subroutine,
  );
  ...
  return resolveParameter(project, subroutine, resource, inspected.id).flatMap(
    binding => {
      const summary = bindingSummary(binding);
      const meta = [
        'resolves to',
        ...(summary.includes('(')
          ? [summary.slice(summary.indexOf('(') + 1, -1)]
          : []),
      ];
      if (binding.owner.kind === 'workflow') {
        const workflowKey = qualifiedKey(
          ownerPath,
          definitionKey('workflow', binding.owner.id),
        );
...
export function terminationNavigation(
  snapshot: ProjectSnapshot,
  current: string,
): TerminationNavigation {
  ...
  const edges = new Set([
    ...
    ...witnesses.flatMap(({edges, opposing_edges}) => [
      ...edges,
      ...opposing_edges,
    ]),
  ]);
  ...
  const references = definition.dependencies.flatMap(({scope, node}) => {
    const item =
      definitions.find(({key}) => key === scope) ??
      definitions.find(
        item => item.kind === 'subroutine' && item.scope === scope,
      );
    if (item === undefined) {
      return [];
    }
    ...
    return [
      {
        ...
        meta: [
          dependency.status === 'ready'
            ? TERMINATION_LABEL[dependency.definition.status]
            : 'Unavailable',
  ...
  const links = [
    ...[...nodes].map(id =>
      canvasComponentEntry(snapshot, current, 'nodes', id, false),
    ),
    ...[...edges].map(id =>
      canvasComponentEntry(snapshot, current, 'edges', id, false),
    ),
    ...localIdentityEntries(snapshot, current, 'features').filter(({id}) =>
      features.has(id),
    ),
    ...references,
  ];
  return {
    state,
    entries: links.map(
      ({removal: _removal, removalBlocked: _blocked, ...entry}) => ({
        ...entry,
        children: [],
        scope: [],
      }),
  ...
  if (category === 'status') {
    return {
      ...
      status: {
        ...
        location:
          ownerPath === undefined ? '/' : `/${packageDirectory(ownerPath)}`,
  ...
  const definition = snapshotDefinitions(snapshot).find(
    item =>
      ...
      (scope.kind === 'subroutine'
        ...
        : item.key === current.id &&
          item.id === scope.definition &&
          item.ownerGraph === scope.ownerGraph),
  );
  return definition === undefined
    ? undefined
    : definitionNavigationTarget(definition);
}
...
export function calledDefinitionEntry(
  ...
  if (!isCallNodeKind(node.data.kind)) {
    return undefined;
  }
  const indexes = snapshotDefinitionIndexes(snapshot);
  const definitions = snapshotDefinitions(snapshot, indexes);
  ...
  const {ownerPath} = subroutineAddress(snapshot.pinned, caller);
  const raw =
    current.scope.kind === 'workflow'
      ? node.id === 'subroutine'
        ? {
            kind: 'subroutine_call' as const,
            operation: {target: current.scope.workflow.subroutine},
          }
        : undefined
      : subroutineIn(snapshot, caller)?.nodes?.find(
...
function definitionForImpact(
  ...
  if (impact.entity !== 'subroutines' && impact.entity !== 'workflows') {
    return undefined;
  }
...
export function removalNavigationEntry(
  ...
  const documented =
    impact.entity !== 'subroutines' && impact.entity !== 'workflows'
      ? impact.entity
      : undefined;
  const declaration =
  ...
  const target =
    impact.entity === 'edges' || impact.entity === 'nodes'
      ? {
          selection:
            impact.entity === 'edges'
