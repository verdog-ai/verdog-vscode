import type { CanvasAction, CanvasSelection, NavigationCategory } from "../model/protocol";
...
const ADDITIONS = [
  ...
  ["add-definition", "+ definition", "Add a subroutine definition or make one runnable"],
...
export const additionsFor = (scope: "subroutine" | "workflow") =>
  scope === "workflow" ? WORKFLOW_ADDITIONS : ADDITIONS;

const PANELS: readonly [NavigationCategory, string][] = [
    ...
    <header className="bar">
        ...
        {writable && scope !== undefined ? (
          <span className="tools">
            {additionsFor(scope).map(([kind, label, title]) => (
              <button key={kind} onClick={() => onAction({ kind })} title={title} type="button">
                {label}
              </button>
            ))}
            {scope === "subroutine" && (
              <>
                {selected?.entity === "nodes" && (
                  <button
                    disabled={!canConnect}
                    onClick={() => onAction({ kind: "connect", source: selected.id })}
                ...
                {selected?.entity === "edges" && (["source", "target"] as const).map((endpoint) => (
                  <button
                    key={endpoint}
                    onClick={() => onAction({ edge: selected.id, endpoint, kind: "relink-endpoint" })}
                    type="button"
                  >
                    Change {endpoint}…
                  </button>
                ...
                <button
                  ...
                  onClick={() => {
                    if (selected?.entity === "edges") {
                      onAction({ entity: "edges", id: selected.id, kind: "remove" });
                    } else if (selected?.entity === "nodes") {
                      onAction({ entity: "nodes", id: selected.id, kind: "remove" });
                    }
                  }}
      ...
      <fieldset className="tool-group">
        ...
        <span className="tools">
          ...
          {PANELS.map(([kind, title]) => kind === "status" && termination !== undefined ? (
            <TerminationBadge
              key={kind}
