...
export function Canvas({host}: {host: CanvasHost}) {
  ...
  const [terminationHighlight, setTerminationHighlight] =
    useState<TerminationHighlight>();
  ...
  const moveThroughTraversal = useCallback(
    (direction: -1 | 1) => {
      cancelNavigation();
  ...
  const showPanel = useCallback(
    (kind: NavigationCategory | undefined) => {
      cancelNavigation();
      if (kind === undefined) {
    ...
    const listen = (event: MessageEvent) => {
      ...
      if (message.kind === 'termination-highlight') {
        ...
        setBrowserHistory(current =>
          current?.canGoBack === message.canGoBack &&
          current.canGoForward === message.canGoForward
            ? current
            : message,
        );
      } else if (message.kind === 'reveal') {
        if (message.navigationVersion < navigationVersionRef.current) {
          return;
        }
        navigationVersionRef.current = message.navigationVersion;
        if ('panel' in message) {
          setPanel(message.panel ?? undefined);
        }
        setPendingReveal({
  ...
  const highlightedRegion = highlightedTerminationRegion(
  ...
  useEffect(() => {
    if (highlightedRegion === undefined) {
      setTerminationHighlight(undefined);
    }
  }, [highlightedRegion, terminationHighlight]);
  const activeWorkflow = activeWorkflowScope(
    scopes,
    traversal.entries,
    traversal.cursor,
  );
  ...
  useEffect(() => {
    ...
    const inspection = keepBrowser
      ? currentDefinitionTarget?.inspection
      : undefined;
    setInspectionTarget(
      inspection === undefined || current === undefined
        ? undefined
        : {inspection, scope: current.id},
    );
    setSelected(undefined);
    if (keepBrowser && inspection === undefined) {
      setPanel('status');
    }
  }, [current?.id]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      (targetWorkflow !== undefined &&
        scopes[targetWorkflow.scope] === undefined)
    ) {
    ...
    if (
      targetWorkflow !== undefined &&
      workflow?.scope !== targetWorkflow.scope
    ) {
    ...
    if (
      ...
      current.nodes.some(({id}) => id === selection.id)
    ) {
      inspection ??=
    ...
    setInspectionTarget(
      inspection === undefined ? undefined : {...target, inspection},
    );
  ...
  const focused: CanvasSelection | undefined =
    inspectionScope === current?.id
      ? (inspectionSelection ??
  ...
  const openCall = useCallback(
    (node: SubroutineNode) => {
      ...
      if (child.scope !== undefined) {
        if (scopes[child.scope] !== undefined) {
          showScope(child.scope);
        }
        return;
      }
  ...
  const propertyInspection =
    inspecting === undefined || inspectionScope !== current?.id
      ? undefined
      : inspecting;
  const propertyPage =
    snapshot === undefined ||
  ...
  const browserTarget: NavigationTarget | undefined =
    current === undefined || browserPage === undefined
      ? undefined
      : propertyPage === undefined || propertyInspection === undefined
        ...
        : {
            ...
            ...(inspectionSelection === undefined
              ? propertyInspection.entity === 'edges' ||
                propertyInspection.entity === 'nodes'
  ...
  return (
    <div className="shell">
      <CanvasBar
        canConnect={
          ...
          current?.nodes.some(
            node =>
              ...
              current.nodes.some(
                target =>
                  edgeDirectionProblem(node.data.kind, target.data.kind) ===
                  undefined,
              ),
          ) === true
        }
        ...
        canGoForward={
          browserHistory?.canGoForward ??
          traversal.cursor + 1 < traversal.entries.length
        }
        onAction={send}
        onBack={() => navigate('back')}
        onForward={() => navigate('forward')}
        onOverview={
          currentDefinitionTarget === undefined
            ? undefined
            : () => {
                cancelNavigation();
                setPanel(undefined);
      ...
      <div className="flow">
        <GraphCanvas
          ...
          onConnect={(source, target) =>
            send({kind: 'connect', source, target})
          }
          onInspect={selection => {
            cancelNavigation();
            if (current !== undefined) {
          ...
          onOpenCall={openCall}
          onRelink={(edge, source, target) =>
            send({edge, kind: 'relink', source, target})
          }
