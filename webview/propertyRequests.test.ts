...
test('property saves gate duplicate events and match acknowledgements', () => {
  ...
  const request = requests.submit({
    kind: 'session-persistence',
    persistent: true,
  });
  assert.ok(request);
  ...
  assert.equal(
    requests.settle({...result, requestId: result.requestId + 1}),
    undefined,
  );
  ...
  assert.deepEqual(
    requests.settle({...result, requestId: retry.requestId, saved: true}),
    {
      request: retry,
      current: true,
    },
