...
/** @fileoverview Turn the shared project snapshot into the graphs the canvas draws. */
    ...
    return {
      ...
      ...(definition.kind === 'workflow'
        ? {
            scope: qualifiedKey(
              ownerPath,
              definitionKey('workflow', definition.id),
            ),
          }
        : {}),
    };
...
function externalSubroutineRef(
  ...
  if (
    definitionIndex(snapshot.pinned[aliasPath]).subroutines.get(
      subroutineId,
    ) === undefined
  ) {
    return undefined;
...
function implementationOf(
  ...
  const ownerPath = reference.alias
    ...
    : reference.ownerPath;
  const project =
    ownerPath === undefined ? snapshot.project : snapshot.pinned[ownerPath];
...
export function snapshotCanvasGraphs(snapshot: ProjectGraphs): SubroutineMap {
  ...
  const workflows = snapshotDefinitions(snapshot).flatMap(definition => {
    ...
    if (target === undefined || target.scope.kind !== 'subroutine') {
      return [];
    }
    ...
    return [[definition.key, graph] as [string, SubroutineGraph]];
  });
...
export function snapshotDefinitionIndexes(
  ...
  return new Map([
    ...
    ...Object.entries(snapshot.pinned).map(
      ([ownerPath, project]) => [ownerPath, definitionIndex(project)] as const,
    ),
  ]);
}
...
export function visibleDefinitionKeys(
  ...
  for (const target of externalSubroutineTargets(snapshot, current)) {
    ...
    keys.add(
      qualifiedKey(pin, definitionKey('subroutine', external.subroutine)),
    );
  }
  return keys;
}
...
function definitionDocuments(
  ...
  if (definition.path === undefined || definition.external !== undefined) {
    return [];
  }
  const base = [
    'src',
  ...
  const present = new Set(
    array(object(project).sources)
      .map(source => text(object(source).path))
      .filter(Boolean),
  );
  return definitionDocumentPaths(base, definition.kind).flatMap(
    ({label, path}) => {
  ...
  return subroutinesIn(project).map(subroutine => {
    const nodes = entries(subroutine.nodes).flatMap(
      (node): SubroutineNode[] => {
        ...
        const definition =
          definitionRef(
            snapshot,
            resolveCall(project, subroutine.id, {kind, operation}),
            pinnedFrom,
          ) ??
          (kind === 'subroutine_call'
            ? externalSubroutineRef(
                snapshot,
    ...
    const edges: SubroutineEdge[] = entries(subroutine.edges)
      .filter(
        edge =>
          text(edge.id) &&
          executableIds.has(text(edge.source)) &&
          executableIds.has(text(edge.target)),
      )
      .map(edge => ({
