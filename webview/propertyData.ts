...
/** @fileoverview Properties projected from the current graph. */
...
import {featureId, graphId, type GraphId} from '../model/identifiers';
...
function isParameterEntity(
  kind: string,
): kind is 'profile_parameters' | 'session_parameters' {
  return kind === 'profile_parameters' || kind === 'session_parameters';
}
...
export function constraintNotation(
  ...
  if (collection === 'conditions') {
    ...
    if (observation === 'equal_zero') {
      return `${id}=0`;
    }
    if (observation === 'greater_zero') {
      return `${id}>0`;
    }
    ...
    if (observation === 'increases') {
      return `${id}↑`;
    }
    if (observation === 'decreases') {
      return `${id}↓`;
    }
    if (observation === 'unchanged') {
      return `${id}=`;
    }
    if (observation === 'unconstrained') {
...
function resourceDetails(
  ...
  const persistent =
    kind === 'sessions' && typeof found.persistent === 'boolean'
      ? found.persistent
      : undefined;
...
/** Lists authored, visit, and called implementation documents for one entity. */
function propertyDocuments(
  snapshot: ProjectSnapshot,
  current: SubroutineGraph,
  inspection: NavigationInspection,
  workflowResource: boolean,
): PropertyNavigationPage['documents'] {
  const documentOwner = workflowResource ? current.id : inspection.subroutine;
  const documentRefs =
    inspection.entity === 'subroutines' || inspection.entity === 'workflows'
      ? undefined
      : snapshot.entity_documents[inspection.entity][documentOwner]?.[
          inspection.id
        ];
  if (
    inspection.entity === 'subroutines' ||
    inspection.entity === 'workflows'
  ) {
    return (
      snapshotDefinitions(snapshot).find(
        definition =>
...
function definitionDetails(
  ...
  const root =
    definition?.declaredIn === undefined && definition?.id === graphIdentifier;
  if (definition?.declaredIn !== graphIdentifier && !root) {
    return undefined;
  }
  ...
  return {
    ...
    idEditable:
      !root &&
      (definition.kind === 'subroutine' || definition.external !== undefined),
...
export function entityDetails(
  ...
  if (kind === 'workflows' || kind === 'subroutines') {
    return definitionDetails(project, graphIdentifier, kind, id);
  }
  ...
  if (kind === 'nodes') {
    return nodeDetails(project, graphIdentifier, found, id);
  }
  if (kind !== 'edges') {
    if (isParameterEntity(kind)) {
      return resourceDetails(
        ...
        id,
        true,
        resolutionFields(project, graphIdentifier, kind, id),
      );
    }
    if (kind === 'profiles' || kind === 'sessions') {
      return resourceDetails(kind, found, id, true);
    }
    if (kind === 'features') {
      ...
      if (Array.isArray(found.values)) {
        fields.push({
          label: 'values',
          value: found.values.map(String).join(', '),
        });
      }
    }
    return {
  ...
  return edgeDetails(graph, found, id);
}
...
function propertyTitle(entity: Entity, kind: string): string {
  if (entity !== 'nodes') {
    return PROPERTY_TITLES[entity];
  }
...
function calledEntries(
  snapshot: ProjectSnapshot,
  current: SubroutineGraph,
  nodeId: string | undefined,
): NavigationEntry[] | undefined {
  const node = current.nodes.find(({id}) => id === nodeId);
  if (
    node === undefined ||
    (node.data.kind !== 'subroutine_call' && node.data.kind !== 'workflow_call')
  ) {
    return undefined;
  }
  const target = calledDefinitionEntry(snapshot, current, node);
  if (target === undefined) {
    return undefined;
  }
  const {removal: _removal, ...entry} = target;
...
export function entityPropertyPage(
  ...
  if (workflowResource) {
    details = workflowResourceDetails(
  ...
  const resources =
    workflowScope !== undefined && inspection.entity === 'workflows'
  ...
  const callEntries = calledEntries(
    snapshot,
    current,
    workflowCall
      ? selection.id
  ...
  return {
    ...
    fields: workflowCall
      ? details.fields.map(field =>
          field.label === 'subroutine' ? {...field, label: 'calls'} : field,
        )
      : details.fields,
    ...
    title: workflowCall
      ? 'Subroutine Call'
      : propertyTitle(inspection.entity, details.kind),
    ...(!workflowCall &&
    (inspection.entity === 'subroutines' || inspection.entity === 'workflows')
