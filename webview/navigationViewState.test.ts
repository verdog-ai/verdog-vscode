...
test("route presentation restores disclosures, scroll and row focus without moving the viewport", () => {
  ...
  const row = {
    ...
    focus: (options: FocusOptions) => { assert.equal(options.preventScroll, true); focused = "row"; },
  };
  const main = {
    scrollTop: 0,
    querySelectorAll: (selector: string) => selector.startsWith("details") ? details : [row],
    focus: () => { focused = "main"; },
  } as unknown as HTMLElement;
  views.restore("profiles", main);
  assert.equal(main.scrollTop, 240);
  assert.deepEqual(details.map(({ open }) => open), [false, true, true]);
  assert.equal(focused, "row");
  visible = false;
  views.restore("profiles", main);
  assert.equal(focused, "main", "a row hidden inside a collapsed branch cannot take focus");
