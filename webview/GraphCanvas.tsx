...
import { edgeTerminationStatus, type DefinitionTerminationState, type EdgeTerminationDisplayStatus, type TerminationRegion } from "../model/termination";
import { edgeDomId, graphvizSource, nodeDomId } from "./graphviz";
import type { SubroutineEdge, SubroutineGraph, SubroutineNode } from "./subroutines";
...
function graphPoint(container: HTMLElement, clientX: number, clientY: number): Position | undefined {
...
function viewportCentre(container: HTMLElement): readonly [number, number] | undefined {
...
const nodeBox = (node: SVGGElement): DOMRect =>
  node.querySelector<SVGGraphicsElement>("path, polygon, ellipse")?.getBBox() ?? node.getBBox();

function edgeEnds(edge: SVGGElement): Record<Endpoint, Position> | undefined {
  const path = edge.querySelector<SVGPathElement>(":scope > path:not(.graph-edge-hit)");
  ...
  return {
    ...
    target: arrowBox === undefined
      ? end
      : { x: arrowBox.x + arrowBox.width / 2, y: arrowBox.y + arrowBox.height / 2 },
  };
}
...
function nodeBounds(container: HTMLElement, ids?: readonly NodeId[]): Bounds | undefined {
...
const graphBounds = (container: HTMLElement): Bounds | undefined => union(
  [...container.querySelectorAll<SVGGElement>("g.node, g.edge")].map((entity) => entity.getBBox()),
);
...
function decorate(
  ...
  const nodes = new Map((graph?.nodes ?? []).map((node) => [nodeDomId(node.id), node]));
  for (const element of container.querySelectorAll<SVGGElement>("g.node")) {
    const node = nodes.get(element.id);
    const active = node !== undefined && (
      (focused?.entity === "nodes" && focused.id === node.id) ||
      (selected?.entity === "nodes" && selected.id === node.id)
    );
    element.classList.toggle("is-focused", active);
    const highlighted = node !== undefined && highlightedRegion?.nodes.includes(node.id) === true;
    element.classList.toggle("is-termination", highlighted);
    element.setAttribute("tabindex", "0");
    element.setAttribute("role", "button");
    element.setAttribute("aria-label", `Node ${node?.id ?? element.id}. Enter to inspect; Shift+Enter to enter a call.${highlighted ? " Highlighted residual region." : ""}`);
    element.querySelector(".graph-connect-handle")?.remove();
    if (!writable || node === undefined || !graph?.nodes.some((target) =>
  ...
  const edges = new Map((graph?.edges ?? []).map((edge) => [edgeDomId(edge.id), edge]));
  for (const element of container.querySelectorAll<SVGGElement>("g.edge")) {
    const edge = edges.get(element.id);
    const active = edge !== undefined && (
      (focused?.entity === "edges" && focused.id === edge.id) ||
      (selected?.entity === "edges" && selected.id === edge.id)
    );
    element.classList.toggle("is-focused", active);
    const highlighted = edge !== undefined && highlightedRegion?.edges.includes(edge.id) === true;
    element.classList.toggle("is-termination", highlighted);
    element.setAttribute("tabindex", "0");
    element.setAttribute("role", "button");
    ...
    element.setAttribute("aria-label", `${label} Enter to inspect.${highlighted ? " Highlighted residual region." : ""}`);
    const title = element.querySelector(":scope > title");
    if (title !== null) title.textContent = `${label} ${badge.description}`;
    element.querySelectorAll(".graph-edge-hit, .graph-edge-source-handle, .graph-edge-status")
    ...
    if (arrow !== null) {
      if (writable) arrow.dataset.relinkEndpoint = "target";
      else delete arrow.dataset.relinkEndpoint;
    }
    const ends = edgeEnds(element);
    if (!writable || edge === undefined || ends === undefined) continue;
    const handle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
...
export const GraphCanvas = forwardRef<GraphCanvasHandle, CanvasProps>(function GraphCanvas(
  props,
  ref,
) {
  const { current, focused, highlightedRegion, selected, termination, writable } = props;
  ...
  const zoomParts = useCallback(() => {
    ...
    return behavior === null || behavior === undefined || selection === null || selection === undefined
      ? undefined
      : { behavior, selection };
  }, []);

  const centre = useCallback((ids?: readonly string[]) => {
    const container = containerRef.current;
    const zoom = zoomParts();
    if (container === null || zoom === undefined) return;
    const bounds = nodeBounds(container, ids);
    const point = viewportCentre(container);
    if (bounds === undefined || point === undefined) return;
  ...
  const applyReveal = useCallback((request: RevealRequest) => {
  ...
  const reveal = useCallback((graph: string, ids: readonly NodeId[]) => {
    const request = {
      graph,
      ids: [...ids],
  ...
  const fit = useCallback(() => {
    ...
    const svg = container.querySelector<SVGSVGElement>("svg");
    const point = viewportCentre(container);
    if (bounds === undefined || svg === null || point === undefined) return;
    const view = svg.viewBox.baseVal;
    const scale = Math.max(
      MIN_SCALE,
      Math.min(MAX_SCALE, (view.width - 30) / bounds.width, (view.height - 30) / bounds.height),
    );
  ...
  const renderGraph = useCallback((
    ...
    if (!force && key !== undefined && key === renderedKeyRef.current) return;
    ...
    nodesRef.current = new Map((graph?.nodes ?? []).map((node) => [nodeDomId(node.id), node]));
    edgesRef.current = new Map((graph?.edges ?? []).map((edge) => [edgeDomId(edge.id), edge]));
    const rendered = graph === undefined
      ...
      .dot(rendered.dot, function layoutReady() {
        ...
        this.render(() => {
          ...
          if (graph !== undefined && requested?.graph === graph.id) {
            ...
            requestAnimationFrame(previous === undefined ? fit : () => centre());
          }
        });
      });
  ...
  const tidy = useCallback(() => {
    ...
    if (graph === undefined || !propsRef.current.writable) return;
    revealedRef.current = undefined;
  ...
  useEffect(() => {
    ...
    const entity = (target: EventTarget | null): CanvasSelection | undefined => {
      const group = groupAt(target);
      if (group === undefined || !container.contains(group)) return undefined;
      const node = nodesRef.current.get(group.id);
      if (node !== undefined) return { entity: "nodes", id: node.id };
    ...
    const stopGesture = () => {
      ...
      if (connection !== undefined && connection.capture.hasPointerCapture(connection.pointerId)) {
        connection.capture.releasePointerCapture(connection.pointerId);
      }
      renderer.removeDrawnEdge();
      connectionRef.current = undefined;
      container.querySelectorAll(".graph-drop-valid, .graph-drop-invalid").forEach((node) =>
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (selection !== undefined) propsRef.current.onInspect(selection);
    };
    const onDoubleClick = (event: MouseEvent) => {
      ...
      if (selection.entity === "nodes") {
        const node = [...nodesRef.current.values()].find(({ id }) => id === selection.id);
    ...
    const onFocus = (event: FocusEvent) => {
      ...
      if (!groupAt(event.target)?.matches(":focus-visible")) return;
      const selection = entity(event.target);
      if (selection === undefined) return;
      propsRef.current.onSelect(selection);
      const edge = selection.entity === "edges"
        ? [...edgesRef.current.values()].find(({ id }) => id === selection.id)
        : undefined;
      centre(selection.entity === "nodes" ? [selection.id] : edge === undefined ? [] : [edge.source, edge.target]);
    };
    const onEntityKey = (event: KeyboardEvent) => {
      ...
      event.preventDefault();
      event.stopPropagation();
      const node = selection.entity === "nodes"
        ? [...nodesRef.current.values()].find(({ id }) => id === selection.id)
        : undefined;
      if (event.key === "Enter" && event.shiftKey && node !== undefined && isCallNodeKind(node.data.kind)) {
        propsRef.current.onOpenCall(node);
      } else {
    ...
    const candidate = (event: PointerEvent, connection: Connection) => {
      const group = document.elementsFromPoint(event.clientX, event.clientY)
        .map((element) => element.closest<SVGGElement>("g.node"))
        .find((element): element is SVGGElement => element !== null && container.contains(element));
      const dropped = group === undefined ? undefined : nodesRef.current.get(group.id);
      const node = (id: NodeId) => [...nodesRef.current.values()].find((item) => item.id === id);
      const source = connection.kind === "connect"
        ? connection.source
        : connection.endpoint === "source" ? dropped : node(connection.edge.source);
      const target = connection.kind === "connect" || connection.endpoint === "target"
        ? dropped : node(connection.edge.target);
      const problem = source === undefined || target === undefined
    ...
    const stopEntityStart = (event: Event) => {
      if (groupAt(event.target) !== undefined) event.stopPropagation();
    };
    const onPointerDown = (event: PointerEvent) => {
      ...
      const edge = edgesRef.current.get(group.id);
      container.parentElement?.focus();
      if (!propsRef.current.writable || !(event.target instanceof Element)) return;
      const endpoint = event.target
        .closest<SVGGraphicsElement>("[data-relink-endpoint]")
      ...
      if (edge !== undefined && endpoint !== undefined) {
        ...
        if (node === undefined || event.target.closest(".graph-connect-handle") === null) return;
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (point === undefined) return;
      const connection = connectionRef.current;
      if (connection?.pointerId !== event.pointerId) return;
      const dropped = candidate(event, connection);
      container.querySelectorAll(".graph-drop-valid, .graph-drop-invalid").forEach((node) =>
        node.classList.remove("graph-drop-valid", "graph-drop-invalid")
      );
      dropped.group?.classList.add(dropped.problem === undefined ? "graph-drop-valid" : "graph-drop-invalid");
      setGestureStatus(dropped.problem ?? `${dropped.source!.id} → ${dropped.target!.id}`);
      const [source, target] = previewEnds(connection, point);
    ...
    const onPointerUp = (event: PointerEvent) => {
      ...
      stopGesture();
      if (!propsRef.current.writable) return;
      if (problem !== undefined || source === undefined || target === undefined) {
        setGestureStatus(problem);
        return;
      }
      if (connection.kind === "connect") {
        propsRef.current.onConnect(source.id, target.id);
      } else if (source.id !== connection.edge.source || target.id !== connection.edge.target) {
    ...
    const observer = new ResizeObserver(([entry]) => {
      if (entry === undefined || entry.contentRect.width < 1 || entry.contentRect.height < 1) return;
  ...
  useEffect(() => {
    const container = containerRef.current;
    if (container === null) return;
    decorate(container, current, focused, selected, writable, highlightedRegion, termination);
    if (
      selected !== undefined
  ...
  const deleteSelection = (event: ReactKeyboardEvent<HTMLDivElement>) => {
    ...
    if (!writable || (event.key !== "Delete" && event.key !== "Backspace")) return;
    if (
      event.target instanceof HTMLElement
      && event.target.closest("button,input,select,textarea,[contenteditable=true]") !== null
    ) return;
    if (propsRef.current.selected === undefined) return;
    event.preventDefault();
  ...
  const zoomFromCentre = (factor: number) => {
    ...
    if (zoom === undefined || point === undefined) return;
  ...
  return (
    ...
    >
      ...
      {writable && current?.scope.kind === "subroutine" && current.nodes.every(({ data }) =>
        data.kind === "enter" || data.kind === "exit" || data.kind === "failure"
      ) && (
        <p className="graph-hint">
          Add a node or call, then select it and choose Connect to… or drag its circle.
        </p>
      )}
      <div aria-live="polite" className="graph-gesture-status" role="status">{gestureStatus}</div>
      <div aria-label="Graph zoom" className="graph-controls">
        <button aria-label="Zoom in" onClick={() => zoomFromCentre(ZOOM_FACTOR)} type="button">
          +
        </button>
        <button
          aria-label="Zoom out"
        ...
        <button aria-label="Fit graph" onClick={fit} type="button">□</button>
      </div>
    </div>
  );
});
