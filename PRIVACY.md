...
The extension stores its Verdog session token in VS Code SecretStorage, bound to the configured backend and GitHub account. VS Code manages its GitHub sign-in session. Neither token is written into project files, logs, or command arguments by the extension. Separate terminal CLI sessions may still store credentials in `$XDG_CONFIG_HOME/verdog/session.json` or `~/.config/verdog/session.json`; those are not the extension's sign-in storage.
...
Catalogue operations invoke VS Code's built-in GitHub sign-in with the `read:user` scope. The extension sends that GitHub token to the configured backend's `/api/v1/auth/github` endpoint and stores the returned Verdog session token in SecretStorage. Authenticated requests carry the Verdog token in an `Authorization: Bearer` header. The backend receives the account ID, username, and any profile fields returned by GitHub, and retains the GitHub token for permission checks. `read:user` does not grant access to private repositories. The extension does not silently request broader scopes or contain GitHub App secrets.
